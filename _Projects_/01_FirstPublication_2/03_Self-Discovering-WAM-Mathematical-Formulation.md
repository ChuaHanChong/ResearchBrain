...
> - $c \in \{1, 2, 5, 6\}$ — 2×2 cell index (1/2 are UWM; 5/6 are Cosmos Policy; odd cells use ACE, even cells use STAC); $c_\text{inj} \in \{00, 01, 10, 11\}$ — injection class (subscript to distinguish from cell index).
...
$$\mathcal{F}_\theta\colon (o_{1:t}, l) \;\mapsto\; \bigl(\hat{O}_{t+1},\; \mathbf{a}_{t:t+H}\bigr)$$

with conditioning $c = (o_{1:t}, l)$ fed to the shared DiT. The two backbones use **different diffusion parameterizations**:
...
Each cell of the 2×2 grid pairs one imag signal with one act signal:

| Axis | Options | Input | Output |
|---|---|---|---|
| **Imag** | `logpZO` | $\hat{O}_{t+1}$ | $s_\text{imag}(t) \in \mathbb{R}_{\geq 0}$ |
...
==We apply (3.1)–(3.2) to the WM-predicted next frame $\hat{O}_{t+1}$ instead of the real observation $O_t$.== Per-cell (cells 1, 2, 5, 6 of the 2×2), we train a **per-backbone CNF** $N_{\psi_A}, N_{\psi_B}$ on success-rollout $\hat{O}_{t+1}$ distributions:
...
## 4. Act Signal #1 — FIPER-ACE

### 4.1 Anchor formulation (3-D actions, [[2510.09459|FIPER]] `fiper/evaluation/method_eval_classes/entropy_eval.py:56-104`)
...
$$\mathcal{H}_\text{ACE}^{(3D)}\bigl(\{\mathbf{a}^{(i)}_t\}\bigr) \;=\; -\sum_{k=1}^{K_\text{cells}} \hat{p}_k \log_2 \hat{p}_k,\qquad \hat{p}_k = \frac{n_k}{\sum_{k'} n_{k'}},\quad n_k \text{ count in cell } k. \tag{4.1}$$
...
$$s_\text{act}^\text{ACE, 3D}(t) \;=\; \frac{1}{H}\sum_{h=0}^{H-1} \mathcal{H}_\text{ACE}^{(3D)}\!\bigl(\{\mathbf{a}^{(i)}_{t+h}\}_i\bigr). \tag{4.2}$$

### 4.2 Dimensionality problem
...
### 4.3 Our generalization (==our design==)
...
$$\boxed{\; s_\text{act}^{\text{ACE-marginal}}(t) \;=\; \frac{1}{H d_a}\sum_{h=0}^{H-1}\sum_{j=1}^{d_a} \mathcal{H}_\text{ACE}^{(1D)}\!\bigl(\{a^{(i)}_{t+h,j}\}_i\bigr) \;} \tag{4.3}$$
...
$$\boxed{\; s_\text{act}^{\text{ACE-PCA}}(t) \;=\; \mathcal{H}_\text{ACE}^{(d')}\!\bigl(\{\mathrm{PCA}_{d'}(\mathbf{a}^{(i)}_{t+h})\}_{i,h}\bigr). \;} \tag{4.4}$$
...
## 5. Act Signal #2 — Sentinel-STAC

### 5.1 MMD with RBF kernel ([[2410.04640|Sentinel]] `sentinel/bc/ood_detection/error_utils.py:62-93`)
...
$$\mathrm{MMD}^2_\text{biased}(\mathbf{X}, \mathbf{Y}) \;=\; \tfrac{1}{K_\text{samp}^2}\!\sum_{i,i'} k(\mathbf{x}^{(i)}, \mathbf{x}^{(i')}) \;+\; \tfrac{1}{K_\text{samp}^2}\!\sum_{j,j'} k(\mathbf{y}^{(j)}, \mathbf{y}^{(j')}) \;-\; \tfrac{2}{K_\text{samp}^2}\!\sum_{i,j} k(\mathbf{x}^{(i)}, \mathbf{y}^{(j)}). \tag{5.1}$$
...
### 5.2 Our default — STAC-all over flattened chunk

==We pre-register `mmd_rbf_all` on the **full flattened action chunk** $\mathbf{a}^{(i)}_{t:t+H} \in \mathbb{R}^{H d_a}$== (not position-only), because the 2×2 design evaluates decorrelation on 7-DoF action spaces and position-only is the same ablation issue as FIPER-ACE's 3-D hardcoding (R5 mirror).

Per-timestep STAC-256 signal:

$$\boxed{\; s_\text{act}^\text{STAC-256}(t) \;=\; \mathrm{MMD}^2\!\bigl(\{\mathbf{a}^{(i)}_{t:t+H}\}_{i=1}^{256},\;\{\mathbf{a}^{(i)}_{t-1:t-1+H}\}_{i=1}^{256}\bigr). \;} \tag{5.2}$$

### 5.3 STAC-single ([[2506.09937|SAFE]] §5 fallback)
...
$$\boxed{\; s_\text{act}^\text{STAC-single}(t) \;=\; 1 \;+\; \tfrac{1}{K_\mathcal{P}^2}\!\sum_{j,j'} k(\mathbf{y}^{(j)}, \mathbf{y}^{(j')}) \;-\; \tfrac{2}{K_\mathcal{P}}\!\sum_{j} k(\mathbf{a}_{t:t+H},\, \mathbf{y}^{(j)}). \;} \tag{5.3}$$

The constant $+1$ is retained in the biased form for consistency with (5.1); it shifts the signal by a constant and does not affect CP calibration (which standardizes via per-timestep means in §7). This reduces per-step cost from $K_\text{samp} = 256$ diffusion samples of the action branch to 1, trading variance for ~2 orders of magnitude compute savings. Deployed by default on Cell 6 (Cosmos × STAC) if compute-bound.

---

## 6. Episode Aggregation — Max-So-Far Convention
...
$$\boxed{\; R_\text{imag}(\omega) \;=\; \max_{1 \leq t \leq T} s_\text{imag}(t),\qquad R_\text{act}(\omega) \;=\; \max_{1 \leq t \leq T} s_\text{act}(t). \;} \tag{6.1}$$
...
**Proposition 6.1 (Measurability).** If $s_\text{imag}, s_\text{act}: \{1,\ldots,T\} \to \mathbb{R}$ are measurable random functions of the trajectory $\omega$, then $R_\text{imag}(\omega), R_\text{act}(\omega)$ are measurable real-valued random variables as finite maxima of measurable functions.

**Proposition 6.2 (Exchangeability preservation).** If $\{\omega_i\}_{i=1}^N$ are exchangeable success rollouts and $s_\text{imag}, s_\text{act}$ are deterministic functions of $\omega$ (modulo independent sampling noise for STAC), then $\{R_\text{imag}(\omega_i)\}_{i=1}^N, \{R_\text{act}(\omega_i)\}_{i=1}^N$ remain exchangeable. This is the hypothesis Conformal Prediction requires.

---

## 7. Success-Only Functional Conformal Prediction + Bonferroni

### 7.1 Per-axis per-timestep CP (from [[2510.09459|FIPER]] §3.4; [[2503.08558|FAIL-Detect]] §3.3)
...
$$\mu_t^X \;=\; \tfrac{1}{N}\sum_{i=1}^{N} S_t^{X,(i)}. \tag{7.1}$$
...
$$D^{(i)} \;=\; \max_{t} \frac{S_t^{X,(i)} - \mu_t^X}{h_t^X},\qquad h_t^X = \mathrm{MAD}\bigl(\{S_t^{X,(i)}\}_{i}\bigr) + \epsilon, \tag{7.2}$$
...
$$q_\alpha^X \;=\; \text{Quantile}_{1-\alpha}\!\bigl(\{D^{(i)}\}_{i=1}^N\bigr). \tag{7.3}$$
...
### 7.2 Per-timestep threshold (one-sided, anomaly direction)

$$\tau_X(t) \;=\; \mu_t^X \;+\; q_\alpha^X \cdot h_t^X. \tag{7.4}$$

### 7.3 Bonferroni correction for joint FPR (==our design==)
...
$$\mathbb{P}\!\bigl(R_\text{imag}(\omega_\text{test}) > \tau_\text{imag}(t_\text{test}) \;\big|\;\text{success}\bigr) \leq \alpha/2, \tag{7.5a}$$

$$\mathbb{P}\!\bigl(R_\text{act}(\omega_\text{test}) > \tau_\text{act}(t_\text{test}) \;\big|\;\text{success}\bigr) \leq \alpha/2, \tag{7.5b}$$

each under exchangeability of the calibration set and the test point.

### 7.4 Joint FPR guarantee (proof)

**Proposition 7.1 (Joint FPR control via Bonferroni).** Let $E_\text{imag} = \{R_\text{imag} > \tau_\text{imag}\}$ and $E_\text{act} = \{R_\text{act} > \tau_\text{act}\}$ denote the per-axis Type-I errors on a held-out success rollout. If (8.5a)–(8.5b) hold at level $\alpha/2$ each, then

$$\mathbb{P}(E_\text{imag} \cup E_\text{act} \mid \text{success}) \;\leq\; \mathbb{P}(E_\text{imag}) + \mathbb{P}(E_\text{act}) \;\leq\; \tfrac{\alpha}{2} + \tfrac{\alpha}{2} \;=\; \alpha. \tag{7.6}$$
...
### 7.5 Why Bonferroni (not copula) for the headline
...
## 8. The 2×2 Attribution Gate (Per Cell)

### 8.1 Binary axis decisions

Given per-cell thresholds $\tau_\text{imag}(t), \tau_\text{act}(t)$, the per-episode binary decisions are

$$b_\text{imag}(\omega) = \mathbb{1}\!\bigl[R_\text{imag}(\omega) > \tau_\text{imag}(t^*)\bigr],\qquad b_\text{act}(\omega) = \mathbb{1}\!\bigl[R_\text{act}(\omega) > \tau_\text{act}(t^*)\bigr], \tag{8.1}$$
...
$$\boxed{\; \ell(\omega) \;=\; \bigl(b_\text{imag}(\omega),\; b_\text{act}(\omega)\bigr)_2 \quad\in\{00, 01, 10, 11\}. \;} \tag{8.2}$$

Every cell of the 2×2 factorial instantiates **the same** $\ell$ under its own signal implementations and thresholds.

---

## 9. Correctness Claims (A, B, C) — Formal Propositions + Falsifiers
...
### 9.1 Claim A — `logpZO` on $\hat{O}_{t+1}$ discriminates imag failure

> **Proposition A.** For each backbone $X \in \{A, B\}$, the imag signal $s_\text{imag} = \text{logpZO}$ separates success from `10`-class rollouts, i.e.
> $$\mathrm{AUROC}\bigl(s_\text{imag}\;;\;\text{success vs. } c{=}10\bigr) \;\geq\; 0.70. \tag{9.A.1}$$

**Falsifier (S9 ablation)**: per backbone, fit ROC on 500 success + 500 `10`-class rollouts. **Statistic**: empirical AUROC $\hat{A}$. **Pre-registered threshold**: reject Proposition A if $\hat{A} + 1.96 \cdot \mathrm{SE}(\hat{A}) < 0.70$ (one-sided 95% CI upper bound below target). Additionally compare $\hat{A}_{O_t}$ vs. $\hat{A}_{\hat{O}_{t+1}}$ to isolate the ==our extension== (§3.2) effect — if predicted-frame AUROC collapses while real-frame AUROC remains strong, the extension has failed and `logpZO(\hat{O}_{t+1})` does not discriminate imagination failure on that backbone.

### 9.2 Claim B — Cross-side non-leakage (weakest claim)

> **Proposition B.** For each cell $c$ of the 2×2, the imag signal responds to imag corruption but not to pure act corruption, and vice versa:
> $$\mathrm{Recall}_{10} \geq 0.60,\quad \mathrm{Recall}_{01} \geq 0.60,\quad \text{per cell}. \tag{9.B.1}$$

**Falsifier (§5.3.2 confusion matrix)**: per-cell 4×4 confusion matrix on $500 \times 4$ injected-failure rollouts. **Statistic**: per-cell recall on rows `10` and `01`. **Pre-registered threshold**: reject Proposition B for cell $c$ if either recall is below 0.60. H4 requires Proposition B to hold in **$\geq 3$ of 4 cells** (descriptive; see Prop. 11.2 for cluster-robust primary test).
...
$$\mathrm{Top\text{-}1} = \tfrac{1}{4}\bigl(\mathrm{Recall}_{00} + \mathrm{Recall}_{01} + \mathrm{Recall}_{10} + \mathrm{Recall}_{11}\bigr). \tag{9.B.2}$$
$\mathrm{Recall}_{00} \geq 1 - \alpha = 0.90$ follows from the joint FPR control of Prop. 7.1. The remaining three recalls are empirical — Claim B pre-registers the $\mathrm{Recall}_{10},\mathrm{Recall}_{01} \geq 0.60$ falsifier; $\mathrm{Recall}_{11}$ is reported alongside but not pre-registered (both axes fire by design under joint corruption, so the floor is expected to be high but is not a falsifier). The H1 target of 75% is a pre-registered aspiration given these inputs; the 70% floor is the kill-gate.

### 9.3 Claim C — Joint calibration under exchangeability
...
> $$\mathbb{P}\bigl(\ell(\omega_\text{test}) \neq 00 \;\big|\;\text{success}\bigr) \leq \alpha. \tag{9.C.1}$$

**Proof**: Proposition 7.1 above.
...
## 10. Injected-Failure Protocol — Output-Token Level

### 10.1 Why output-token injection (not weight-module)
...
**Important caveat**. Injecting $\delta_v$ on video-output tokens and $\delta_a$ on action-output tokens does **not mathematically guarantee** that $s_\text{imag}$ responds only to $\delta_v$ and $s_\text{act}$ responds only to $\delta_a$. Both modalities share upstream weights $\theta$ and shared visual tokenizers; perturbing output tokens does not perturb the shared upstream pathway. Cross-side separability (Claim B, §10.2) is therefore an **empirical property to be measured**, not a property that output-token injection enforces by construction. The 2×2 factorial is specifically designed to quantify this separability via per-cell confusion matrices.

### 10.2 Formal noise model per class
...
$$\mathcal{C}_c\bigl(z_\text{video}^{(K)}, z_\text{action}^{(K)}\bigr) \;=\; \bigl(z_\text{video}^{(K)} + \delta_v,\;\; z_\text{action}^{(K)} + \delta_a\bigr), \tag{10.1}$$
...
### 10.4 $\sigma_\mathrm{inj}$ calibration (pre-registered before S5)
...
$$\sigma_\phi^{X,m} \;=\; \sqrt{\tfrac{1}{|\mathcal{D}_\text{cal}| \cdot N_m \cdot d}\sum_{\omega \in \mathcal{D}_\text{cal}} \bigl\|z_m^{*}(\omega) - \bar{z}_m^{(K)}\bigr\|_F^2}, \tag{10.2}$$
...
## 11. 2×2 Factorial Composition

### 11.1 Cell structure

Each cell $c \in \{1, 2, 5, 6\}$ is defined by a pair $(X, S_\text{act})$ where $X \in \{A, B\}$ and $S_\text{act} \in \{\text{ACE}, \text{STAC}\}$; the imag signal is always `logpZO`. Every cell instantiates:
...
### 11.2 Per-cell FPR control (shared CP + Bonferroni)

**Proposition 11.1 (Per-cell FPR).** For each cell $c$, under exchangeability of its calibration set $\mathcal{D}_\text{cal}^{(c)}$ and test point:

$$\mathbb{P}\!\bigl(\ell_c(\omega_\text{test}) \neq 00 \;\big|\;\text{success}\bigr) \;\leq\; \alpha. \tag{11.1}$$

**Proof**. Apply Proposition 7.1 to each cell independently; the calibration set, signal implementations, and thresholds are cell-specific but the framework is identical. $\qed$

### 11.3 Dependence of cell-level inferences — honest framing

**Proposition 11.2 (Cross-cell dependence structure).** The per-cell FPR guarantees in (11.1) each hold **conditional on exchangeability within that cell's calibration set**. However, the 4 cells are **not statistically independent** in general:

- Cells 1–2 share backbone A (UWM); cells 5–6 share backbone B (Cosmos Policy). Within a backbone, calibration rollouts $\{\omega_i^{(A)}\}$ are **reused across the 2 cells** that use that backbone (the imag signal $s_\text{imag}^\text{logpZO}$ is computed on the *same* $\hat{O}_{t+1}$ frames and differs only in the act signal).

**Effective independent units**: not 4. Under strongest dependence (full agreement within backbone), n_eff ≈ 2 (one per backbone). Under H2 holding (signals decorrelated), n_eff is between 2 and 4.

**Corollary — H4 binomial test with cluster-robust framing**. The naive binomial test $\mathbb{P}(\mathrm{Bin}(4, \pi_\text{pass}) \geq 3)$ assumes 4 independent trials and is **anti-conservative** when cells within a backbone share data. Two pre-registered options:

1. **Conservative backbone-level test** (primary): $\mathbb{P}(\text{H4}_\text{backbone-level})$ = probability both backbones have $\geq 1$ of 2 cells passing. Under $\pi_\text{pass} = 0.80$ per cell and within-backbone cell outcomes treated as exchangeable (but not independent), a conservative bound via Markov inequality gives $\approx 0.78$.
...
$$\mathbb{P}\bigl(\text{H4 passes (backbone-level)}\bigr) \;=\; \mathbb{P}\bigl(\text{at least 1 of 2 cells pass per backbone, both backbones}\bigr). \tag{11.2}$$

The naive Bin(4, $\pi$)-based statistic is reported only as a descriptive upper bound on the effective pass rate, not as the test statistic for H4.

### 11.4 Winning-cell selection (S4)

> **S4 Gate (H2)**: Per committed cell, measure $\hat{\rho}_c = \rho_S(R_\text{imag}, R_\text{act})$ on 500 success rollouts.

$$c^\star \;=\; \arg\min_{c \in \mathcal{C}_\text{committed}} \hat{\rho}_c, \tag{11.3}$$
...
## 12. Composition Proof — End-to-End Well-Definedness

> **Theorem 12.1 (Whole-pipeline well-definedness and FPR control).** Under the exchangeability of success rollouts within each cell's calibration set, the 2×2 attribution gate defined by §2–§11 satisfies:
>
...
> 3. **Cross-cell inference (descriptive)**: H4 is evaluated under the cluster-robust framing of Prop. 11.2. The primary test is the backbone-level criterion (≥ 1 of 2 cells passing within *each* backbone); the naive Bin(4, $\pi_\text{pass}$) "≥ 3 of 4 cells" statistic is reported descriptively only, with the explicit caveat that within-backbone cells share calibration data so the effective number of independent trials is $n_\text{eff} \in [2, 4]$. At $n_\text{eff}=2$ backbones, H4 is a descriptive generality statement, not an inferential claim with nominal Type-I control.

**Proof sketch by component chain**:

| § | Component | Input | Output | Property invoked |
|---|---|---|---|---|
| 2 | DiT forward | $(o_{1:t}, l)$ | $(\hat{O}_{t+1}, \mathbf{a}_{t:t+H})$ | Deterministic mod. sampling noise; DDPM-VP well-defined (UWM), rectified-flow well-defined (Cosmos Policy) |
...
| 4 | ACE | $\{\mathbf{a}^{(i)}_{t+h}\}$ | $s^\text{ACE}(t)$ | Shannon entropy is well-defined on finite multinomials; our generalizations (4.3)–(4.4) are strict extensions for $d_a \neq 3$ |
| 5 | STAC | $\{\mathbf{a}^{(i)}_{t:t+H}\}$ | $s^\text{STAC}(t)$ | Biased MMD$^2$ V-statistic is well-defined for any $K_\text{samp} \geq 1$; RBF kernel is characteristic [[2410.04640\|Sentinel]] |
| 6 | Max-so-far | $s_X(1:T)$ | $R_X(\omega)$ | Prop. 6.1–6.2: measurability + exchangeability preserved |
| 7 | CP | $\{R_X^{(i)}\}$, $\alpha/2$ | $\tau_X(t)$ | Standard split-CP coverage under exchangeability with signed one-sided scores [[2510.09459\|FIPER]] Appx A |
| 8 | Gate | $(R_\text{imag}, R_\text{act}, \tau_\text{imag}, \tau_\text{act})$ | $\ell \in \{00,01,10,11\}$ | Deterministic cross-tab |
| 9 | Claims | Prop. A–C | Falsifiers | Hypothesis tests with pre-registered thresholds |
| 10 | Injection | $z_m^{*}$ | $z_m^{*} + \delta_m$ | Measurable translation; $\sigma_\mathrm{inj}$ pre-registered |
| 11 | Factorial | 4× pipelines | $\{\ell_c\}_{c \in \{1,2,5,6\}}$ | Prop. 11.1–11.2: per-cell Bonferroni + cross-cell binomial |

**Each row shows**: inputs are well-defined → the operation is measurable / differentiable / deterministic → outputs feed the next row. Exchangeability for CP is inherited from frozen backbones + i.i.d. success-rollout sampling. Bonferroni over-protects under any positive dependence between axes, so (7.6) is conservative and cannot be violated by empirical signal correlations.

**Per-cell FPR (part 2)** follows from Prop. 11.1, which applies Prop. 7.1 (proved via union bound on (7.5a)–(7.5b)) to each cell's own calibration set.

**Part (3) — cross-cell framing**. Cells 1–2 share UWM calibration data; cells 5–6 share Cosmos Policy calibration data. Therefore the 4 cell-level pass/fail outcomes are **not independent Bernoulli trials**, and Proposition 11.2's cluster-robust framing replaces the naive binomial. The **primary H4 test** is the backbone-level statistic (both backbones independently hit ≥ 1 of 2 cells at Top-1 ≥ 70%); the "≥ 3 of 4 cells" statistic is reported descriptively with a note that $n_\text{eff} \in [2, 4]$. At $n_\text{eff} = 2$ backbones the generality claim is **descriptive, not inferential**.

$\qed$

---

## 13. Summary Table — Contributions vs. Sourced Components
...
| 4.3–4.4 | ==ACE generalization== | — | ==Novel: per-dim marginal + PCA fallback for 7-DoF== |
| 5.1 | MMD-RBF kernel | [[2410.04640\|Sentinel]] `error_utils.py` | — |
| 5.2 | ==STAC-all default== | — | ==Pre-registered `mmd_rbf_all` on full 7-D chunk== |
| 5.3 | STAC-single | [[2506.09937\|SAFE]] §5 | — |
| 6 | Max-so-far aggregation | [[2510.09459\|FIPER]], [[2506.09937\|SAFE]] | Formal statement (Prop. 6.1–6.2) |
| 7.1–7.2 | Functional CP | [[2510.09459\|FIPER]] §3.4 | — |
| 7.3–7.4 | ==Bonferroni joint correction== | — | ==Novel: $\alpha/2$ per axis + union-bound proof (7.6)== |
| 8 | 2×2 cross-tab | — | ==Replaces FIPER's AND-gate with structured attribution== |
| 9 | Claims A–C | — | Falsifiers pre-registered |
| 10 | ==Output-token injection== | — | ==Novel: compromise protocol for shared-DiT backbones== |
| 11 | 2×2 factorial | — | ==Novel: first dual-backbone factorial evaluation of dual-signal CP attribution on diffusion-WAMs== |
| 12 | Composition proof | — | ==Full end-to-end proof== |
