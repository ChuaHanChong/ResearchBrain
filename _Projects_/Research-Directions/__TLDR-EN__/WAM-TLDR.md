...
> [!abstract] The bet
> Use a WAM head that forecasts a *future 6-DoF wrench* (force+torque) as a rolled-forward output the policy acts on inside imagination, with *no* force sensor at deploy. The surviving wedge is this *sensorless* future-wrench rollout: the real comparison is sensorless rollout vs a *sensored* future-wrench ceiling (a FAWAM-style head at ~85% that reads a measured F/T sensor), not FD-VLA alone. It still clears FD-VLA's present-time force-*token* baseline (61.1%) by ≥5 pp on contact-rich SR, and recovers a vision-only WAM's tactile drop (DexViTac 83.3%→43.3% pipetting) toward ~63%. The load-bearing novelty: *future* tense + *6-DoF wrench* + *acted-on inside the rollout*.
