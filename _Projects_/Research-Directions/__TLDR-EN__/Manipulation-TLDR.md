...
**Why**: Reactive feedback arrives only *after* contact, by then the misalignment already happened. Predict-then-act is co-discovered consensus (VT-WM, VTAM, the 2019 Deep Tactile MPC root), and a fresh wave of four groups (ViTacWorld, FeelWorld, the NeoteAI/Fudan-TEAI team behind both N0-TWAM and N0-VTLA, and TacWAM) landed five papers on tactile-future prediction within one short window (N0-TWAM and N0-VTLA are a paired release from one lab, not two independent arrivals), with N0-VTLA directly measuring the principle: predicting future contact hits 92.3% accuracy vs only 57% for reading the current tactile frame. N0-TWAM's own no-foresight ablation (84.5%→71.8% UniVTAC) is a matched-capacity delta at realistic forecast quality, but it's a WM-vs-WM ablation with no contact-onset stratification. Still, no paper isolates the gain against a genuinely reactive baseline, or whether imagined tactile replaces the sensor.
...
**Why**: Single-weight policies already transfer across morphologies (GET zero-shots to 10 hand configs, DexFormer controls 32 variants), DexFormer via implicit history-conditioning, GET via graph-joint-space. UniCross now supplies a second full-cycle existence proof, chaining grasp+relocate+rotate+translate to 87.4%/96.3% SR across three hand morphologies, though as an explicit-intent-adjacent parameterization it does not itself settle which encoding wins. (D1: the cycle; A2: the grasp.)
