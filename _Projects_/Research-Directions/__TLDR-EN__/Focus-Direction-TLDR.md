...
> - Explicit ≈ implicit (no ~+3 pp, no gain concentrated on aggressive motions) → the contribution is void. But you learned it cheaply, and the clear ablation is publishable through VERIFY's metric.
...
> - The forecast may not survive without force sensors at deploy → the reframe-as-WAM build gates on sensor-free survival of the reaction forecast.
...
> [!warning] Risks
> - Without verify you cannot prove the bet or fill the benchmark gap, and FID-style metrics let imagination and action drift → ship the causal-consistency metric as the harness throughout.
