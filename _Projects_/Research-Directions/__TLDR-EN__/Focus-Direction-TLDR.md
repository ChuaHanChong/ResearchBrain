...
> [!abstract] The bet
> Make the coupling an *explicit predicted term*. [[2604.07993|HEX]] reaches **79.8 ID / 61.8 OOD** vs part-wise **70.2 / 41.0**. Add a predicted base-reaction head $\hat\delta_{\text{base}} = \hat M_{\text{base,arm}}\,\ddot q_{\text{arm}}$ — it should *widen* the 41.0→61.8 OOD margin over HEX's implicit MoE on the same data, backbone, tasks, in sim, zero new data.
...
> - Explicit ≈ implicit (no margin widening, no gain on aggressive motions) → the contribution is void. But you learned it cheaply in about 6 months, and the clear ablation is publishable through VERIFY's metric.
...
The cheapest falsification is milestone 1, run first: ablate the explicit base-reaction head against HEX's implicit MoE — same data, backbone, tasks, in sim, zero new data. If explicit ≈ implicit (the 41.0→61.8 OOD margin does not widen, no gain on aggressive arm motions), the contribution is void and you've learned it in about 6 months. That it can prove *itself* wrong, cheaply and up front, makes it a good problem rather than just an attractive one.
