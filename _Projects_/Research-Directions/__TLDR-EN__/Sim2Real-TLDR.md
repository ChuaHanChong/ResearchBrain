...
**First-principles**: *Principle:* the controller is part of the plant; gains set closed-loop dynamics, so they belong in the transfer distribution. *Challenged:* PACE (Bjelonic/Tischhauser/Hutter, ETH RSL) argues the opposite with real hardware evidence — identify the dynamics precisely at known, fixed gains and you need no dynamics randomization at all, and the identified sim stayed faithful even under PD gains other than the ones used to identify it. That collides with Tune-to-Learn's own finding in the same table: gain choice interacts with the DR-trained distribution strongly enough that the stiffest, lowest-sysID-error gains transfer *worst*. DiffCoTune sits between the two, co-optimizing gains and dynamics jointly but only per-system against a *fixed* deployment target, never a *randomized* distribution. *Wager:* neither PACE's precision-replaces-randomization route nor DiffCoTune's fixed-target route captures the joint term only randomized co-optimization can.
