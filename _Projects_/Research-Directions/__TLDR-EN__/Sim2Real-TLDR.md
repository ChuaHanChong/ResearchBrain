...
| B: Real-to-Sim-to-Real: Grounding the Simulator | B1, B2, B3 | Recovering appearance + dynamics $(\phi^\star, \psi^\star)$ from real data is the new bottleneck, and a *chosen* law caps it |
| C: Reality-Gap Measurement as Inference | C1–C3 | High in-distribution r, untested under deliberate shift, no provable bound |
...
**First-principles**: *Principle:* affordances, mass, and material cause outcomes; pixels are downstream. *Challenged:* not a belief anyone argues for, no cited work defends fixed asset-generation-time timing over per-episode resampling; it's an inherited default. The one piece of timing evidence (Grounding-Sim-to-Real: frame-wise beats episode-wise) points the same way, but only on appearance. *Wager:* a first test of whether per-episode semantic resampling helps at all, not an overturned belief.

**Sharpest questions**: 1) Does per-episode in-loop resampling beat every fixed-timing baseline (appearance-only DR, fixed-cousin, no-photoreal diversity) at matched budget? 2) Does affordance/material randomization add real-SR on top of fidelity (semantic main-effect survives at high PBR)? 3) Is the gain carried by material-parameter resampling or affordance-label resampling, and does it concentrate on geometrically-ambiguous tasks?
...
**First-principles**: *Principle:* the controller is part of the plant; gains set closed-loop dynamics, so they belong in the transfer distribution. *Challenged:* PACE (Bjelonic/Tischhauser/Hutter, ETH RSL) argues the opposite with real hardware evidence — identify the dynamics precisely at known, fixed gains and you need no dynamics randomization at all, and the identified sim stayed faithful even under PD gains other than the ones used to identify it. That collides with Tune-to-Learn's own finding in the same table: gain choice interacts with the DR-trained distribution strongly enough that the stiffest, lowest-sysID-error gains transfer *worst*. DiffCoTune sits between the two, co-optimizing gains and dynamics jointly but only per-system against a *fixed* deployment target, never a *randomized* distribution. A new controlled sim-to-sim pendulum study (Sim2Real-Budget-Allocation) now hands PACE's side a clean number: measuring the true parameters beats randomizing around any band, even a wide one that merely brackets the truth — but the study is single-DOF and contact-free, exactly the regime A3's own boundary hypothesis already expects a null in, so it sharpens rather than settles the disagreement. *Wager:* neither PACE's precision-replaces-randomization route, DiffCoTune's fixed-target route, nor the pendulum study's free-space identification route captures the joint term only randomized co-optimization can.
...
*Invert reality into the simulator first. Recover appearance and dynamics from real data, and forward transfer follows, but not for free: a simulator predicts reality forward no better than it captured it backward, plus whatever floor its own engine adds even at the true parameters (GAUGE: order-of-magnitude errors on contact and deformation given ground-truth-calibrated physics) — and that floor is orthogonal to how well the *inversion loop* runs, but not necessarily to how the *ground-truth reference itself* was measured (see B1's H6).*

### B1: Closing the Real-to-Sim Gap: Reconstruction Fidelity as the New Bottleneck
> [!abstract] The bet
> The headline is a *within-object causal* test, not a cross-object correlation: degrading *the same object's* reconstruction in controlled steps **monotonically lowers that object's forward r** (a cross-object regression conflates fidelity with per-object difficulty). The commit gate is **H5**: even at r>0.9 from joint appearance+physics inversion, REALM's control-dynamics misalignment caps the rest, and only co-optimizing control alignment (links A3) recovers that residual, if the within-object slope holds *and* the control term closes a measurable residual, the "reconstruction fidelity is the new bottleneck" thesis commits; if either fails, the direction de-risks to the observational law. The joint-inversion advantage is **widest on deformables, floor-adjusted** (Real-to-Sim GS 0.901 vs Isaac Lab 0.237 on rope, 0.915 vs 0.649 push-T, net of GAUGE's regime-specific engine-floor error — though that floor number may itself be inflated by GAUGE's quasi-static reference for a dynamic task, see H6), the regime the navigation counter-example skips.

**Why**: Name a real-to-sim gap separate from the forward one (REALM) and the arrow flips: forward correlation is capped by inversion fidelity *and* by the simulator's own engine floor. The joint-inversion *engine* is consensus (Splatting Physical Scenes, One-Shot Real-to-Sim, TwinAligner, D-REX); the bottleneck is the *law* mapping fidelity to r, and whether that law is a within-object *cause* or a confounded cross-object correlation.

**First-principles**: *Principle:* a simulator predicts reality forward no better than it captured it backward, and no better than its own solver reproduces reality at the true parameters: $|\text{Gap}_{\text{S2R}}| \geq \text{Gap}_{\text{R2S}} + \text{Gap}_{\text{floor}}$ qualitatively (GAUGE: even ground-truth-calibrated Isaac Sim/Genesis/Newton show order-of-magnitude errors on contact and deformation). *Challenged:* not "invert jointly" (consensus) but that fidelity *monotonically* and *causally* predicts forward transfer; Lower-Fidelity Sim2Real broke it (lower fidelity gave *higher* transfer in navigation). *Wager:* the bound is joint-reconstruction-bound (Real-to-Sim GS), so the within-object fidelity→r slope must be *measured*, not inferred from a cross-object fit, and read against the engine's own regime-specific floor.

**Sharpest questions**: 1) (Headline) Does degrading *one object's* reconstruction in controlled steps monotonically lower *that object's* forward r, the within-object causal slope a cross-object regression cannot give? 2) (Commit gate) Does control-dynamics misalignment (REALM) cap correlation even at r>0.9, and does co-optimizing control alignment (A3) close a measurable residual, the go/no-go for the causal-ceiling thesis? 3) Is the inversion advantage widest on deformables (the r>0.9 bet per class), and can the law rank twins offline before any rollout? 4) (H6) Does re-measuring GAUGE's foam/rubber/textile assets at dynamic strain rates shrink its reported engine floor, confirming a rate-regime confound in the floor-subtraction recipe, or does the floor hold, confirming it's solver-only?
...
> - GAUGE's engine-floor number may itself be inflated → its foam/rubber calibration is quasi-static (compression tests + DIC) but scored against high-acceleration tasks (twisting, flinging), a rate-regime mismatch (dynamic modulus > static modulus for viscoelastic materials) → don't subtract it as a clean solver-only correction; H6 tests a dynamically-recalibrated floor directly.
> - Amortizing per-object parameter recovery is engineering, not an open bet → UP-OSI (2017) already trains a feedforward net that predicts dynamics parameters in one forward pass and generalizes 100% outside its training range; Dynamics-as-Prompts, Latent-Intuitive-Physics, SLAT-Phys, Pixie, and the Distributional-Treatment Real2Sim2Real paper extend the same amortized-inference mechanism. Reuse one of these rather than re-deriving amortization; what remains is a robotics-specific supervision source + clutter robustness.
...
**Why**: Parameter-only sysID recovers the *parameters* of a law the engineer chose, capping generalization at the functional form. NCLaw embeds a *neural* constitutive law in a differentiable MPM; MASIV makes learn-the-form-from-video *consensus*. Two deltas stay unattacked: the learned-law-vs-parameter-fit advantage *on held-out geometry* (MASIV never isolates it), and the *closed robot loop* (neither supplies it).
...
> - Differentiable MPM is mature for soft bodies, not rich contact → scope the 1M-particle bet to elastoplastic/fluid materials. CLASH is an early answer on rigid contact specifically, but it sidesteps differentiability (distill the collision solver into a frozen surrogate, then gradient-identify only the contact parameters from ~10 real collisions) rather than solving it, so it doesn't settle the learned-law-vs-parameter-fit question there.
...
> [!abstract] The bet
> Decomposing outcome variance (sim-real r, collision rate, or SR) across **3 or more** physical units of the nominally same platform, three Franka Panda / DROID arms, say, will show a between-unit variance component with **ICC > 0.10**, not the ≈0 the field's pooled estimators assume. Duckietown's own measured inter-robot effect (MPD std **3.4 cm** vs **1.3 cm** same-robot, a **2.6×** SD inflation) is the only real-world number showing this kind of variance is not zero *somewhere*, on a different, low-cost platform. A conformal detector calibrated on unit A should also show degraded coverage on unit B with no other shift.

**Why**: Every gap-object this doc defines is a property of one, implicitly homogeneous real distribution. RoboArena pools 4,284 rollouts across 7 institutions' DROID arms into one ranking, its own Introduction names "manufacturing differences between said robots," then its model has zero terms for it. RoboChallenge runs a literal 10-machine fleet and catalogues tester bias and lighting drift but never inter-unit hardware variance. REALM and SIMPLER each fit sysID to one canonical unit per platform.

**First-principles**: *Principle:* this doc's own Gap objects are functionals of one real distribution; physically "the real robot" is N units, each with its own distribution, so every real distribution this doc's benchmarks estimate is an unindexed mixture over whichever units generated the trials, a fact about the data, not a modeling choice; the doc's own bound still holds per unit, it just needs an index it doesn't have. *Challenged:* not a decided sufficiency, a tooling artifact, one URDF per product line, one sysID fit per platform. RoboArena's authors see the obstacle in their own prose and never model it; even the doc's own most careful safety proofs (Robust-Conformal-CBF/CLF's Assumption 3, RAIL's URDF-geometry occupancy) only consider policy-induced shift, never unit-identity shift. *Wager:* the population structure is measurable and non-trivial, not merely theoretical.

**Sharpest questions**: 1) Does a fixed policy on ≥3 physical units show ICC > 0.10 between-unit variance? 2) Does a conformal detector calibrated on unit A show degraded coverage on unit B with no other shift? 3) Does B1's fidelity→r law change when re-fit per-unit vs pooled? 4) (Cheapest test) Does adding a unit term to RoboArena's ranking model change any pairwise ranking, on its existing data? 5) Is calibration protocol, not raw manufacturing cost, the moderator (ToddlerBot's engineered mitigation vs Duckietown's unmitigated variance)?

> [!warning] Risks
> - RoboArena may not have released unit-level metadata → treat the ranking-model re-analysis as a collaboration ask if institution-level granularity proves insufficient; run the hardware test in parallel.
> - A null result could be underpowered, not evidence of absence → pre-register a minimum-detectable-effect check for the ICC>0.10 threshold before testing.
> - If confirmed, the fix is a doc-wide retrofit no single direction can make → treat a positive result as an update to C2's and E3's own risk callouts and B1's scope, not a standalone footnote.
...
> [!warning] Risks
> - Differentiable dynamics may not exist for rich contact/friction transients → start in the aerial/analytical-core regime; expand via B3's differentiable-MPM cautiously.
...
**First-principles**: *Principle:* on hardware an action's safety cost is a hard constraint with no recovery, so adaptation must optimize reward *subject to* it. *Challenged:* not "unconstrained adaptation is unsafe" (measured), but SafeAdapt's own abstract claim that its guarantee holds for "arbitrary RL algorithms" — its method text scopes that to gradient updates projected onto the protected region, and its evidence is PPO alone on two gridworld tasks. Cluster-D's engines don't have that shape: RMA's update never touches the protected weights; AdaWorldPolicy's LoRA update may sit near-orthogonal to them. *Wager:* whether that "arbitrary" claim survives a real Cluster-D engine on real hardware, tested as one scale-sweep from a single wrapped engine to N successive domains.

**Sharpest questions**: 1) Does composition hold both wrapping a single Cluster-D engine and scaling to N successive domains (one sweep, not two separate claims)? 2) If composition fails, does it concentrate at the Fisher-vs-update-subspace interface, vacuous for RMA's latent-extrinsics update, mis-targeted for AdaWorldPolicy's LoRA update? 3) Do transfer-time (SPiDR) and adaptation-time (SCDA) safety *compose* to fewer violations than either alone?
...
> [!abstract] The bet
> (i, front line) Robust-Conformal-CBF/CLF's proof that a conformal FPR bound survives *through* a closed CBF/CLF feedback loop is proven under a Lipschitz-continuous-policy assumption. RAIL's discrete backup-planner switch violates that assumption by construction, so the bound is predicted to **measurably degrade** there, not merely be untested. The confirmatory half: the same theorem extends cleanly to SCDA's continuous episodic update, preserving FPR within **2 pp** of nominal while cutting user-facing failures by **at least 30%**. (ii) SAFE's internal-feature detector **still does not transfer across architectures**, even though TDQC's black-box Q-function/logit score already does, because the two live in structurally different representation spaces (shared output space vs private per-architecture latent basis); pairing SAFE with FAIL-Detect's policy-agnostic density score recovers the detection TDQC's transfer result doesn't predict.

**Why**: Detecting failures from successes alone, via a conformal threshold with an FPR guarantee, zero-shot to unseen tasks, is now consensus (FIPER does it near-verbatim, beating the FAIL-Detect lineage; Sentinel is the success-only root). What's left is *structural*, not just untested: whether a bound proven for smooth CBF/CLF controllers survives a discontinuous backup switch, and whether a detector built on one architecture's internal representations has any reason to transfer where a shared-output-space score does.

**First-principles**: *Principle:* a failure is an OOD event against the success manifold, spottable from successful runs alone; a conformal threshold caps the false-alarm rate with a guarantee holding even on a small calibration set. *Challenged:* not "detection needs failure-labeled data" (dropped by Sentinel/FIPER), and not a belief anyone holds about the two narrower claims either, Robust-Conformal-CBF/CLF and TDQC never claim their results generalize beyond what they tested. But two structural reasons make the broader case genuinely uncertain: the CBF/CLF bound's own proof assumes a Lipschitz-continuous policy, which RAIL's discrete switch violates by construction (SCDA's continuous update doesn't); and TDQC's transfer is carried by a shared-output-space score (action logits), which says nothing about whether SAFE's private-latent-basis features transfer the same way. *Wager:* the RAIL extension degrades where the SCDA extension holds, and SAFE's features don't transfer where TDQC's score does.

**Sharpest questions**: 1) Does bound-preservation degrade under RAIL's discrete backup switch (violating the theorem's Lipschitz assumption) while surviving SCDA's continuous episodic update (which the theorem's regime already covers)? 2) Does SAFE's internal-feature detector fail to transfer across architectures where TDQC's shared-output-space score already does, for a structural, not just untested, reason? 3) Do step-level localization (Hide-and-Seek) + root-cause (RAPT) enable a *targeted* response recovering more SR than detect-then-halt?
...
> - Both surviving sub-claims extend very recent (2026) theorems and could be preempted before this direction runs, the highest-preemption-risk direction in the doc → the mitigation is structural, not just speed: the RAIL-discrete-switch prediction is a *negative* result (the bound degrading), which is harder to preempt than a routine confirmatory extension.
