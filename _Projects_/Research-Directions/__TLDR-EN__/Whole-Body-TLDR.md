...
> [!abstract] The bet
> Predict the base reaction *explicitly*: $\hat\delta_{\text{base}}=\hat M_{\text{base,arm}}\ddot q_{\text{arm}}$, analytic term plus learned residual, fed as an auxiliary-loss target. Beats HEX's implicit MoE *on the same fixed dataset*, widening the 41.0→61.8 OOD margin by ≥ +3 pp into the mid-60s, most in the top arm-acceleration quartile where SEEC's "arms-are-light" assumption is wrong. Falsifiable: if explicit ≤ 61.8% at matched latency, leave the coupling to the MoE.
...
**Sharpest questions**: 1) Does an explicit base-reaction target on HEX's backbone + dataset lift OOD past 61.8% at matched params? 2) Does the gain concentrate in the top arm-acceleration quartile? 3) Does a shared *predictor* carry more coupling than FALCON's shared *observation* at equal capacity?
...
> [!abstract] The bet
> Run the *two-LAM-vs-one-LAM* ablation no one external has run. WholeBodyVLA's two visual-dynamics-split VQ-VAE latents beat a single shared latent; use MotionWAM's *unified* whole-body motion latent as the negative control at matched capacity. The separate-over-shared margin should scale with camera motion, largest on loco-heavy splits, near-zero on manip-heavy. Prediction: ≥78.0% unified-split vs ≤56.7% same-LMO decoupled on Bag/Box/Cart, the gain tracing to +38.7% action-free pretraining and +24.0% loco-RL, reaching LeVERB's 58.5%, 7.8× over naive hierarchical. Falsifiable: a single shared latent matches the split, or the margin is flat across camera-motion bins, then the split is not the lever.
...
**First-principles**: *Principle:* manipulation and locomotion have different visual dynamics, so one shared latent underfits one; yet the next action is a joint over both, and separate stacks re-introduce misalignment. *Challenged:* the single-shared-latent bet; WholeBodyVLA's separate VQ-VAE latents beat a shared model (+38.7% pretraining, +24.0% loco-RL; same-LMO trails 78.0%). *Wager:* the *split by visual statistics* earns the win.

**Sharpest questions**: 1) Do two visual-dynamics-split LAMs beat a single shared LAM, the gap scaling with camera motion? 2) Does a learned loco command interface beat a velocity controller most on precise locomotion subgoals (squat-to-place)? 3) Does action-free human-video pretraining replace an embodiment-gap-capped fraction of teleop data?
