...
> | **Ground — Sim2Real** | [[Sim2Real#B2 — Amortized Differentiable System-ID: Zero-Per-Object Gradient sysID in Clutter\|S2R · B2]] — recover $M_{\text{base,arm}}$ from real data by differentiable sysID | per-object gradient sysID beats DR on OOD mass ([[2603.01151\|D-REX]] 9–10/10 vs 4–9/10 below DR support; [[2510.11689\|Phys2Real]] 57% vs 23%) |
...
### [[Sim2Real#B2 — Amortized Differentiable System-ID: Zero-Per-Object Gradient sysID in Clutter|Sim2Real · B2]] — *ground* the coupling
The explicit term has **zero** advantage over an implicit MoE if the inertia model is wrong — a bad URDF poisons $\hat M_{\text{base,arm}}$ (this is A1's own named risk). B2 is the only direction in the corpus that recovers those exact constitutive parameters — mass distribution, inertia — by **differentiable system-ID**: gradient descent through a differentiable simulator that identifies an object's physics from interaction and beats domain randomization precisely on OOD *mass distribution* ([[2603.01151|D-REX]] **9–10/10 vs 4–9/10** below the DR support; [[2510.11689|Phys2Real]] **57% vs 23%** on the weight-top T-block) — which is exactly the parameter the coupling depends on. For one robot's coupling term, B2's per-object gradient recovery applies directly (the card's solved baseline); its headline bet pushes further to an *amortized* inference net that infers parameters for unseen objects at zero per-object demos. B2 makes the explicit term *true on the real robot*.
