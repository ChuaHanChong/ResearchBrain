...
|---|---|---|
| **Three falsifiers in parallel** | explicit-vs-implicit head per channel on a fixed whole-body dataset — **A** inertial reaction, **B** autoregressive-vs-flat, **C** anticipated-vs-stiff wrench (force channels A/C add a reactive-observer third arm); plot OOD success + balance recovery **vs** where each cross-term is largest | per-channel go / no-go (a channel can fail independently) |
| **Ground the terms** | differentiable system-ID of the base-arm inertia (A) and the contact model (C) from a few real demos; beat domain randomization on OOD mass/load; show the calibrated terms transfer sim→real | the explicit terms work on a real humanoid |
