...
| **Anchor papers** | [[2506.20134\|3D World Models Survey]] (survey), [[2510.16732\|World Models for Embodied AI Survey]] (survey), [[2604.22748\|Agentic World Modeling Survey]] (survey), [[2606.03943\|PointAction]] (method), [[2508.09071\|GeoVLA]] (method), [[2605.21414\|PointACT]] (method) |
...
- **Today's answers**: [[2606.03943|PointAction]] splits into an embodiment-agnostic video-to-point model + a small point-to-action decoder (43.0% zero-shot on an xArm7 it never trained on, 47.7% RoboCasa365 ID); [[2508.09071|GeoVLA]] runs a 3D point branch beside a frozen VLM (97.7% LIBERO) with height/scale/viewpoint robustness; [[2605.21414|PointACT]] couples points to actions at multiple scales (82.3% RLBench). All add geometry — none isolate *where* on the appearance-shift axis it pays off.
...
**Hypotheses & tests.** The FP bet — explicit point geometry beats RGB exactly where geometry holds and pixels move — decomposed into falsifiable sub-hypotheses, each landing on a Related-table row.
1. **H1 — Geometry-vs-RGB gap widens monotonically with appearance shift.**
   - *Prediction*: holding the backbone fixed and sweeping a controlled appearance shift (texture → lighting → camera pose), the point-head-minus-RGB SR margin grows monotonically with shift magnitude; at zero shift the gap is ≈0.
   - *Test*: re-run a [[2508.09071|GeoVLA]]-style point head vs an RGB head on a graded LIBERO-Plus-style shift sweep; plot margin vs shift.
   - *Row*: GeoVLA / SpatialVLA.
   - *Falsifier*: a flat or non-monotone margin → the advantage isn't the geometry-fixed-under-shift principle.
2. **H2 — The point-native advantage concentrates in contact sub-segments.**
   - *Prediction*: decomposing SR by task phase, the point-head margin is concentrated in contact-rich segments (where geometry sets the action) and near-zero in free-space transit.
   - *Test*: phase-label trajectories on [[2606.03943|PointAction]]'s RoboCasa365 tasks; report margin per phase.
   - *Row*: PointAction.
   - *Falsifier*: a uniform margin across phases → the point state helps globally, not via contact geometry.
...
4. **H4 — The point *representation*, not the factorized decoder, carries the zero-shot win.**
   ...
   - *Test*: 2×2 representation-vs-decoder swap on the embodiment-free pretraining.
   - *Row*: PointAction.
...
| **Key targets** | Stable rollout ≥3,000 frames / ≥4 km ([[2603.28887\|OccSim]]); +22.1% relative mIoU vs asset-based sim; 67% zero-shot; manipulation port: beat a pixel-world-model baseline on geometry-bound [[2406.02523\|RoboCasa]] tasks |
...
- **Today's answers**: [[2510.10125|CTRL-WORLD]] makes the pixel forward model controllable (38.7→83.4% on unseen objects); [[2506.23135|RoboScape]] augments it with temporal depth + keypoint dynamics (91% Robomimic on 200 synthetic trajectories). Both still imagine in or near pixels, so geometry is recovered, not planned-over.
...
- **Assumption being challenged**: that a manipulation forward model should predict pixels (the [[2510.10125|CTRL-WORLD]] / video-WM convention) because that's what video foundation models predict. [[2603.28887|OccSim]]'s 80× horizon gain shows the pixel substrate is itself the *source* of the drift, and rigid-body forecasters like [[2605.09196|RigidFormer]] (8× faster than FIGNet, stable to 217 objects) confirm explicit geometric state forecasts further than pixels.
- **The bet**: an occupancy forward model in the manipulation loop holds geometric stability an order of magnitude longer than a pixel-WM baseline on the same tasks (target: rollout-horizon-to-divergence ≥10× the pixel baseline at tabletop scale), and the downstream SR gain tracks [[2603.28887|OccSim]]'s +22.1% relative mIoU lift — the stability transfer is real, not driving-specific.
...
| [[2510.10125\|CTRL-WORLD]] | controllable pixel video | **38.7→83.4%** on unseen objects | pixel substrate re-parses geometry every step; drifts at long horizon |
...
**Hypotheses & tests.** The FP bet — explicit occupancy holds geometric stability an order of magnitude longer than pixels in the manipulation loop — decomposed.
1. **H1 — The occupancy inner loop beats the pixel inner loop on horizon-to-divergence at tabletop scale.**
   - *Prediction*: swapping a manipulation WM's pixel-prediction loop for an occupancy loop (backbone fixed) raises rollout-horizon-to-geometric-divergence ≥10×, and the 80× driving gain partially survives the scale change.
   - *Test*: matched-backbone occupancy-vs-pixel forward models on [[2406.02523|RoboCasa]]; measure frames-to-divergence + downstream SR.
   - *Row*: OccSim / CTRL-WORLD.
   - *Falsifier*: occupancy ≤ pixel horizon at tabletop → the driving gain is scale-specific.
...
4. **H4 — Predicted future-occupancy can condition A1's action head directly.**
   ...
   - *Test*: condition the A1 head on forecast vs present occupancy; report long-horizon SR.
...
| **Anchor papers** | [[2504.05786\|3D Spatial Reasoning in LLM Survey]] (survey), [[2506.20134\|3D World Models Survey]] (survey), [[2510.16732\|World Models for Embodied AI Survey]] (survey), [[2510.14836\|QDepth-VLA]] (method), [[2508.09071\|GeoVLA]] (method) |
...
- **Today's answers**: [[2510.14836|QDepth-VLA]] feeds depth as discrete VQ-VAE tokens to a depth-expert that reads the VLM's vision features without touching semantic alignment (+8.8% LIBERO-Spatial, +29.7% long-horizon over `open_pi_0`, +10–20% real, single-view); [[2605.14950|Evo-Depth]] injects implicit depth via FiLM (95.4% LIBERO, 0.9B params at 12.3 Hz); [[2606.03240|GeoAlign]] trains a depth encoder on RGB-D but drops it at execution (99.0% LIBERO, +13.8 pp geometry-critical real). All add geometry cheaply — none measure the recovery-fraction against a full-3D branch on a common backbone.
...
- **Assumption being challenged**: the [[2508.09071|GeoVLA]] / [[2606.03943|PointAction]] position that real 3D-awareness needs a full parallel 3D branch and joint re-training. [[2510.14836|QDepth-VLA]] shows a *frozen-backbone* depth-token task captures much of the gain, and [[2605.10485|VEGA]] / [[2510.12276|Spatial Forcing]] show train-time-only geometry distillation works at *zero* inference overhead — so the cheap/expensive split is false.
- **The bet**: a single-view depth-token bridge recovers ≥80% of a full-3D branch's spatial-task SR gain (same RGB-only baseline) at a fraction of the added parameters and zero re-training — and, because the backbone stays frozen, leaves the model's semantic alignment intact (the very thing full-branch fusion risks disturbing) while working from *single-view* input where full-3D methods need multi-view or sensed point clouds.
...
**Hypotheses & tests.** The FP bet — a cheap depth side-channel recovers most of a full-3D branch's gain without disturbing alignment — decomposed.
1. **H1 — The recovery-fraction curve has a knee well below full-branch cost.**
   - *Prediction*: sweeping depth-token capacity (codebook size, expert depth), the recovered fraction of [[2508.09071|GeoVLA]]'s full-3D-branch SR gain reaches ≥80% at a small fraction of the added parameters, with a clear knee.
   - *Test*: capacity sweep on a common backbone; plot recovered-fraction vs added params.
   - *Row*: QDepth-VLA / GeoVLA.
   - *Falsifier*: recovery scales linearly to full-branch cost → no cheap operating point exists.
2. **H2 — Quantization, not just the decoupled expert, drives the noise-robustness gain.**
   - *Prediction*: in a 2×2 (quantized vs continuous depth) × (decoupled vs fused expert) ablation, quantization contributes a separable robustness gain beyond the expert.
   - *Test*: the 2×2 on [[2510.14836|QDepth-VLA]]'s setup under depth-estimator noise.
   - *Row*: QDepth-VLA / Evo-Depth.
   - *Falsifier*: continuous-depth matches quantized at equal expert → quantization is incidental.
...
4. **H4 — The side-channel perturbs semantic alignment far less than full-branch fusion.**
   - *Prediction*: a held-out VQA probe shows the frozen-backbone depth expert perturbs semantic alignment measurably less than full-branch fusion that backprops into the backbone.
   - *Test*: VQA-probe drift, depth-expert vs full-branch fusion, same data.
   - *Row*: QDepth-VLA / GeoVLA.
...
5. **H5 — Train-time-only distillation matches the inference depth channel on geometry-bound tasks.**
   - *Prediction*: [[2510.12276|Spatial Forcing]]-style train-time alignment recovers a comparable fraction of the full-3D gain to QDepth-VLA's inference channel on geometry-critical tasks, at *zero* inference cost — so the cheapest deployable form needs no depth at test.
   - *Test*: train-time-distill vs inference-depth-token on geometry-critical real tasks at matched backbone.
   - *Row*: Spatial Forcing / QDepth-VLA.
...
- **Today's answers**: [[2507.13362|VLM Spatial Reasoning RL]] decomposes the scene into a relational graph before answering (Scene-Graph CoT, 77.69% CVBench, +19.5% Depth-OOD where SFT degrades); [[2501.10074|SpatialCoT]] pushes it into action via bidirectional coordinate alignment (82.57% manip, 61.83% nav). Both add structure — neither isolates whether the gain is the *metric* edges or merely topological ordering.
...
- **The bet**: a scene-graph CoT layer with RL grounding closes a measurable fraction of the [[2601.13304|CausalSpatial]] 54.17%→84.49% gap on the *causal* tasks where ungrounded language fails hardest, with the gain largest where the scene-graph is most metric and ≈0 on object-naming (where RGB already suffices); and RL grounding ([[2507.13362|VLM Spatial Reasoning RL]]'s +19.5% Depth-OOD) transfers the graph habit OOD where SFT overfits.
...
2. **H2 — RL grounding transfers the scene-graph habit OOD better than SFT.**
   - *Prediction*: GRPO-grounded scene-graph CoT generalizes to [[2601.13304|CausalSpatial]]'s causal tasks where SFT-trained CoT degrades, replicating the +19.5% Depth-OOD pattern on causal-OOD.
   - *Test*: RL vs SFT on the same scene-graph backbone, evaluated on held-out causal tasks.
   - *Row*: VLM Spatial Reasoning RL / CausalSpatial.
   - *Falsifier*: SFT matches RL OOD → grounding is prompt-tuning, not a transferable habit.
3. **H3 — The residual human gap is reasoning-bound on trajectory, perception-bound on occlusion.**
   ...
   - *Test*: per-task (collision/occlusion/trajectory) gain measurement on CausalSpatial.
   - *Row*: CausalSpatial / Embodied3DBench.
...
| **Anchor papers** | [[2506.20134\|3D World Models Survey]] (survey), [[2504.05786\|3D Spatial Reasoning in LLM Survey]] (survey), [[2510.16732\|World Models for Embodied AI Survey]] (survey), [[2605.05126\|ConsisVLA-4D]] (method), [[2508.07917\|MolmoAct]] (method) |
...
- **Today's answers**: [[2605.05126|ConsisVLA-4D]] enforces 4D coherence with *implicit* spatio-temporal consistency attention (no frame generation at inference): 98.1% LIBERO, 70.0% real long-horizon bimanual vs OpenVLA 28.5%, at 2.31× faster inference / 1.36× cheaper training; [[2508.07917|MolmoAct]] makes the temporal-geometry reasoning *explicit and steerable* (depth-aware tokens + visual traces) for 72.1% OOD SimplerEnv, 86.6% LIBERO. Both add consistency — neither isolates whether *consistency* or *raw 3D* drives the OOD gain.
...
1. **H1 — Ablating consistency collapses OOD more than ablating perception.**
   - *Prediction*: removing [[2605.05126|ConsisVLA-4D]]'s consistency attention (perception held fixed) collapses OOD SR more than removing any single perceptual feature, and more than it dents ID SR.
   - *Test*: consistency-ablation vs perceptual-feature-ablation; report ID-vs-OOD delta for each.
   - *Row*: ConsisVLA-4D.
   - *Falsifier*: a perceptual-feature ablation hurts OOD as much → raw 3D, not consistency, drives robustness.
2. **H2 — Implicit 4D is Pareto-dominant on the SR-vs-latency plane, except on a task sub-class.**
   ...
   - *Test*: build the frontier across implicit and explicit methods on matched tasks.
...
5. **H5 — A scene-flow prior recovers most of the consistency gain at lower cost.**
   - *Prediction*: [[2603.25399|LaMP]]'s dense 3D-scene-flow prior recovers a large fraction of ConsisVLA-4D's OOD gain (toward 79.3% LIBERO-Plus) at lower architectural cost than full consistency attention — flow is the cheap form of the constraint.
   - *Test*: scene-flow prior vs full consistency attention on LIBERO-Plus at matched backbone.
...
- **Today's answers**: sibling **C3** ([[2604.26694|X-WAM]]) pushes the *latent-4D* corner — geometry injected into a DiT, decoded internally, 15 Hz, Chamfer 0.0049 — and is excellent on per-frame fidelity; [[2510.10125|CTRL-WORLD]] makes the pixel substrate controllable. Neither gives a *planner-readable* state, and both fail on *horizon length* where latent error compounds.
...
- **Assumption being challenged**: that the rollout substrate should be the same RGB-D latent the policy sees (the [[2510.10125|CTRL-WORLD]] / C3-latent [[2604.26694|X-WAM]] convention). [[2603.28887|OccSim]]'s 80× horizon gain shows a purpose-built occupancy substrate wins on the long-horizon stability that matters for planning, and [[2504.20995|TesserAct]]'s depth-augmented WM marks the midpoint between the two extremes.
- **The bet**: a voxel-semantic occupancy WM holds geometric coherence over minute-scale manipulation horizons where a latent-4D baseline (sibling C3, [[2604.26694|X-WAM]]) drifts, and the gap is in *horizon-to-divergence* (not per-frame Chamfer, where latent-4D is strong, 0.0049) — making the two complementary, not competitive.
...
1. **H1 — Occupancy beats latent on horizon-to-divergence at tabletop scale.**
   - *Prediction*: on matched manipulation tasks, frames-to-geometric-divergence for an occupancy WM exceeds sibling C3's [[2604.26694|X-WAM]]-class latent by ≥one order of magnitude, partially surviving the driving→tabletop scale change.
   - *Test*: matched-task occupancy vs X-WAM latent; measure frames-to-divergence.
   - *Row*: OccSim / X-WAM.
...
2. **H2 — Externally-readable occupancy beats a decoded latent for collision-checking.**
   ...
   - *Test*: planner-on-occupancy vs planner-on-decoded-latent on collision-check tasks.
   - *Row*: OccSim / X-WAM.
...
5. **H5 — Occupancy-grid drift, not Chamfer, predicts downstream planning failure.**
   ...
   - *Test*: regress planning SR on both metrics across rollouts.
   - *Row*: OccSim / X-WAM.
...
| **Anchor papers** | [[2506.20134\|3D World Models Survey]] (survey), [[2510.16732\|World Models for Embodied AI Survey]] (survey), [[2604.22748\|Agentic World Modeling Survey]] (survey), [[2507.01099\|Geometry-aware 4D Robot Video]] (method), [[2507.13347\|Pi3]] (method) |
...
- **Today's answers**: [[2507.01099|Geometry-aware 4D Robot Video]] makes geometry a *predicted output* — Stable Video Diffusion predicting future multi-view RGB *and* aligned 3D pointmaps (pointmap VAE + cross-view pointmap diffusion loss + multi-view cross-attention), no explicit camera poses at inference — so off-the-shelf trackers read 6-DoF straight off, for 0.64 avg task SR (≈5× the 0.12 baselines) and novel-viewpoint generalization. [[2602.09878|MVISTA-4D]] reaches the same cross-view-consistent geometry but routes it to an end-to-end head, not a tracker.
...
- **The bet**: jointly predicting RGB + cross-view-consistent pointmaps yields directly-readable 6-DoF trajectories at ≥0.64 avg task SR vs ~0.12 for RGB-plus-estimator baselines (≈5×), and the gain tracks *cross-view consistency* (mIoU) — ablating the cross-view pointmap loss collapses trajectory accuracy more than degrading RGB quality (FVD) does.
...
| [[2604.27106\|RecGen]] | reconstruction-by-generation → shape + 6-DoF | **92.7%** ADD-SB@0.1, **+38.2 pp** under occlusion vs SAM3D | per-object reconstruction, not a full multi-view rollout |
...
1. **H1 — Cross-view consistency, not RGB fidelity, drives the action gain.**
   - *Prediction*: removing the cross-view pointmap diffusion loss (RGB quality held fixed) collapses trajectory-extraction accuracy and downstream SR more than degrading RGB FVD does.
   - *Test*: ablate the cross-view loss vs degrade RGB; report SR delta for each.
   - *Row*: Geometry-aware 4D Robot Video.
   - *Falsifier*: degrading RGB hurts SR as much → appearance, not consistency, carries the gain.
...
| **Anchor papers** | [[2506.20134\|3D World Models Survey]] (survey), [[2510.16732\|World Models for Embodied AI Survey]] (survey), [[2604.26509\|3D Generation for Embodied AI Survey]] (survey), [[2604.26694\|X-WAM]] (method), [[2503.18945\|Aether]] (method), [[2506.01103\|DeepVerse]] (method) |
...
- **Today's answers**: the field's defense is that 4D is a luxury — high-fidelity video needs many denoising steps, actions need few, online 3D is assumed too slow. [[2603.17240|GigaWorld-Policy]] (9× speedup, no video at inference) and [[2606.03188|GeoSem-WAM]] (98.55% LIBERO, aux branches dropped at test) keep geometry only at *train time*; [[2602.10098|VLA-JEPA]] (97.2% LIBERO) keeps a pure latent with no geometric decoder. None keep explicit 4D *online*.
...
- **Assumption being challenged**: that a pixel substrate suffices once frames look right, and explicit 4D is too expensive to deploy, so geometry is recovered by a separate two-stage pipeline. [[2604.26694|X-WAM]] shows the two-stage path is both worse geometrically (Chamfer 0.0680 vs 0.0049) *and* slower than a unified 4D model with asynchronous denoising; [[2503.18945|Aether]] confirms a 4D-reconstruction objective lifts planning (zero-shot KITTI AbsRel 0.056).
- **The bet**: a 4D-native substrate beats latent/pixel baselines on geometry-bound manipulation — 79.2% over 24 [[2406.02523|RoboCasa]] tasks, +12.1 pp over [[2601.16163|Cosmos Policy]] — with higher-fidelity geometry (Chamfer 0.0049 vs 0.0680, +2.34 dB PSNR) at *no* deployment penalty: a 4.5× action-latency speedup (4665→1033 ms) at 5 steps, 15 Hz on a physical robot — and the same substrate transfers from a world-model consumer to a policy action head.
...
1. **H1 — Native 4D beats lift-after-pixel on geometry-bound tasks.**
   ...
   - *Test*: native-4D vs pixel+post-hoc-depth at matched backbone on RoboCasa.
...
2. **H2 — The async action schedule shrinks to 1–4 steps without degrading the read-out geometry.**
   ...
   - *Test*: sweep action denoising steps; report geometry fidelity (Chamfer) vs steps.
   - *Row*: X-WAM.
...
5. **H5 — The 4D substrate transfers from a WM consumer to a policy action head.**
   - *Prediction*: rolling the 4D substrate forward under candidate actions, planning in explicit geometry beats planning in latent on spatially-bound tasks (insertion, stacking, pouring), *and* the same substrate plugged into a policy action head matches its world-model-consumer gain.
   - *Test*: 4D-plan vs latent-plan on spatial tasks; then swap the substrate into an action head, compare to the WM-consumer result.
   - *Row*: X-WAM / Aether.
   - *Falsifier*: latent planning matches, or the policy-head transfer fails → the substrate isn't model-agnostic.
...
> - **Real-time 4D rests on one result** ([[2604.26694|X-WAM]]). → Treat H1's native-vs-recovered ablation as the go/no-go before claiming 4D belongs at deployment rather than as a train-time auxiliary.
...
|---|---|
| **Cluster** | C — Geometry-Native World Models & Memory |
| **Thesis** | Long-horizon coherence needs geometric object permanence — remembering where things are when they leave view and return — and that drifts away in attention-only models. The field assumes a long-enough context window or a Markovian latent suffices. The bet is in First-principles below. |
| **Anchor papers** | [[2604.22748\|Agentic World Modeling Survey]] (survey), [[2504.21853\|Interactive Generative Video Survey]] (survey), [[2602.04411\|Self-evolving Embodied AI]] (survey), [[2603.17117\|MosaicMem]] (method), [[2603.24576\|Chameleon (Episodic Memory)]] (method), [[2605.10921\|RoboMemArena]] (benchmark) |
...
**Why it matters.**
- **The gap**: [[2504.21853|Interactive Generative Video Survey]] names persistent memory and dynamics fidelity as the two open problems blocking explorable simulators, and [[2604.22748|Agentic World Modeling Survey]]'s L2 Simulator must "compose multi-step rollouts that respect domain laws" — which fails over long horizons when the model forgets where things were.
- **Today's answers**: the usual fix — longer context, or a Markovian latent — drifts. [[2603.17117|MosaicMem]] documents implicit attention "suffers from inaccurate egomotion (drift), redundancy," while static explicit-3D caches "struggle with dynamic scenes"; [[2604.16484|DexWorldModel]]'s O(1) TTT memory holds *efficiency* over 2,000 steps but not geometric *permanence*. None pin imagined geometry to a persistent frame.
- **The opening**: a hybrid exists — [[2603.17117|MosaicMem]] lifts 2D patches to 3D and conditions a DiT geometry-consistently (Warped RoPE / Warped Latent), reaching RotErr **0.51°** vs [[2503.14489|SEVA]]'s 1.42° / [[2506.03141|CaM]]'s 4.65° at 16 FPS — and [[2605.10921|RoboMemArena]] proves the need is real: **68.9%** of its subtasks genuinely require history, and reactive policies fail them.

**First-principles framing.**
- **First principle**: long-horizon coherence requires *object permanence* — remembering where things are when they leave view and return. This is a geometric memory problem, not a sequence-length one: an attention-only model with unbounded context still drifts because nothing pins imagined geometry to a persistent frame. [[2510.01183|EvoWorld]]'s explicit evolving 3D memory cuts drift (FVD 106.81 vs GenEx 199.76) precisely by pinning geometry.
...
|---|---|---|---|
| [[2603.17117\|MosaicMem]] | lifted-3D patches + Warped RoPE/Latent (hybrid) | RotErr **0.51°** vs 1.42°/4.65°, **16 FPS**, minute-level | camera-controlled video, not action-conditioned manipulation |
| [[2603.24576\|Chameleon (Episodic Memory)]] | disambiguated indexable episodic events + latent imagination | **100%** episodic-recall / **73.5%** spatial-tracking / **72.2%** sequential DSR | events, not persistent geometry — the complement to MosaicMem |
...
**Hypotheses & tests.** The FP bet — explicit geometric memory holds minute-scale coherence where Markovian/long-context substrates drift — decomposed.
1. **H1 — Geometric memory translates RotErr coherence to long-horizon SR for action-conditioned consumers.**
   - *Prediction*: porting [[2603.17117|MosaicMem]]'s lifted-3D-patch memory from camera-controlled video to *action*-conditioned manipulation translates its RotErr-style geometric coherence into higher long-horizon SR vs an attention-only policy.
   - *Test*: MosaicMem-memory policy vs attention-only on memory-required manipulation.
   - *Row*: MosaicMem / 3D-ALP.
   - *Falsifier*: SR doesn't improve → camera-frame coherence doesn't transfer to action-conditioned tasks.
2. **H2 — Episodic events + geometric patches compound.**
   - *Prediction*: combining [[2603.24576|Chameleon (Episodic Memory)]]'s indexable events with [[2603.17117|MosaicMem]]'s geometry-consistent patches beats either alone on [[2605.10921|RoboMemArena]] — disambiguated *events* + persistent *geometry* address different failure modes.
   - *Test*: events-only vs geometry-only vs both on RoboMemArena.
   - *Row*: Chameleon / MosaicMem / Remember to be Curious.
   - *Falsifier*: both ≈ max(either) → the two memory types are redundant.
3. **H3 — Predictive-coding memory doubles as a calibration signal.**
   - *Prediction*: [[2605.10921|RoboMemArena]]'s predictive-coding head, made sensitive to state transitions, doubles as a forward-inverse calibration target that improves trust in the memory's geometry.
   - *Test*: use the predictive-coding head as a calibration loss; measure memory-geometry error vs uncalibrated.
   - *Row*: RoboMemArena.
   - *Falsifier*: calibration doesn't reduce geometry error → predictive coding isn't a usable trust signal.
4. **H4 — World-frame memory holds coherence better than robot-frame memory.**
   - *Prediction*: a geometric memory pinned to a persistent *world*-frame ([[2603.17117|MosaicMem]]'s lifted-3D patches) holds long-horizon coherence better than a robot-frame memory — decoupling memory from the body's pose reduces drift.
...
5. **H5 — A world-frame memory is the persistence layer for C1/C2/C3.**
   - *Prediction*: pinning C1's occupancy, C2's pointmaps, or C3's latent-4D to C4's world-frame memory keeps each substrate's geometry coherent over minute-scale horizons that the substrate alone cannot hold.
   - *Test*: each substrate with vs without the C4 memory layer on minute-scale tasks.
   - *Row*: MosaicMem / 3D-ALP.
   - *Falsifier*: the substrates hold coherence alone → C4 adds nothing the substrate doesn't already do.
...
| **Anchor papers** | [[2604.26509\|3D Generation for Embodied AI Survey]] (survey), [[2506.20134\|3D World Models Survey]] (survey), [[2510.16732\|World Models for Embodied AI Survey]] (survey), [[2404.09833\|Video2Game]] (method), [[2605.26115\|TriSplat]] (method), [[2506.06440\|Vid2Sim]] (method) |
...
- **Today's answers**: [[2404.09833|Video2Game]] builds for interaction from a *single video* — a real-time browser-compatible environment fusing a NeRF + baked game-engine mesh + physics module, 100+ FPS; [[2605.26115|TriSplat]] outputs Isaac-Sim/Unity-loadable meshes (33–249× faster, no post-hoc meshing); [[2506.06440|Vid2Sim]] reconstructs appearance + geometry + physics for the simulator. All build readiness — none score the readiness-vs-fidelity orthogonality the principle claims.
...
1. **H1 — Fidelity-optimal scenes fail readiness criteria measurably more often.**
   - *Prediction*: building the same scene optimizing (a) PSNR vs (b) [[2604.26509|3D Generation for Embodied AI Survey]]'s four readiness criteria, the fidelity-optimal asset fails ≥1 readiness criterion substantially more often — quantifying the orthogonality the first principle claims.
   - *Test*: dual-objective reconstruction; report per-criterion pass/fail rates.
   - *Row*: Video2Game / 3D Generation for Embodied AI Survey.
   - *Falsifier*: fidelity-optimal assets also pass readiness → fidelity and readiness aren't orthogonal.
2. **H2 — Single-video recovers rigid structure but not articulation.**
   - *Prediction*: [[2404.09833|Video2Game]]-style single-video reconstruction recovers rigid-entity decomposition reliably but fails to recover articulated structure (joints, DoF) for URDF/MJCF export without multi-view or interaction data.
   - *Test*: single-video vs multi-view kinematic recovery on articulated scenes; report URDF-exportability.
   - *Row*: Video2Game / SAPIEN.
   - *Falsifier*: single-video recovers articulation → the multi-view requirement is unnecessary.
...
| **Anchor papers** | [[2604.26509\|3D Generation for Embodied AI Survey]] (survey), [[2506.20134\|3D World Models Survey]] (survey), [[2605.21572\|PhysX-Omni]] (method), [[2605.30347\|NeuROK]] (method), [[2503.17973\|PhysTwin]] (method), [[2311.12198\|PhysGaussian]] (method) |
...
- **Today's answers**: [[2605.21572|PhysX-Omni]] inverts the order — a unified generator emitting rigid, deformable, *and* articulated objects with physics baked in (kinematic **0.9185** PhysXVerse, abs-scale error **2.79** vs >298, assets deployed in policy learning); [[2605.30347|NeuROK]] generates 4D neural object kinematics energy-conserving on *unseen* categories (Chamfer-L1 **0.028**, IoU **0.764**); [[2503.17973|PhysTwin]] / [[2605.09538|PhysHanDI]] reconstruct deformable twins from video/RGB-D. All recover physics-with-geometry — none score the rigid-vs-deformable-vs-articulated gap on one common readiness scorecard.
...
- **Assumption being challenged**: the geometry-first 3D-generation convention (NeRF/3DGS-then-annotate) that appearance can be generated independently and physics bolted on, and that deformable/articulated assets must be hand-authored. [[2605.21572|PhysX-Omni]] and [[2605.30347|NeuROK]] show a *unified* generator recovers material + kinematics in the same pass and generalizes to unseen categories — so the staged pipeline is a legacy of fidelity-only objectives.
- **The bet**: a physics-jointly-generated object hits per-object readiness (kinematic score ≥0.92, [[2605.21572|PhysX-Omni]]; Chamfer-L1 ≤0.028 with energy conservation, [[2605.30347|NeuROK]]) across rigid + deformable + articulated categories where a geometry-first generator + downstream physics-annotation fails at least one category — and the gap is *largest on deformable/articulated* objects, where the survey says no readiness solution exists, not on rigid props where a mesh nearly suffices.
...
|---|---|---|---|
| [[2605.21572\|PhysX-Omni]] | unified generator: rigid + deformable + articulated, physics baked | kinematic **0.9185** PhysXVerse, abs-scale **2.79** vs >298, deployed in policy learning | rigid/articulated-leaning benchmark — deformable readiness undertested |
...
1. **H1 — Joint generation passes the kinematic/material criterion the staged pipeline fails.**
   - *Prediction*: generating the same object (a) geometry-first then annotate vs (b) jointly via a [[2605.21572|PhysX-Omni]]-style unified head, the staged pipeline fails the kinematic/material readiness criterion substantially more often than the joint one.
   - *Test*: dual-pipeline generation; per-criterion readiness ([[2604.26509|3D Generation for Embodied AI Survey]]'s four) on a matched object set.
   - *Row*: PhysX-Omni / Physics3D.
   - *Falsifier*: staged annotation passes as often as joint → the consistency the joint pass enforces isn't load-bearing.
2. **H2 — The joint-generation advantage concentrates on deformable + articulated objects.**
   - *Prediction*: decomposing readiness by class, the joint-generation advantage is near-zero on rigid props (a mesh nearly suffices) and concentrated on deformable + articulated objects, as the first principle predicts.
   - *Test*: per-class (rigid/deformable/articulated) readiness comparison, joint vs staged.
   - *Row*: PhysX-Omni / PhysTwin / PhysHanDI.
...
| No benchmark splits the human-model spatial gap into *reasoning* vs *perception* failure, nor isolates *genuine metric 3D* from *shortcut* answering | B1 | [[2601.13304\|CausalSpatial]] (GPT-5 54.17% vs human 84.49%) measures the gap; [[2605.29074\|Embodied3DBench]], [[2605.30161\|Why Far Looks Up]] (36.9 pp), [[2605.30557\|SpatialUncertain]] diagnose the shortcut, but none isolate reasoning from depth-perception failure on one probe |
...
| No benchmark scores *native-4D-at-deployment vs lift-after-pixel* on geometry-bound tasks (Chamfer + SR + latency jointly), nor whether the latent-4D substrate transfers WM↔policy | C3 | [[2604.26694\|X-WAM]] (native 4D, single system, no native-vs-recovered ablation) + [[2605.20752\|GaussianDream]] (4D at train, dropped at deploy) |
| No benchmark scores *persistent geometric + episodic memory* on memory-dependent manipulation over minute-scale horizons, model-agnostic across substrates | C4 | [[2605.10921\|RoboMemArena]] (demand-side, reactive baselines) + [[2603.17117\|MosaicMem]] (geometric memory on camera-video, not action-conditioned) |
...
| No benchmark scores *per-object* physical-asset generation uniformly across *rigid + deformable + articulated* classes — readiness metrics are class-siloed | D2 | [[2605.21572\|PhysX-Omni]] PhysX-Bench (kinematic 0.92) is rigid/articulated-leaning; [[2503.17973\|PhysTwin]]/[[2605.09538\|PhysHanDI]] are deformable-specific; [[2605.30347\|NeuROK]] reports Chamfer/IoU — none score all three classes on one scorecard |
