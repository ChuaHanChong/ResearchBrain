...
1. **H1 — Gaze + generation beats either alone on sparse-foothold tasks at fixed compute.**
   ...
   - *Test*: four-arm comparison — blind / gaze-alone / generation-alone / gaze+generation — at matched compute on sparse stepping-stones; report SR and FLOPs.
...
| **Anchor papers** | [[2403.10506\|HumanoidBench]] (benchmark), [[2603.20147\|AGILE]] (benchmark), [[2506.12851\|KungfuBot]] (method), [[2605.06593\|ReActor]] (method), [[2605.10063\|EFGCL]] (method), [[2511.09484\|SPIDER]] (method), [[2510.14454\|Adaptive Motion Tracking]] (method) |
...
1. **H1 — Rejection + adaptive tolerance beats scaled projection-then-track on the high-rejection tail.**
   ...
   - *Test*: split a skill set by KungfuBot's rejection ratio; three-arm comparison — track-raw / scaled-projection-then-track / rejection+adaptive-tolerance — at fixed FLOPs; report error per rejection stratum.
   - *Row*: KungfuBot (filter + adaptive tolerance) vs SPIDER (scaled projection-then-track).
...
2. **H2 — An adaptive tracking factor learns skills a fixed tolerance cannot.**
   ...
   - *Row*: Adaptive Motion Tracking (adaptive phase/tracking adapters) / KungfuBot (adaptive tracking factor).
...
| **Anchor papers** | [[2107.04034\|RMA]] (method), [[2212.07740\|TERT]] (method), [[2301.10602\|DreamWaQ]] (method), [[2312.11460\|HIM]] (method), [[2507.07825\|LoadAdapt]] (method), [[2403.13358\|QUARD-Auto]] (method), [[2403.10506\|HumanoidBench]] (benchmark) |
...
2. **H2 — Backbone trades the payload axis against the discontinuous-terrain axis — no single one holds both.**
   ...
   - *Test*: hold supervision fixed; for each backbone sweep payload (0→80% body weight) × terrain (flat→stairs); report the joint SR surface and the payload-vs-terrain Pareto.
   - *Row*: HIM (contrastive-IMC) / TERT (Transformer) / RMA (TCN) / LocoFormer (Transformer-XL).
...
2. **H2 — Recurrent structure governs spatial retention over horizon.**
   ...
   - *Row*: SRU (recurrent memory) vs Blind Nav Agents (generic LSTM, emergent map to 1,000 steps).
