...
- **Filter (maximal, quality-gated)**: a direction is kept only if it passes all four gates — distinct sub-problem (not a re-slice of a sibling), KH-sourced measurable bet, non-consensus framing, ≥1 vault anchor. Cluster D was the drop-candidate and survived with three distinct directions: retargeting that must preserve *object + scene* contact (D1, unlike Locomotion's pure motion-imitation), teleop + robot-free demos (D2), and cross-embodiment transfer (D3). The miscitation trap [[2104.11181|H2O]] (interaction *recognition*, not retargeting) is excluded.
...
| **D — Whole-Body Teleoperation & Human-Motion Retargeting** | D1, D2, D3 | The data wall — demonstrations of *coupled* loco-manipulation are scarce, embodiment-mismatched, and per-platform | **D2 is the breadth substrate**: coupled demos buy generalization breadth ([[2602.10106\|EgoHumanoid]]'s +51 pp) that explicit coupling on fixed data cannot manufacture, but on a fixed budget the architecture is the lever — so D2 *complements* Cluster A, it doesn't stand upstream of it. D1's interaction-preserving retargeting is the feasible reference D2 and A1–A4 all consume; D3 amortizes whatever D1/D2 produce across bodies. [[2509.26633\|OmniRetarget]] and [[2605.23733\|Any2Any]]'s ~1%-compute transfer are the shared levers |
...
- **First principle**: $M(q)$ has non-zero off-diagonal blocks coupling arm, leg, and base — by the equations of motion, an arm acceleration is a base/leg reaction torque. The coupling holds regardless of training data, so a policy's value over whole-body actions is non-separable: $V(a_{\text{leg}}, a_{\text{arm}}) \neq V(a_{\text{leg}}) + V(a_{\text{arm}})$.
- **Assumption being challenged**: That a humanoid policy can split into an arm policy plus a leg/balance policy. Part-wise baselines ([[2604.07993|HEX]] beats GR00T N1.5, ACT, π0.5) bet on this; the 41.0%→61.8% OOD gap is the cost of ignoring the cross-body term, and more per-part data cannot recover it — the missing information is in the coupling, not the parts.
...
4. **Q4 — Blended skills as feasible RL references.** Use the blended-skill output as the downstream-RL reference (à la [[2604.00202|DreamControl-v2]]'s pre-retargeting) — does it raise the valid-trajectory rate toward 68%+ and RL toward 0.925?
...
|---|---|
| **Cluster** | A — Whole-Body Loco-Manipulation |
| **Thesis** | Build a single policy that emits *joint* locomotion + manipulation latent actions, not a manipulation policy stuck in a fixed in-place workspace plus a separate locomotion stack. The reason it must work: manipulation and locomotion have *different visual dynamics*, so they need distinct latent action models, yet must be predicted jointly to avoid decision-execution misalignment. The field assumes the policy is a manipulation model and locomotion is a downstream controller. The bet: a unified-latent policy beats the best decoupled stack head-to-head — a [[2504.16054\|π0.5]] / GR00T-class manipulation policy driving a separate velocity controller (the strongest modular baseline [[2512.11047\|WholeBodyVLA]]'s 78.0% already tops) — at WholeBodyVLA's 78.0% on Bag/Box/Cart, with the joint-latent gain traced to +38.7% from action-free human-video pretraining and +24.0% from a loco-oriented RL policy over a velocity controller, and reaches [[2506.13751\|LeVERB]]'s 58.5% whole-body semantic control (7.8× over a naive hierarchical policy's 7.5%). |
...
- **The bet**: A unified-latent policy beats the best decoupled stack head-to-head — a [[2504.16054|π0.5]] / GR00T-class manipulation policy + a separate velocity controller (the strongest modular baseline [[2512.11047|WholeBodyVLA]]'s 78.0% already tops) — at WholeBodyVLA's 78.0% on Bag/Box/Cart (joint-latent gain: +38.7% from action-free human-video pretraining, +24.0% from the loco-oriented RL policy) and reaches [[2506.13751|LeVERB]]'s 58.5% (7.8× over naive hierarchical). The head-to-head to run: unified-latent vs the decoupled stack on the *same* tasks; the decoupled-stack SR is the comparator, not asserted here.
...
**First-principles framing.**
- **First principle**: The base velocity is a manipulation degree of freedom — it changes the reachable workspace continuously during the task. So the optimal arm action is conditional on the chosen base action, $p(a_{\text{arm}} \mid a_{\text{base}}, a_{\text{torso}}, o)$; a pipeline that commits the base pose first cannot exploit in-task repositioning, and an independent joint factorization amplifies cross-subsystem drift.
...
*The whole-body data wall — demonstrations of coupled loco-manipulation are scarce, embodiment-mismatched, and per-platform.*
