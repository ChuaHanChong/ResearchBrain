...
- **DDR's CEM-MPC is zeroth-order by construction, even though the substrate underneath it is differentiable-capable - and the two literature objections to going gradient-based both have recent, concrete fixes, checked against the actual papers, not assumed solved.** CEM (Cross-Entropy Method) is sampling-based; Hydrax's JAX+MJX stack supports autodiff, but DDR doesn't use it. Two standing objections explain why a naive switch would fail, and both have named fixes: (1) *MJX's raw contact gradients are measurably wrong under stiff contact*, not just noisy - [[2506.14186|DiffMJX]] traces this to ODE time-discretization error and fixes it with adaptive-stepsize integration (Diffrax) plus Contacts-From-Distance (a straight-through estimator giving informative gradients even between non-touching bodies), reducing gradient/loss error by orders of magnitude at matched compute on a bounce-contact benchmark. (2) *Backprop through long contact-rich rollouts explodes* - [[2204.07137|SHAC]] (Short-Horizon Actor-Critic) fixes this by truncating the backward pass to a short window (h=16-32 steps) and bootstrapping the rest with a learned critic, cutting the gradient norm from >10⁶ (full-horizon BPTT on a 1000-step humanoid task) to stable, and training a 152-DOF muscle-actuated humanoid 17x faster than PPO. A third, independent line - [[2604.18161|DDCG]]'s IVW-H, a per-step inverse-variance mixture of 0th- and 1st-order gradient estimators - found empirically that on standard MuJoCo-style contact tasks, *variance* (not bias/discontinuity) is the dominant obstacle, and a simple per-step reweighting beats more complex discontinuity-detection schemes. None of these three papers touch DDR or terrain-mesh retargeting specifically - this is a transferability bet, not a reproduction (see Phase 2.6, Falsifiable bets).
...
## Phase 2.6 - gradient-coupled retargeting ablation (zeroth-order CEM vs differentiable optimization)

- Orthogonal to both prior ablations: on Gate A's extended rollout (terrain-mesh contact, steps 0-2 of the Proposed method), swap CEM-MPC's zeroth-order sampling for gradient-based trajectory optimization, using [[2506.14186|DiffMJX]]'s adaptive-integration + Contacts-From-Distance fix for correct contact gradients and [[2204.07137|SHAC]]'s short-horizon-window + critic-bootstrap for stable backprop through the multi-contact rollout. This is the concrete, buildable step toward coupling the retargeting stage's gradient with downstream signal, rather than the fully joint (WAM-style) coupling that isn't buildable yet - DDR already sits on the differentiable-capable stack (Hydrax/MJX), so this is a solver swap, not a new mechanism.
- **Precondition, not yet done:** neither DiffMJX nor SHAC has been applied to terrain-mesh multi-body contact (both validated on simpler primitives/self-contact) - Gate A-style convergence check first, same discipline as the rest of this project, before trusting the ablation's numbers.
- If gradient-based optimization converges to feasible terrain-contact trajectories faster (wall-clock) or to lower infeasibility than zeroth-order CEM, promote it as the Phase 3 fold-back solver - directly strengthens Gate B's break-even margin, since a cheaper per-round correction is the load-bearing premise Bet 4 depends on.
- If it doesn't converge, or converges no faster, keep CEM-MPC throughout and report the negative result - informative either way, since no prior work has tested differentiable trajectory optimization against DDR's own zeroth-order baseline on terrain contact.
...
7. **Gradient-coupled retargeting beats zeroth-order CEM (Phase 2.6, non-load-bearing, feeds Gate B).** DiffMJX+SHAC-based differentiable optimization converges to feasible terrain-contact trajectories at lower wall-clock, or to a lower infeasibility rate at matched wall-clock, than DDR's own zeroth-order CEM-MPC. Not load-bearing for the core thesis (Bet 3 stands or falls independent of this), but a win here directly strengthens Bet 4's cheap-fold-back precondition, and is this project's first concrete step toward gradient-coupling retargeting and policy training rather than running them as two disconnected stages.
...
> - **Gradient-coupling transfer risk (Phase 2.6, non-load-bearing):** DiffMJX and SHAC are both validated on simpler contact regimes (single-object bounce/manipulation primitives, self-contact locomotion) than DDR's terrain-mesh multi-body contact - neither paper touches retargeting or DDR's specific problem. Positive results elsewhere don't guarantee transfer; Gate-A-style convergence checking is required before trusting Phase 2.6's numbers, same discipline as everywhere else in this project.
...
- Whether a successful Phase 2.6 (gradient-coupled retargeting) should eventually replace the external fold-back loop's discrete rounds (Proposed method, steps 4-5) with a continuously-updated gradient signal between retargeting and policy training, rather than round-by-round re-solves - the deeper "shared gradient across the whole pipeline" question this project's scoping raised but doesn't attempt; out of scope here, a candidate for a future extension once Phase 2.6's transfer risk is resolved either way.
...
- [[2506.14186|DiffMJX]], [[2204.07137|SHAC]], [[2604.18161|DDCG]] - gradient-coupled retargeting candidates for Phase 2.6, fixing MJX contact gradients and long-horizon backprop instability respectively, neither yet tested on terrain-mesh contact
