...
2. [Survey landscape](#survey-landscape)
3. **[Direction 1 — Joint VLA/WAM Evaluation: Causal Consistency Between Imagination and Action](#direction-1--joint-vlawam-evaluation-causal-consistency-between-imagination-and-action)**
4. **[Direction 2 — Single-Loop Co-Evolving VLA + World Model in Latent Space](#direction-2--single-loop-co-evolving-vla--world-model-in-latent-space)**
5. **[Direction 3 — Tactile-Egocentric Pretraining: From Hand Video Alone to Force-Aware VLA](#direction-3--tactile-egocentric-pretraining-from-hand-video-alone-to-force-aware-vla)**
6. **[Direction 4 — Causally-Important Step Rewards for Latent VLA Reasoning](#direction-4--causally-important-step-rewards-for-latent-vla-reasoning)**
7. **[Direction 5 — Verifiable Physics-Consistent Training for Open-World VLA Generation](#direction-5--verifiable-physics-consistent-training-for-open-world-vla-generation)**
8. **[Direction 6 — Cross-Sensor Tactile Foundation Models for Plug-And-Play Force-Aware VLAs](#direction-6--cross-sensor-tactile-foundation-models-for-plug-and-play-force-aware-vlas)**
9. **[Direction 7 — Long-Horizon Memory + Failure Recovery Loops for Real-World Deployment](#direction-7--long-horizon-memory--failure-recovery-loops-for-real-world-deployment)**
10. **[Direction 8 — Real-Time-Deployable VLAs via Architectural-Algorithmic-Data Co-design](#direction-8--real-time-deployable-vlas-via-architectural-algorithmic-data-co-design)**
11. [Cross-cutting themes](#cross-cutting-themes)
12. [Benchmark gaps (consolidated)](#benchmark-gaps-consolidated)
...
## Direction 1 — Joint VLA/WAM Evaluation: Causal Consistency Between Imagination and Action
...
## Direction 2 — Single-Loop Co-Evolving VLA + World Model in Latent Space
...
## Direction 3 — Tactile-Egocentric Pretraining: From Hand Video Alone to Force-Aware VLA
...
## Direction 6 — Cross-Sensor Tactile Foundation Models for Plug-And-Play Force-Aware VLAs
...
4. **Real-time joint VLA+WM in latent space.** Direction 2's joint loop is currently bottlenecked by WM latency. A Mamba-based latent WM ([[2511.15605|SRPO]]-style V-JEPA-2 substrate) inside the joint loop should run at >30 Hz. Verify on a fixed simulation testbed.
...
3. **Egocentric pretraining as the dominant data substrate.** Directions 3, 7 (memory built from human video), 8 (data-side efficiency). The [[2602.16710|EgoScale]] log-linear scaling law is the first predictable axis for robot pretraining; [[2510.24795|Efficient VLA Survey]] confirms internet-scale human video as one of three dominant data-collection levers.
4. **Detection-diagnosis-recovery as a unified stack.** Direction 7 explicitly; Directions 2 (failure-finder), 4 (CoT faithfulness), 5 (physics violation detection) implicitly. The diagnostic stack is becoming as important as the policy. [[2602.04411|Self-evolving Embodied AI]]'s 5-module framework formalizes this.
5. **Force/tactile as a first-class modality with dedicated experts.** Directions 3, 6 explicitly. [[2505.22159|ForceVLA]] / [[2603.15169|ForceVLA2]] / [[2603.15257|HapticVLA]] established the architecture; data and cross-sensor transfer are the remaining bottlenecks.
6. **Efficiency as a prerequisite, not optimization.** Direction 8 explicitly; Directions 1, 2, 7 implicitly (joint loops, recovery loops, and memory stacks all need real-time budgets). [[2510.24795|Efficient VLA Survey]] and [[2603.28489|Video Gen as WM Survey]] both reframe efficiency from "nice to have" to "deployment-blocking". The [[2505.04769|VLA Survey]]'s 3–5 Hz ceiling diagnosis is the quantitative anchor.
