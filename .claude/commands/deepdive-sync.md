...
**Anti-pattern H (over-long bullet)** is a special case — repairing it is *content compression, not restructuring*, so it carries an extra discipline: **read the paper's `_KnowledgeHub_/{ID}.md` and verify every retained `**bold metric**` and `==method==` is accurate before trimming** (fix any misquote from KH — a shorter bullet that preserves a wrong number is worse than the long one). Then trim to the L3 length ceilings, keeping all metrics + the lead method highlight and cutting only connective prose. Locate bullets by arxiv ID (line numbers shift as you edit).

Repair operations preserve all existing content — they restructure (H: compress), never delete a metric or wikilink.
...
3. **L3 — Bullet-per-paper** (five rules):
...
3. **Edges.** Adjacent boxes connect with `───►` between borders, no gap. **Fanout:** `─┬──►`, then `├──►`, last `└──►`; two independent edges from one source each get their own branch, never chained through one another (the most common bug). **Cross-lane:** `──────►` + bare target name + `[destination, below|above]` — name the destination, never a bare `[below]`. For 2+ cross-lane targets, hang the branches from a `┬` in the source box's own bottom border with the fields in fixed columns; never use a prose caption like `(X also feeds)`, whose connector floats between boxes instead of under the source. Skip the pointer entirely, using a bare `│`/`▼`, when the target's box is the very next thing in reading order. Draw a cross-lane edge **only where the target's own L3 bullet names the source** — that is checkable, and it is the difference between a real edge and a plausible one.
4. **Deltas.** An arrow says *came after*; the delta says *what changed*. Every arrow carries one — cross-lane pointers inline after the `[…]` hint. Source it from the target's own L3 bullet; never invent one, and skip the label rather than write something vague. Never strip existing deltas during a formatting pass.
   - **Placement:** its own line directly above the box-top row, never inline in the arrow. Horizontal space is the scarce axis (rule 6); vertical is free.
   - **Alignment:** the left edge of the box the arrow points **into**. A delta states what the *successor* adds, so it belongs above the successor; source-alignment labels the wrong paper and leaves the newest box bare. The first box in a chain correctly has none.
   - **Too long for its box?** Widen the box to `max(len(label) + 4, len(delta))` — box width is free. Failing that, wrap to a second line at the same column; failing that, shift a colliding neighbour right. **Never abbreviate:** `+language, 100 tasks` must not become `+lang, 100 tasks`.
   - **Two grammars only:** `+X` — the successor *adds* X. `X → Y` — the axis *moves*, the successor replacing rather than accumulating (`few-shot → zero-shot`). Writing `+Y` for a move is wrong. A finding still states a move: `RL forgets less than SFT` becomes `SFT → on-policy RL`.
5. **Thread depth and shape.** Every thread must link **≥4 papers** — count papers in the thread, not the longest path, or branched threads get wrongly rejected. **Shape follows content**, with the reason recorded beside it: *fork* where one paper spawned genuinely parallel directions, *converge* where two lines merged into one paper, *straight* where each step supersedes the last (`Dreamer → V2 → V3 → 4`). A file whose threads all share one topology is a template, not an evolution. Two failures cost equally: forcing a fork onto a genuine ladder, and chaining papers that never descended from each other (`Physion++ → VideoPhy → PhysBench` invented a lineage among three independent benchmarks). A node qualifies if it is cited in the same `#### N.N` sub-section *and* supports the succession. A thread short of 4 must be extended from that sub-section's roster (thin threads are usually under-selection, not a real gap), merged into a sibling, or dropped to the body text — never padded with an invented edge. One exception: a lone box is allowed when its only edge is cross-lane, since it inputs to another lane rather than being a thread.
6. **Width.** Keep every line ≤~110 chars. When a deep vertical drop would blow the budget, hang the connector from the source box's own border (rule 3) rather than indenting to match a distant column — and never pack unconnected boxes onto one row to save space.
7. **Fidelity.** Each node appears as exactly one real box; each edge exactly once — none invented, none dropped, no false chaining. **Re-verify after every width or style edit** — that is where topology silently regresses: regenerating a row blanks its prefix and drops the parent's trailing `─┐`, orphaning every branch below, and an inserted delta line breaks any vertical channel crossing it. Confirm each branch column has an unbroken `│` from the parent's bottom border to its last `├`/`└`, and that channels keep their original column.
8. **Legend.** One line at the end, only if the file has ≥1 landmark node: `Legend: ╔═╗ double border = landmark/foundational paper.`
