...
Physical reasoning sits one level above physical generation: the model must *talk about* physics consistently, not just produce physics-compliant pixels. Three very different mechanisms land here — a trained foundation model that internalizes physics via SFT→RL, a training-free agent that reconstructs an explicit, executable physics simulation from the video itself, and a frozen VLM that discovers physics at test time through a verify-before-commit memory loop.
