...
```text
1. Implicit Physics   (physics baked into a 3D representation)
· physics in Gaussians
╔═════════════════════╗
║ PhysGaussian (2023) ║─┐
╚═════════════════════╝ │
                        │    +material params        +constitutive net
                        │    ┌──────────────────┐    ┌───────────────────┐
                        ├───►│ Physics3D (2024) │───►│ OmniPhysGS (2025) │
                        │    └──────────────────┘    └───────────────────┘
                        │    +annotated assets
                        │    ┌─────────────────┐
                        └───►│ PhysX-3D (2025) │
...
· differentiable twins
                         +deformable twin       +real2sim policy
╔═══════════════════╗    ╔═════════════════╗    ╔══════════════════╗
║ DiffTaichi (2019) ║───►║ PhysTwin (2025) ║───►║ PhysWorld (2025) ║
╚═══════════════════╝    ╚════════┬════════╝    ╚══════════════════╝
                                  │    +physics-aware WM
                                  │    ┌──────────────────┐
                                  └───►│ RoboScape (2025) │
...
4. Benchmarks   (does the model actually know physics?)
...
Four lanes, four different shapes. [[2311.12198|PhysGaussian]] split into a material-model ladder and a separate data effort ([[2507.12465|PhysX-3D]]). Physics-informed video is a single ladder of tightening constraints, with [[2510.13809|PhysMaster]] leaving it for an RL reward instead. The twin line runs straight from [[1910.00935|DiffTaichi]] to policy learning, with [[2506.23135|RoboScape]] branching into world-model territory. The benchmarks never chained at all: three groups independently asked whether video generators, VLMs, and likelihood probes know physics.

| Year | Paper | Track | Contribution |
|------|-------|-------|--------------|
| 2019 | [[1910.00935\|DiffTaichi]] | Real2Sim Twin | Foundational differentiable-physics-simulator language: two-scale automatic differentiation |
| 2023 | [[2306.15668\|Physion++]] | Benchmark | A benchmark testing online inference of latent mechanical properties (mass, friction, elasticity) from dynamic interactions |
| 2023 | [[2311.12198\|PhysGaussian]] | Implicit Physics | Implicit: each scene element carries physical attributes so a custom MPM integrates Gaussian particles directly |
| 2024 | [[2403.14404\|PIDM]] | Physics-Informed Video | Foundational physics-informed diffusion: treats the PDE residual R(x0) as a virtual observable maximized via a Mean/Sample |
| 2024 | [[2406.03520\|VideoPhy]] | Benchmark | The original predecessor to VideoPhy-2, a physical-commonsense benchmark of **688** human-verified captions covering |
| 2024 | [[2406.04338\|Physics3D]] | Implicit Physics | Generation: each denoising step is constrained by physics |
| 2025 | [[2501.16411\|PhysBench]] | Benchmark | An interleaved video-image-text benchmark of **10,002** human-annotated entries evaluating **75** VLMs |
| 2025 | [[2501.18982\|OmniPhysGS]] | Implicit Physics | Implicit: constitutive Gaussians (a learnable neural constitutive model per particle, chosen from a 12-combination ensemble) |
| 2025 | [[2503.09595\|PISA]] | Physics-Informed Video | Dynamics: Physics Supervised Fine-Tuning (PSFT) + Object Reward Optimization (ORO) over the PISA benchmark of real |
| 2025 | [[2503.17973\|PhysTwin]] | Real2Sim Twin | External-solver: multi-stage joint optimization of geometry + physical properties + appearance via spring-mass models |
| 2025 | [[2506.23135\|RoboScape]] | Real2Sim Twin | A physics-informed embodied WM: a Dual-branch Co-autoregressive Transformer jointly generating RGB + temporal depth |
| 2025 | [[2507.12465\|PhysX-3D]] | Implicit Physics | A physics-grounded 3D-asset generator pairing PhysXNet (26K objects with part-level scale/material/affordance/kinematics) |
| 2025 | [[2509.20358\|PhysCtrl]] | Physics-Informed Video | A two-stage method lifting image → 3D point cloud → a diffusion-based generative physics network predicting 3D point |
| 2025 | [[2509.21309\|NewtonGen]] | Physics-Informed Video | Explicit-loss: a Neural Newtonian Dynamics layer (physics-informed neural ODEs) injected into the T2V backbone predicts |
| 2025 | [[2510.11512\|LikePhys]] | Benchmark | A training-free likelihood-preference evaluator scoring video-diffusion physics understanding via denoising-loss ELBO |
| 2025 | [[2510.13809\|PhysMaster]] | Physics-Informed Video | Training: PhysEncoder refined via three-stage DPO over I2V diffusion |
| 2025 | [[2511.07416\|PhysWorld]] | Real2Sim Twin | External-solver: task-conditioned video generation + geometry-aligned 4D reconstruction + object-centric residual RL over |
...
- **[[2602.06618|PotentialNet]]** — Physics-in-architecture for electromagnetic actuation: MLPs preserve ==current-linearity== so inversion stays a ==closed-form Moore-Penrose pseudoinverse==, and predicting a ==scalar potential== whose negative gradient is the field makes it curl-free by construction; **zero** curl error at **~0.5 mT** RMSE, evaluated in **0.25–0.75 ms**.
...
- **[[2606.21753|RAF]]** — Breaks the monolithic-3DGS-solver silo: a ==representation abstraction layer== turns Gaussians (==opacity thresholding== + ==Poisson-disk ghost particles==), meshes, and fluids into one particle list, then a ==unified kernel== dispatches rigid / articulated / ==MPM== / ==SPH== / ==PBD== coupled by ==SDF impulse response==; **five** two-way scenarios under **10 min**.
...
Same function as §2.1 — simulate scene dynamics directly, producing a rollout rather than a tagged asset — but the particle representation isn't Gaussians and the solver isn't MPM. Mesh vertices, a learned kinematic coordinate frame, and convex-hull support functions stand in instead.
...
- **[[2602.18886|PhysConvex]]** — Swaps Gaussians for ==dynamic convex fields==: explicit hull vertices advect under Newtonian motion while ==half-space support functions== give differentiable occupancy, advected by ==neural skinning eigenmodes== in a ==reduced-order== solve; log(E) sysID error **0.37** vs Vid2Sim's **0.51**, **30.00** PSNR, trained in **6** min.
...
- **[[2608.08053|Structured Physical CoT]]** — Makes image-to-part reasoning an explicit supervised trajectory: a parseable ==Structured Physical Chain-of-Thought== (part count, boxes, relations, primitives) serves as SFT target, geometry condition, and ==CoT-aligned GRPO== reward at once, with ==position-factorized RLE== local geometry; CD **0.041** vs **0.095**, scale error **6.57%**.
- **[[2607.13586|UniPhysGen]]** — Joins articulation and material estimation in one pass: ==SAM==-lifted part priors + ==Apriori itemset mining== segment uncurated meshes, an ==LVLM== picks geometry-constrained axis-pivot candidates, and ==SO(3) augmentation== regresses properties; **89.96%** joint-type accuracy, **0.099** pivot error, material accuracy **77.92%** vs NeRF2Physics' **53.09%**.
...
- **[[2603.23386|SIMART]]** — A unified MLLM (Qwen3-VL-8B) generates sim-ready articulated assets in one pass via a ==Sparse 3D VQ-VAE== (zero-token voxel sparsity, ~70% token cut) jointly predicting part segmentation + URDF kinematics from a monolithic mesh; **0.928** joint-type accuracy / **0.080** axis error on SIMART-Bench (vs **0.822**/**0.208** baseline), assets run in Isaac Sim.
...
- **[[2606.29303|MF-MOD]]** — Reframes occlusion-induced multi-object entanglement as ==sparse-view reconstruction==: SAM2 segmentation-affinity splitting plus joint 2D+3D ==Score Distillation Sampling== and intra/inter-object similarity priors fill missing geometry before ==MPM== robot-collision simulation; **17.83** PSNR vs PhysGaussian's **13.86**, robust to segmentation-backbone choice.
- **[[2606.03994|SimuScene]]** — Puts a physics engine inside the reconstruction loop: ==sequential diagnostic simulation== releases one object under gravity to measure penetration/displacement, driving ==OBB-axis stretching== or ==DPO-fine-tuned amodal shape resampling==; strict ABO **0.507** vs SAM3D's **0.218**, penetration **16.2%→2.5%** on GenWild — physics as measurement, not validation.
- **[[2605.30338|REST3D]]** — VLM agents infer a gravity-aware support scene tree, canonicalize SAM3D meshes against it, then ==Cross-Entropy-Method== optimization scores 6-DoF pose candidates by ==Isaac Gym== rollouts (stability, velocity, penetration, layout energies); stability **8.3%→95.8%** on Replica, collision **66.7%→0.0%** — simulator inside the objective, not post-hoc.
- **[[2605.30239|SAM3D-Phys]]** — Training-free pipeline: PGSR reconstruction + LaMa inpainting yields a clean background, ==SAM3D== generative priors complete occluded multi-object geometry, physics-constrained render-and-compare pose fitting (ground-contact + non-penetration losses) restores placement before ==MPM== simulation; in-mask PSNR **23.92→30.04**.
...
- **[[2607.19804|V2F]]** — A physics-informed residual neural network augments the classical ==Hertz contact equation==, learning only the residual between idealized elastic-contact prediction and a date fruit's true force response from vision-only geometric features; **R²≈0.7** across unseen cultivars, grasp trials leave **<1mm** residual deformation and no observable bruising.
...
- **[[2608.07981|PhyS]]** — Three-stage streaming-WAM recipe: physics-aware ==LoRA== SFT of a Wan2.1-14B teacher on **PhyS-120K** (120K annotated real clips), ==DMD distillation== into a 1.3B causal student, then online RL with ==Temporal Credit Routing== routing overlapping-window V-JEPA2 + optical-flow rewards to denoising steps; PhysicsIQ **+31.4%** over causal baselines at unchanged 4-5 NFE.
...
- **[[2606.00499|OptiWorld]]** — Inference-time optimal control: a zero-shot world-state extractor plans a ==Riemannian-manifold shortest-path== trajectory (safety+goal+smoothness+efficiency as one metric), then conditions any frozen video generator; goal error **0.310** / safety-violation **0.260** (best of 4), motion energy **0.0126** vs **0.0860** — no retraining.
- **[[2605.18396|NEWTON]]** — A ==Planner-Executor-Verifier== agent steering a frozen generator through physics-aware tool calls (keyframe boundary conditioning, sandboxed trajectory/momentum computation, prompt refinement), with ==Flow-GRPO== training only the planner inside the live loop; VideoPhy-2 joint **21.36%→29.66%**, HARD subset **4.44%→12.22%**.
...
- **[[2503.08257|DexGrasp Anything]]** — Applies the same ==surface-pulling / external- and self-penetration== force gradients twice, in the denoising loss and again as ==physics-guided sampling==, over an ==LLM-enhanced object representation== and the 3.4M-grasp DGA dataset; **57.5%** Suc.6 on DexGraspNet vs UGG's **46.9%**, guided sampling **53.6%** vs training-only **46.8%**.
...
- **[[2412.00547|Motion Dreamer]]** — Decouples motion reasoning from RGB synthesis: sparse-to-dense ==instance flow== (per-object optical flow from user arrows) plus segmentation/depth condition a two-stage CogVideoX renderer via ==softmax-splatting==; Physion FVD **157.8** vs Mofa-Video (**170.1**), **72%** user preference.
...
- **[[2404.13532|SpringGrasp]]** — Replaces bilevel force-closure with a differentiable ==spring-damper== energy solved in closed form (==Wahba/Kabsch==), jointly optimizing pregrasp pose, contacts, and per-finger ==impedance gains== under a ==GPIS== shape-uncertainty term; **89%** / **84%** real Allegro success from two / one view vs **62–71%** for fixed-gain baselines.
...
- **[[2606.25212|RigPI]]** — VLM (Intern-S1/Gemini-3) reads RGB-D to seed an informed initialization and bounded search space for mass/COM/inertia/friction, then ==NVIDIA Newton== differentiable simulation refines parameters via pose loss + bound-penalty over measured wrenches; mass error **1.50-7.21%** vs **11-196%** least-squares, stable across unconstrained and multi-link rigid bodies.
- **[[2606.13842|Kernel-Adaptive Quadrotor Control]]** — A ==random-Fourier-feature== disturbance kernel with learnable coefficients and bandwidth trains offline via ==backprop through time== in a differentiable simulator (50s), then adapts both online via ==least-squares projected gradient descent== at 50Hz; simulation RMSE **4.11cm** vs **10.71-14.22cm** baselines, hardware gains up to **34%**.
...
- **[[2508.06181|HyperPM]]** — A ==causal network + B-spline== predictor encodes unmodeled dynamics as time-varying parameter trajectories over the MPC horizon (not constant), feeding **HyperMPC** via one lightweight forward pass; **300%** control-cost gain (pendulum), **48.75%** prediction gain (F1TENTH), **~1.85ms** overhead.
- **[[2506.04646|ActivePusher]]** — Extends [[1903.11239|TossingBot]]'s residual recipe to data selection: an MLP residual on an analytical quasi-static pushing model is read as a ==Gaussian Process== via the ==Neural Tangent Kernel==, so ==BAIT== Fisher-information batches pick informative pushes; baseline SE(2) accuracy on **55%** of the data, **100%** Push-to-Region success.
- **[[2505.20404|Soft Gripper Co-Design]]** — Trains a ==neural physics== surrogate (PointNet+MLP) on FEM simulation to jointly optimize gripper ==stiffness distribution== and grasp pose via gradient descent; **~1000x** faster than direct differentiable sim, **92.1%** vs rigid baselines' **60.5%**, real-world transfer confirmed.
...
- **[[2608.11461|Koopman Haptic Rendering]]** — Buys certification rather than speed: ==EDMD== lifts a ==Duffing-oscillator== virtual environment into nine monomial states so the coupled device-environment loop reduces to a linear ==unit-circle eigenvalue test==, with ==retract-and-relift== each step; lower position error than the nonlinear model where Lyapunov candidates were inconclusive.
...
- **[[2509.19597|SPACE2TIME]]** — Recasts spatially-varying disturbances as ==temporal variations== so a ==Physics-Informed== ==time-varying safety value function== can be learned offline (DeepReach) and queried online via an adaptive "time to return"; **100%** vs baseline's **20%** success on a hardware Crazyflie drone.
...
Physical reasoning sits one level above physical generation: the model must *talk about* physics consistently, not just produce physics-compliant pixels. Four very different mechanisms land here — a trained foundation model that internalizes physics via SFT→RL, a training-free agent that reconstructs an explicit, executable physics simulation from the video itself, a frozen VLM that discovers physics at test time through a verify-before-commit memory loop, and an agent that acquires physical properties by physically probing for them.

- **[[2608.21031|PhysCaP]]** — Acquires physics by acting: training-free ==PhysX== routines read mass from differential joint torque and stiffness from jaw displacement versus motor effort, while a ==Planner== agent decides when to probe and a ==Prioritizer== ranks candidates; **9/10** real success at **1.33–2.5** interactions, LIBERO **78%** where OpenVLA scores **0%**.
...
- **[[2602.20323|PhysMem]]** — A frozen-VLM planner learns physics at test time via a ==Scientific Memory Loop==: resonance-checked experience → clustered hypotheses → interaction-verified principles injected into the prompt; brick insertion **76%** SR vs **23%** direct-retrieval, verification ablation **76% → 64%**.
