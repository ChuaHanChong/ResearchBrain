...
- **[[2607.06856|Gen4U]]** — A deliberate counter-example to the paradigm above: probes ==intermediate spatiotemporal activations== of a frozen ==Veo3==/==Wan 2.2== diffusion model at a ==~60% noise bottleneck== via one forward pass, no reconstruction trained or run; **72.6%** SSv2 (beats V-JEPA-H), **0.075** AbsRel depth, complicating the "wasted capacity" framing above.
...
==Discriminative SSL that aligns related views==, either via negative-pair contrast (InfoNCE/SimCLR) or teacher-student self-distillation without negatives (DINO/DINOv2). Learns view-invariant features cheaply but contains no forward-dynamics signal — useless on its own for prediction-based planning.
...
The JEPA lineage walks a four-step conceptual ladder: visual-only self-supervision → dense local features → vision-language alignment → full action conditioning. Each rung adds one missing capability needed for robot control, and each is a single architectural change to its predecessor — not a from-scratch redesign. In this file the four rungs fold into two physical sub-sections below: §2.1 covers the first two (visual-only, then dense), §2.2 covers the last two (language, then action). This matters because it makes the ladder *composable*: you can pick the rung that matches your deployment constraint without inheriting irrelevant complexity from the higher rungs.
...
==Internet-scale video pretraining without robot data or language==. Establishes the "predict future embeddings from masked context" recipe and re-acquires local detail via dense supervision when downstream tasks need it. Both members keep encoder-only training; neither conditions on action. That "no robot data or language" framing describes the *pretraining objective* only — the flagship **80%** pick-and-place result below comes from V-JEPA-2-AC, a separately post-trained, action-conditioned variant (fine-tuned on **62 hours** of robot video) run inside an MPC loop, not from the frozen visual-only encoder in isolation.
...
==Add language alignment, then action conditioning, to the visual JEPA substrate==. All four members extend the prediction-not-generation bet across modalities: VL-JEPA via InfoNCE-aligned text embeddings, VLA-JEPA via a flow-matching action head on a VLM backbone, UWM-JEPA via an explicit belief state over the latent, Demo-JEPA via one-shot cross-embodiment imitation. Together they close the loop from "JEPA understands video" to "JEPA controls robots".
...
| Few-step in-domain fine-tune from a strong starting point | [[2506.09985\|V-JEPA-2]] → action head | Skip past dense features, language alignment, and full action conditioning (§2.1's second half and all of §2.2) if none are needed |
...
Beyond the core JEPA version chain in Part A, this section surveys the rest of the latent-prediction landscape for embodied AI: JEPA extended toward acting agents (§3.1), JEPA itself as an object of theoretical study (§3.2), non-JEPA latent architectures divided across four axes — algebraic-structured latent-action models, dual-branch training, frozen-feature dynamics, and unified diffusion (§3.3–§3.6) — field resources (§3.7), an alternative-latent-model lineage thread (§3.8), and the historical precursors that predate JEPA's own formalization (§3.9).

#### 3.1 JEPA for Embodied Control & Robotics

Papers that extend the JEPA framework toward acting agents: each adds one missing control axis (action conditioning, causal intervention, test-time adaptation, RL value learning).
...
- **[[2608.22764|LpWM]]** — A LeCun-coauthored sparse-code world model porting [[2602.01456|LpJEPA]]'s ==RDMReg== plus a ==RepReLU== exact-zero activation into action-conditioned dynamics; sparse latents beat dense LeWM by **24–57%** PushT SR at intermediate predictor capacity and **84.7%** vs **65.3%** on Piecewise navigation, the binary support decoding zone identity at **94–99%**.
...
- **[[2512.24497|JEPA-WM]]** — A ==component-wise empirical study== isolating ==Joint-Embedding Predictive World Model== design choices (encoder, rollout loss, planner) — an ablation, not a new-capability paper; proprioception always helps, **2-step** rollout loss optimal in sim vs **6-step** for real DROID, and tuning-free Nevergrad matches CEM; beats DINO-WM and V-JEPA-2-AC.
...
#### 3.3 Algebraic-Structured LAMs

Latent-action or latent-dynamics models with explicit compositional/algebraic structure imposed on the latent space — composition and reversal constraints, program-like decomposition, or structured mode primitives — rather than a free-form learned embedding.
...
#### 3.4 Dual-Branch Training

World models trained via two coupled networks or stages rather than one — a deployable branch aligned at training to a privileged, future-informed branch, or two models (forward and inverse dynamics) trained jointly and reciprocally.
...
#### 3.5 Frozen-Feature Dynamics

Dynamics predicted on top of a representation held fixed once the dynamics module takes over — either a frozen pretrained visual encoder ([[2304.07193|DINOv2]], SigLIP) or a self-trained tokenizer (VQ-VAE occupancy codes) whose weights stop updating during rollout prediction.
...
- **[[2409.11356|RenderWorld]]** — A vision-only [[2311.16038|OccWorld]] successor that self-generates its own 3D occupancy labels via anchor-based ==Gaussian Splatting== (Img2Occ), then tokenizes air and non-air voxels in two separate ==VQ-VAE codebooks== (==AM-VAE==); **27.87** mIoU on Occ3D-nuScenes (best among 2D-input methods), **20.80** vs OccWorld's **17.14** 4D-forecast mIoU.
...
- **[[2407.15843|CarFormer]]** — Runs driving dynamics on ==frozen SAVi slots== extracted from BEV sequences, with a ==GPT-2 autoregressive decoder== + ==block attention== jointly emitting actions and forecasting future slots; **74.89** Longest6 driving score at half PlanT's run-to-run variance, and dropping the forecasting loss costs **0.79 → 0.63** infraction score.

#### 3.6 Unified Diffusion

World models where a diffusion or flow-matching objective is the shared mechanism unifying generation, dynamics, and — in several — the action head itself.
...
#### 3.7 Latent-WM Field Resources

Resources that don't fit a single-paper method bullet: field-defining surveys that taxonomize the latent-prediction landscape (one architectural — decoupled vs unified, one cognitive — perception vs meta-cognition), position/blueprint papers that argue for a research direction without running experiments, and the training-data resource that operationalizes what "interactive world modeling" requires.
...
- **[[2603.15381|Autonomous-Learning-Framework]]** — A LeCun-Malik-Dupoux cognitive-science blueprint for self-improving AI, no experiments run: ==System A== (observation → JEPA-style world model) + ==System B== (action) + ==System M== (meta-control), bootstrapped by ==Evolutionary-Developmental bilevel optimization==; positions latent world models as the substrate action-learning consumes.
...
#### 3.8 Alternative-Latent-Models Thread
...
```text
╔═════════════════════════════╗    ╔═════════════════╗    ┌────────────┐    ╔═════════════════╗    ╔═══════════════╗
║ DINOv2 (Frozen visual SSL)  ║───►║ DINO-WM (2024)  ║───►│ UWM (2025) │───►║ LaDi-WM (2025)  ║───►║ Motus (2025)  ║
╚═════════════════════════════╝    ╚═════════════════╝    └────────────┘    ╚═════════════════╝    ╚═══════════════╝
                                            ▲                    ▲
                                            │                    │
                                  ┌───────────────────┐  ┌─────────────────┐
...
> [[2411.04983|DINO-WM]] showed you don't even need to train a new encoder — frozen [[2304.07193|DINOv2]] features are rich enough for world modeling, with the dynamics model operating entirely in frozen feature space for zero-shot planning. From there the variant choice is constraint-driven: need **action conditioning** → [[2602.10098|VLA-JEPA]] / [[2602.11832|JEPA-VLA]] / [[2411.04983|DINO-WM]]; need **zero-shot transfer** → freeze the encoder ([[2411.04983|DINO-WM]]); need **dense local features** → [[2603.14482|V-JEPA-2.1]]; need **multi-camera driving** → [[2511.19221|Percept-WAM]]; need **value learning** → [[2510.00739|TD-JEPA]]. The non-JEPA thread ([[2504.02792|UWM]] → [[2505.11528|LaDi-WM]] → [[2512.13030|Motus]]) reaches the same latent-prediction insight by a different route. This same frozen-vs-trainable-encoder choice reappears on the WAM deployment side in [[06_WAM#3.1 JEPA Family]], which catalogues the same JEPA variants as backbone options for world-action models. The tactile counterpart, where the frozen encoder is pretrained on touch streams rather than pixels, is [[11_Contact-Rich-and-Tactile-Control#2.2 Touch Foundation Models — SSL Representations on Tactile Streams]]. [[2505.13696|ESWM]]'s episodic spatial world model, with its external editable memory bank, sits in [[09_Robot-Memory#1.3 Episodic & Compression Memory for Manipulation]].
...
==Reason about 3D space and physical relationships== via either latent embeddings ([[2604.02097|LatentUM]], [[2510.00855|DyVA]]) or explicit metric/symbolic maps that ground the reasoning trace ([[2601.11442|Map2Thought]]). Bridges perception and action through a spatial substrate the policy can directly consume.
...
==Reason by iterating on continuous embeddings== rather than generating discrete text tokens. A mixed bucket: general-domain latent-CoT techniques developed and evaluated outside embodied AI ([[2604.22709|Abstract-CoT]], [[2511.19418|COVT]], [[2601.05877|iReasoner]], [[2601.21598|ATP-Latent]]) that extend the [[2412.06769|Coconut]] / [[2502.05171|Huginn]] "thinking in latent space" paradigm and sit here as precursors, alongside early embodied instantiations ([[2509.25681|dVLA]], [[2503.15558|Cosmos-Reason1]]) that already run on physical agents. For the full reasoning-insertion taxonomy see [[05_VLA-Reasoning-and-CoT#3. Latent Reasoning — Token-Free CoT]].
...
- [[16_Self-Evolving-VLA-WAM]] — Self-evolving VLAs & WAMs ([[2602.10098|VLA-JEPA]] as a self-evolving target)
...
- [[14_Egocentric-Pretraining-and-Human-Video]] — Egocentric pretraining substrates for latent models
- [[11_Contact-Rich-and-Tactile-Control]] — Force/tactile policies deep-dive; complements latent representation for multi-sensor inputs
- [[01_Embodied-AI-101]] — Embodied AI basics

---

*See [[06_WAM]] for the full WAM taxonomy, [[05_VLA-Reasoning-and-CoT]] for latent reasoning insertion, or [[16_Self-Evolving-VLA-WAM]] for how latent world models enable self-evolution.*
