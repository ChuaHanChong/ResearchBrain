...
==Learned tactile representations== — spanning SSL foundation encoders, architecture search, few-shot adaptation, and cross-domain (neuro/medical) representation studies — that inform how touch signals should be encoded, whether or not the encoder is deployed downstream in a robot policy. The touch analog of [[2304.07193|DINOv2]] anchors one end of this axis: pretrained frozen encoder beats end-to-end task-specific training by a wide margin once labels are scarce.
...
#### 2.4 Tactile Data-Capture Hardware — Rigs & Passive Capture
...
- **[[2608.19372|Spatially Distributed Tactile Feedback]]** — A teleop interface relaying GelSight fingertip deformation to a ==32-DoF electro-osmotic cutaneous shape display== atop a 2-DoF ==bilateral kinesthetic== telemanipulator; full feedback cuts ==DTW== distance to natural human trajectories **29-79%** and lowers inter-trial variability, which the authors link to demonstration quality.
...
#### 3.1a Dedicated-Sensor Injection

The most direct integration: force enters at the action expert (or its controller), giving the policy a hybrid position-force output space. These papers preserve the pretrained VLM backbone and add force capacity through dedicated parameters at the action stage, driven by a real dedicated F/T or tactile sensor injected via cross-attention, an adapter, or a gating mechanism.
...
#### 3.1b Sensorless/Proxy Force Estimation

A complementary minority pattern: rather than adding hardware, these papers estimate force from signals the robot already produces — motor current, joint-torque residuals, bilateral-teleop reaction forces, or a tactile-equipped teacher distilled away at inference — explicitly avoiding a dedicated F/T or tactile sensor at deployment.
...
#### 4.5a In-Hand Dexterous Reorientation & Blind Grasping

Multi-finger dexterous-hand tasks — in-hand reorientation, blind (vision-free) grasping, and finger-gaiting — where tactile is often the primary or sole perceptual channel, frequently distilled from a privileged-state sim teacher.

- **[[2607.13479|Sparse-Touch Mesh Reconstruction]]** — A ==topology-agnostic grid-query cross-attention== estimator reconstructs deformable-object meshes from sparse touch, no vision; cuts error **~66-80%** vs GPIS/IDW baselines. Arm-probing touch-only perception, not in-hand/dexterous — grouped here on the blind (vision-free) axis.
...
- **[[2604.20295|ETac]]** — A lightweight tactile simulator splitting elastomer deformation into a learnable ==decay-based propagation kernel== plus a ==PointNet residual correction== calibrated on ==FEM==; **0.058/0.116 mm** flat/curved RMSE vs **0.163-0.447 mm** for Taxim/TacSL, at **869 FPS** over **4,096** envs, lifting full-hand blind grasping to **84.45%** SR vs **62.97%** pose-only.
...
#### 4.5b Arm/Bimanual Visuotactile Manipulation

Arm- and gripper-level manipulation — insertion, bimanual grasping, deformable-object handling, feeding — where vision remains primary and tactile is a complementary channel covering occlusion and regulating grip force.
...
- **[[2501.10356|DexForce]]** — ==Kinesthetic teaching== on a 6-axis-F/T dexterous hand converted into ==force-informed position targets== (observed pose plus measured contact force, reproduced by a ==Cartesian impedance controller==), with human-free replay collecting clean visuals for a ==Diffusion Policy==; **76%** avg SR over 6 tasks vs near-zero position-only.
- **[[2501.03606|VTAO-BiManip]]** — ==Masked visual-tactile-action pretraining== on 216 human bimanual demos (Hololens2 + tactile gloves + mocap) that also reconstructs object 6-DoF pose/size, feeding frozen features to two-stage ==curriculum RL== (fixed then released bottle); **63±4%** seen / **51±7%** unseen bottle-cap twisting, **+20pp** over prior visual-tactile pretraining.
...
- **[[2608.06208|ErgoSurf]]** — ==Ergodic coverage of unknown surfaces==: contact wrench from a ==momentum-based observer== feeds an online ==Gaussian Process Implicit Surface==, approximated by a tangent-plane point cloud over which a ==heat-equation (HEDAC)== field diffuses task goals; **5-21x** lower ergodic cost than uncertainty-gradient exploration, real **7-DoF** coverage at **4 Hz**.
