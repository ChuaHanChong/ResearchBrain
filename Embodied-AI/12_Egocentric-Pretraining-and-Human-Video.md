...
- **[[2604.27621|Robot Learning from Human Videos Survey]]** — hierarchical taxonomy by *what the human video supervises*: ==task-oriented==, ==observation-oriented==, ==action-oriented==. Quantifies the 2024 tipping point: **49%** of observation-oriented and **52%** of action-oriented methods use egocentric viewpoints; **44%** of action-oriented methods deployable from human video alone.
...
- **[[2602.10106|EgoHumanoid]]** — *Robot-free* egocentric demonstrations for humanoid loco-manipulation via a ==VR-based collection rig==, ==depth-based view alignment==, ==unified 6-DoF delta end-effector poses==; the co-trained VLA omits proprioception. **+19pp** in-domain SR (**78% vs 59%**), **+51pp** novel-environment (**82% vs 31%**), at **~2×** collection speed.
...
- **[[1706.04261|Something-Something]]** — **108,499** crowd-acted clips across **174** fine-grained ==human-object interaction== classes via ==natural-language caption-templates== with ==contrastive "pretending" examples==; 2D+3D-CNN baselines top out at **44.9%** top-1 error on 10 classes and **88.5%** on all 174 — a low-level common-sense verification benchmark.
...
- **[[2110.07058|Ego4D]]** — **3,670 hours** of egocentric video from **931 wearers** across 9 countries / 74 locations with **3.85 million** dense narrations and ==multimodal data== (audio, ==3D environment meshes==, eye gaze, stereo); a ==five-task benchmark suite== with large headroom (NLQ R@1 IoU **5.47%**, AVD WER ~**60%**). First dataset large enough for VLA pretraining at internet scale.
...
- **[[2505.11709|EgoDex]]** — **829 hr** ==Apple Vision Pro== egocentric video, **338,000** demos across **194** tabletop tasks, annotated at **30 Hz** with ==SE(3) poses== for head, arms, and **25** hand joints via ARKit; benchmarks ==dexterous trajectory prediction==. Flow-matching + diffusion beat BC when K>1; visual goal-conditioning gives **22%** average-distance reduction.
...
- **[[2602.10106|EgoHumanoid]]** — *Robot-free* egocentric demonstrations aligned to humanoid loco-manipulation via ==depth-based view alignment== + ==unified 6-DoF delta end-effector poses==; **~2×** faster to collect than teleoperation, **+51pp** novel-environment generalization (**82% vs 31%**). Bridges the embodiment gap at *data-collection time* rather than training or policy time.
- **[[2503.13441|PH2D]]** — Task-oriented egocentric demo dataset via *consumer-grade VR* (**~3.02M** frames / **~27k** demos); paired with ==Human Action Transformer (HAT)==, a policy co-trained on mixed human/robot data over a **54-dim** state-action space; **~100%** relative OOD generalization vs robot-only; collection **~5×** faster than teleop (**4.09s** vs **19.72s** grasping).
...
- **[[2606.04708|VISTA]]** — adapts human-collected ==UMI fisheye== data for VLA training via ==UMI-VQA== (**8M** pairs) + ==physics-validated trajectory scoring== + ==flow-matching action-expert== co-training; **0.598** avg SR over 20 real tasks (**+7.0** over π0.5, **+28.5** over LingBot-VLA); validated subsets hit **0.65** vs **0.00** for low-score data.
- **[[2605.13083|TouchAnything]]** (EgoTouch) — **first multi-view egocentric + bimanual dense tactile** dataset: **20 hours** of synchronized head + wrist video with bimanual 3D hand pose *and* dense tactile pressure maps. Shared encoder + cross-view fusion with ==view dropout training==; Volumetric IoU **+6.1%** over egocentric-only, all-view → ego-only drop **−27.20% → −5.78%**.
- **[[2605.09613|SABER]]** — **100+ hours** of grocery-store activity with ==egocentric== + ==exocentric== cameras; three action streams (==[[2410.11758|LAPA]] latent actions==, Dex-Retargeting, Body Pose). As post-training, lifts [[2503.14734|GR00T N1.6]] on [[2511.10276|RoboBenchMart]] **13.4% → 29.3%** mean SR (**2.19×**); `close_fridge` **100%**, `open_fridge` **12% → 82%**.
...
- **[[2602.16710|EgoScale]]** — log-linear loss curve over **20,854 hours** of heterogeneous egocentric data, *deployment-predictive*. **+54%** task SR on 22-DoF dexterous hand; **88%** shirt-folding and **55%** bottle-unscrewing from a single robot demo; **+30%** cross-embodiment on 7-DoF tri-finger transfer. Two-stage recipe keeps the curve flat at scale.
...
> Before [[2602.16710|EgoScale]], robot pretraining had no scaling law — practitioners gathered "as much data as they could" without a principled stop point. [[2602.16710|EgoScale]]'s log-linear curve enables *compute-optimal* training: pick the data-compute trade-off that maximizes downstream performance per dollar. The open frontier — embodiment-specific curves, modality-mixed scaling, long-tail domain coverage — is now framed as a tractable research agenda. Cross-reference [[05_VLA#1. Design-Space Principles]] for how the [[2412.14058|RoboVLMs]] 600-experiment study mapped the design-space empirically, and [[13_Self-Evolving-VLA-WAM#3. Core Mechanisms of Self-Evolution]] for how self-evolution might *extend* the scaling curve via synthetic-data generation.
...
- **[[2312.13139|GR-1]]** — Pioneer ==GPT-style decoder-only transformer==: ==language-conditioned video generative pre-training== on **800k** Ego4D clips, then finetune on CALVIN + real-robot data, jointly predicting actions/future frames. **94.9%** CALVIN multi-task (vs HULC **88.9%**), **+32.1pp** zero-shot unseen-scene, **0.79** real-world transport (vs RT-1 **0.27**).
- **[[2410.06158|GR-2]]** — Generation-2 ==GPT-style transformer== over tokenized text/video/actions: ==video-language pre-training on 38M== clips, then fine-tune with a ==conditional VAE== for diverse action trajectories. **97.7%** multi-task tabletop SR, **79.0%** industrial bin-picking (vs GR-1 **35.9%**), **98.6%** CALVIN single-task with **4.64** task-sequence length.
- **[[2409.20537|HPT]]** — Modular ==stem-trunk-head== transformer pretrained on real-robot + sim + human-video + multi-embodiment data: embodiment-specific stems tokenize proprioception + vision into a shared trunk via ==cross-attention==. Validates robot scaling laws up to **>1B** params; **+10–30%** sim and **+20%** real SR over training from scratch.
...
- **[[2606.06194|ActiveMimic]]** (Active Perception) — recovers *synchronized* egocentric camera + bimanual wrist trajectories from human video as a unified ==27-D action vector== via ==mix-of-transformers== (human pretrain → robot fine-tune); **90.1%** Restocking / **91.7%** Finding over robot-pretrained baselines, **79.0%** under flashing light — camera motion as action, not noise.
- **[[2507.15597|Being-H0]]** (Physical Instruction Tuning) — three-stage: Stage 1 VLA pretraining on UniHand via ==GRQ-VAE== for part-level (wrist + finger) motion tokenization; Stage 2 physical-space alignment; Stage 3 robot-data post-training. **99.8–100%** valid generation rate, beating [[2503.14734|GR00T N1.5]] on MPJPE; **25%** of teleop data matches **50–100%** baselines.
- **[[2605.00078|Being-H0.7]]** (Latent Dual-Branch) — replaces pixel WAM with a **latent world-action model**: ==Mixture-of-Transformers== aligning a deployable "prior" branch to a "posterior" branch fed privileged future embeddings; posterior dropped at deploy. **3–4 ms/step**; **99.2%** [[2306.03310|LIBERO]], **62.1%** [[2406.02523|RoboCasa]], **67.5%** real-world Dynamic Scene.
...
- **[[2512.22414|π0.5 + ego]]** (Co-Training Recipe) — simplest recipe: integrate egocentric human video into a pre-trained VLA's training mixture with the same loss as robot data; **no explicit kinematic alignment**. Head-worn + optional wrist cameras with 6D pose + hand-keypoint estimation. Scene gen **32 → 71%**, Dresser **25 → 50%**, egg-sorting **57 → 78%** — transfer **emerges**.
- **[[2602.10106|EgoHumanoid]]** (Cross-Embodiment Loco-Manipulation) — Robot-free egocentric demos via head-worn camera; ==depth-based view alignment== + ==unified 6-DoF delta EE poses== retarget human body motion to humanoid joints. **~2×** faster than teleoperation; **+19pp** in-domain (**78%**), **+51pp** novel-env (**82%**); human-only reaches **100%** on navigation-dominated subtasks.
- **[[2604.15483|π0.7]]** — Steerable-generalist SOTA: a **5B** VLA with ==multimodal context conditioning== (subtask instructions + multi-view subgoal images + episode metadata), ==Knowledge Insulation==, ==prompt dropout==; out-of-the-box matches task-specific fine-tunes on dexterous long-horizon tasks and transfers cross-embodiment to bimanual UR5e with no platform-specific data.
- **[[2604.24681|MoT-HRA]]** (Human-Intention Priors) — pretrains on **HA-2.2M**; hierarchical ==embodiment-agnostic 3D trajectory prediction== → ==latent human-intention model== (MANO) → robot action expert, with ==knowledge insulation==. **66.1%** SimplerEnv avg SR (**+22.3pp** over next-best **43.8%**); **0.136 m ADE** / **34.16°** finger-joint error on Ego4D.
- **[[2510.21571|VITRA]]** (Real-Life Human Activity Videos) — automated pipeline turning *unstructured* human videos into VLA data: ==3D hand+camera pose tracking==, ==atomic-action segmentation==, VLM-generated instructions; PaliGemma-2 + ==Diffusion-Transformer action expert==. Yields a **1M-episode / 26M-frame** dataset; after **1.2K** real robot trajectories generalizes to unseen objects.
...
- **[[2602.09013|VIDEOMANIP]]** — **3D hand-object trajectory reconstruction** from monocular RGB human video (3D meshes + hand poses + metric depth), refined with ==contact optimization==; synthesizes multiple demos per video. RGB human video as the *only* data source. Dexterous LEAP Hand: **63.75%** grasping (20 objects in IsaacGym), **62.86%** real-world (7 tasks).
- **[[2602.10106|EgoHumanoid]]** — **Robot-free egocentric retargeting**: head-worn egocentric demos + ==depth-based view alignment== + ==unified 6-DoF delta EE poses== retarget human body motion to humanoid joints, omitting proprioception in the co-trained VLA; **+51pp** novel-environment SR (**82% vs 31%**) — solves the embodiment hop at *data-collection time*.
- **[[2604.07457|CMP]]** — **Competence-manifold projection** for safety-critical loco-manipulation: ==Lower-Bounded Safety Estimator== + ==Isomorphic Latent Space== for ==O(1) projection== of unsafe commands onto the closest safe hypersphere. Up to **10×** survival in OOD-Geometry sim (**46.9% vs 4.7%**), **100%** real in-distribution, **86.7%** extreme OOD at **2.99 ms**.
...
- **[[2606.02745|SeeTraceAct]]** — ==Visibility-aware latent planning== from cross-embodiment demo videos: a ==diffusion==-based VLA derives a ==visual latent plan== supervised by a trace decoder predicting future EE 2D coordinates *and* validity, handling off-screen EE. **+4.4 pp** on cross-embodiment ROBOCASA-DC (**12.8%** vs **8.4%**), real-world unseen-task SR **37.5% → 50.0%**.
- **[[2512.22414|π0.5 + ego]]** — **"treat humans as another embodiment"** recipe: egocentric human video integrated into the pre-trained VLA's training mixture with the *same* loss as robot data; no MANO projection, no kinematic alignment. Head-worn + optional wrist cameras. Scene gen **32 → 71%**, Dresser **25 → 50%**, egg-sorting **57 → 78%**; transfer **emerges**.
...
- **[[2604.20012|EmbodiedMidtrain]]** — quantifies the VLM↔VLA *data*-distribution gap (==Maximum Mean Discrepancy==); a ==data engine== scores VLM samples by proximity to the VLA distribution and mid-trains on the top-K closest. A **1.1B** InternVL3.5 surpasses expert VLA baselines **3–8×** larger across Calvin ABC-D, SimplerEnv Bridge, and LIBERO-10.
- **[[2602.16710|EgoScale]]** — mid-training keeps the log-linear scaling curve flat at scale: Stage 1 broad human pretraining (**20,854 hours**), Stage 2 mid-training on a smaller embodiment-aligned human-robot dataset bridges the kinematic gap *before* fine-tuning. **+54%** task SR on 22-DoF hand; **88%** shirt-folding from a single robot demo; **+30%** cross-embodiment. See §3.
...
- **[[2604.18564|MultiWorld]]** — Multi-agent multi-view video world model on ==action-conditioned diffusion== + ==Flow Matching==: a ==Multi-Agent Condition Module== + ==Global State Encoder== over pretrained ==VGGT==, handling variable view counts. **FVD 179** vs baselines' **207–245**, **RPE 0.67** vs **0.72–0.75** from one checkpoint.
- **[[2410.06158|GR-2]]** — Generation-2 ==GPT-style video-pretraining backbone== pretrained on **38M** clips, ingesting egocentric streams as part of its general video corpus, then fine-tuned with action heads + ==conditional VAE== for trajectory diversity. **97.7%** multi-task tabletop SR and **75%** with only **50 demos/task** — the architecture the co-pretraining recipe (§6.2) extends.
...
- **[[2604.15483|π0.7]]** — Generalist SOTA combining egocentric pretraining (action priors) + video-WAM pretraining (spatiotemporal priors) atop a **5B** backbone; ==multimodal context conditioning==, ==multi-view subgoal images== (WM-regenerated), and ==episode metadata== with ==Knowledge Insulation==. The two objectives compose cleanly on overlapping data — neither degrades the other.
- **[[2604.20100|JoyAI-RA]]** — Replication of the egocentric + video-WAM co-pretraining recipe: ==multi-source pretraining== (web + ==EgoLive egocentric== + sim + real-robot), a ==unified camera-frame action space==, three-stage ==VLM → VLA co-pretraining → post-training==. **90.48%** / **89.28%** RoboTwin 2.0 Easy/Hard, **63.2%** RoboCasa GR1, **0.74** real AgiBot G1 (vs π0.5 **0.62**).
- **[[2312.13139|GR-1]]** / **[[2410.06158|GR-2]]** — Generation-2 ==GPT-style video-pretraining + action-decoder== pattern: GR-1 pretrains on **800k** Ego4D clips with joint [ACT]/[OBS] tokens (**94.9%** CALVIN); GR-2 scales to **38M** clips with a ==cVAE action head== (**97.7%** multi-task tabletop SR). The architectural anchor every modern generalist VLA descends from.
- **[[2602.15922|DreamZero]]** — Canonical **14B** pixel-space WAM reusing egocentric corpora: ==autoregressive diffusion transformer== with a ==flow-matching objective== jointly predicting future frames and actions, plus a ==DreamZero-Flash== variant at **7 Hz**. **62.2%** progress on seen tasks/unseen envs, **39.5%** unseen tasks, **+42%** relative cross-embodiment from 10–20 min of video.
...
> [!tip] The 2026 Stack
> Diverse cross-embodiment pretraining ([[2310.08864|OXE]]) + egocentric human pretraining ([[2110.07058|Ego4D]], [[2505.11709|EgoDex]], UniHand) + video-WAM pretraining (Cosmos, [[2602.15922|DreamZero]]) → flow-matching action head → in-domain post-training. This is the recipe behind [[2604.15483|π0.7]], [[2604.20100|JoyAI-RA]], and the next generation of generalist VLAs. Cross-reference [[07_WAM#2. VideoGen WAMs]] for the video-WAM side of the pipeline (where the same egocentric corpora are reused as pixel-prediction substrate) and [[05_VLA#5. World-Model-Augmented VLAs]] for how world-model-augmented VLAs compose this stack with planning and reasoning layers.
...
- [[05_VLA]] — VLA deep-dive; §1 generalist VLAs ([[2604.15483|π0.7]], [[2512.22414|π0.5+ego]]) build on egocentric pretraining
- [[07_WAM]] — WAM deep-dive; §2 video pretraining for robot policies overlaps egocentric pretraining
- [[08_Latent-World-Models]] — Latent world models; some egocentric-pretrained VLAs use latent prediction
- [[13_Self-Evolving-VLA-WAM]] — Self-evolution; egocentric pretraining provides robust priors that resist forgetting
- [[11_Physics-Aware-Embodied-AI]] — Physics priors complement egocentric pretraining for the 2026 generalist stack
- [[06_VLA-Reasoning-and-CoT]] — Reasoning-augmented VLAs that consume egocentric pretraining

---

*See [[05_VLA]] for the broader VLA design space, [[02_Dataset-Benchmark-Environment]] for dataset details, or [[01_Embodied-AI-101]] to start from the basics.*
