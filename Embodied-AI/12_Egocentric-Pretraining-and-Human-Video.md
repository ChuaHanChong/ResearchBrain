...
- **[[1706.04261|Something-Something]]** — **108,499** crowd-acted clips across **174** fine-grained ==human-object interaction== classes built via ==natural-language caption-templates== with ==contrastive "pretending" examples==; standard 2D+3D-CNN baselines top out at **44.9%** top-1 error on 10 classes and **88.5%** on all 174 — established that even simple actions are hard for visual reasoning, and remains a low-level common-sense verification benchmark for egocentric pretraining.
...
- **[[2505.11709|EgoDex]]** — **829 hr** ==Apple Vision Pro== egocentric video with **338,000** demos across **194** tabletop manipulation tasks, annotated at **30 Hz** with ==SE(3) poses== for head, shoulders, arms, and **25** hand joints via ARKit; benchmarks ==dexterous trajectory prediction== and ==goal-conditioned inverse dynamics==. Flow-matching + diffusion beat BC when K>1, performance scales with data, and visual goal-conditioning gives **22%** average-distance reduction.
...
- **[[2503.13441|PH2D]]** — Large-scale task-oriented egocentric demo dataset via *consumer-grade VR* (**~3.02M** frames / **~27k** demos); paired with ==Human Action Transformer (HAT)== — a unified policy co-trained on mixed human/robot data over a **54-dim** state-action space common to humans and humanoids; **~100%** relative OOD generalization vs robot-only; data collection **~5×** faster than teleop (**4.09s** vs **19.72s** grasping).
...
- **[[2606.04708|VISTA]]** — adapts human-collected ==UMI fisheye== data for VLA training via ==UMI-VQA== (**8M** pairs realigning the VLM to distorted wrist views) + a ==physics-validated trajectory scoring== pipeline + ==flow-matching action-expert== co-training; **0.598** avg SR over 20 real tasks (**+7.0** over π0.5, **+28.5** over LingBot-VLA), and validated subsets hit **0.65** vs **0.00** for low-score data.
- **[[2605.13083|TouchAnything]]** (EgoTouch) — **first multi-view egocentric + bimanual dense tactile** dataset: **20 hours** of synchronized head + wrist egocentric video with bimanual 3D hand pose *and* dense tactile pressure maps. The framework fuses head + wrist via shared encoder + cross-view fusion with ==view dropout training==; pushes Volumetric IoU **+6.1%** over egocentric-only baselines and reduces the all-view → ego-only drop from **−27.20% → −5.78%**.
- **[[2605.09613|SABER]]** — **100+ hours** of real grocery-store human activity with synchronized ==egocentric== + ==exocentric== cameras; three action streams (==[[2410.11758|LAPA]] latent actions==, Dex-Retargeting, Body Pose Retargets). As a post-training layer (SABER-MM, ==conditional flow-matching loss==), lifts NVIDIA [[2503.14734|GR00T N1.6]] on [[2511.10276|RoboBenchMart]] from **13.4% → 29.3%** mean SR (**2.19×** gain); `close_fridge` **100%**, `open_fridge` **12% → 82%**.
...
- **[[2602.16710|EgoScale]]** — log-linear loss curve over **20,854 hours** of heterogeneous egocentric data, strongly correlated with real-robot performance (*deployment-predictive*). **+54%** task SR on 22-DoF dexterous hand; **88%** shirt-folding and **55%** bottle-unscrewing from a single robot demo; **+30%** cross-embodiment improvement on 7-DoF tri-finger transfer. Two-stage recipe (broad pretraining → embodiment-aligned mid-training) keeps the curve flat at scale.
...
- **[[2312.13139|GR-1]]** — Pioneer ==GPT-style decoder-only transformer== with two-phase training: ==language-conditioned video generative pre-training== on **800k** Ego4D clips, then finetune on CALVIN + real-robot data using frozen ==CLIP text== + ==MAE ViT== encoders and jointly-predicted actions/future frames via ==[ACT]/[OBS] tokens==. **94.9%** CALVIN multi-task (vs HULC **88.9%**), **+32.1pp** zero-shot unseen-scene, **0.79** real-world transport SR (vs RT-1 **0.27**).
...
- **[[2409.20537|HPT]]** — Modular ==stem-trunk-head== transformer pretrained on a mixture of real-robot, simulation, human-video, and multi-embodiment data: embodiment-specific stems tokenize proprioception + vision into a shared trunk via ==cross-attention==, decoupling the backbone from the action head. Validates robot scaling laws (loss drops with data / model / compute up to **>1B** params); **+10–30%** sim and **+20%** real SR over training from scratch.
...
- **[[2606.06194|ActiveMimic]]** (Active Perception) — recovers *synchronized* egocentric camera + bimanual wrist trajectories from in-the-wild human video with off-the-shelf vision models, as a unified ==27-D action vector== trained via ==mix-of-transformers== (human pretrain → robot fine-tune); **90.1%** Restocking / **91.7%** Finding beating robot-pretrained baselines, **79.0%** under flashing light — models camera motion as action, not noise.
- **[[2507.15597|Being-H0]]** (Physical Instruction Tuning) — three-stage paradigm: Stage 1 VLA pretraining on UniHand via ==Grouped Residual Quantization (GRQ-VAE)== for part-level (wrist + finger separate) motion tokenization; Stage 2 physical-space alignment; Stage 3 post-training on robot data. **99.8–100%** valid generation rate, beating [[2503.14734|GR00T N1.5]] on MPJPE; **25%** of teleop data matches **50–100%** baselines.
- **[[2605.00078|Being-H0.7]]** (Latent Dual-Branch) — replaces pixel WAM with a **latent world-action model**: dual-branch transformer aligning a deployable "prior" branch to a "posterior" branch receiving privileged future embeddings from egocentric video. ==Mixture-of-Transformers== with per-branch attention masks; deploy-time posterior dropped. **3–4 ms/step**; **99.2%** [[2306.03310|LIBERO]], **62.1%** [[2406.02523|RoboCasa]], **67.5%** real-world Dynamic Scene.
...
- **[[2604.15483|π0.7]]** — Current steerable-generalist SOTA: a **5B**-parameter VLA with ==multimodal context conditioning== (subtask instructions + multi-view subgoal images + episode metadata), ==Knowledge Insulation== training, and ==prompt dropout==; out-of-the-box matches task-specific fine-tunes on dexterous long-horizon tasks (espresso making, box building, laundry folding) and transfers cross-embodiment to bimanual UR5e with no platform-specific data.
- **[[2604.24681|MoT-HRA]]** (Human-Intention Priors) — pretrains on **HA-2.2M**, a ==2.2M-episode human-demonstration dataset==; hierarchical decomposition into ==embodiment-agnostic 3D trajectory prediction== → ==latent human-intention model== (MANO) → robot action expert, with ==knowledge insulation== via a ==read-only key-value transfer==. **66.1%** SimplerEnv average SR (**+22.3pp** over next-best **43.8%**); **0.136 m ADE** / **34.16°** finger-joint error on Ego4D.
- **[[2510.21571|VITRA]]** (Real-Life Human Activity Videos) — automated pipeline turning *unstructured* real-life human videos into VLA data: ==3D hand+camera pose tracking==, ==wrist-speed-minima atomic-action segmentation==, and VLM-generated instructions; PaliGemma-2 backbone + ==Diffusion-Transformer action expert==. Yields a **1M-episode / 26M-frame** human VLA dataset; after fine-tuning on only **1.2K** real robot trajectories it generalizes to unseen objects and environments.
...
- **[[2606.01951|Ego-Video Robot Nav]]** — Explicit ==camera-motion projection== for navigation: estimates 6-DoF egocentric camera motion (SuperPoint + LightGlue), then ==kinematically projects== it to a ==2-DoF planar action space== (forward + angular velocity), before co-training a VLA with robot data. Human-only nearly fails (**11.1%** SR) but co-training reaches **100%** on seen objects, **94.4%** with distractors, and **75.0%** on untrained positions.
...
- **[[2604.07457|CMP]]** — **Competence-manifold projection** for safety-critical loco-manipulation: a ==hierarchical latent space== with a ==Lower-Bounded Safety Estimator== and an ==Isomorphic Latent Space== enabling ==O(1) projection== of unsafe commands onto the closest safe hypersphere. Up to **10×** survival-rate gain in OOD-Geometry sim (**46.9% vs 4.7%**), **100%** real-world in-distribution survival, **86.7%** under extreme OOD at **2.99 ms** latency.
...
- **[[2606.02745|SeeTraceAct]]** — ==Visibility-aware latent planning== from cross-embodiment demo videos: a ==diffusion==-based VLA derives a ==visual latent plan== supervised by a trace decoder whose dual heads predict future EE 2D coordinates *and* their validity, handling off-screen EE. **+4.4 pp** on precision-sensitive cross-embodiment ROBOCASA-DC (**12.8%** vs **8.4%**) and real-world unseen-task SR **37.5% → 50.0%**; gain concentrates on high-precision tasks.
...
- **[[2604.20012|EmbodiedMidtrain]]** — quantifies the VLM↔VLA *data*-distribution gap (==Maximum Mean Discrepancy== + t-SNE), then a ==data engine== scores VLM samples by proximity to the VLA distribution and mid-trains the backbone on the top-K closest. A **1.1B** InternVL3.5 surpasses expert VLA baselines **3–8×** larger; consistent gains across Calvin ABC-D, SimplerEnv Bridge, LIBERO-10 — the *embodiment* gap is really a *distribution* gap solvable by selection.
- **[[2602.16710|EgoScale]]** — mid-training is the mechanism that keeps the log-linear scaling curve flat at scale: Stage 1 broad human pretraining (**20,854 hours**), Stage 2 mid-training on a smaller embodiment-aligned human-robot dataset bridges the kinematic gap *before* final fine-tuning; without it the slope collapses earlier. **+54%** task SR on 22-DoF hand; **88%** shirt-folding from a single robot demo; **+30%** cross-embodiment improvement. See §3.
...
- **[[2604.15483|π0.7]]** — Current generalist SOTA combining egocentric pretraining (action priors) + video-WAM pretraining (spatiotemporal priors) atop a **5B** backbone; ==multimodal context conditioning== over subtask instructions, ==multi-view subgoal images== (WM-regenerated asynchronously), and ==episode metadata== with ==Knowledge Insulation== keeps suboptimal data from degrading performance. The two objectives compose cleanly on overlapping data — neither degrades the other.
- **[[2604.20100|JoyAI-RA]]** — Independent replication of the egocentric + video-WAM co-pretraining recipe: ==multi-source pretraining== (web + ==EgoLive egocentric== + simulation + real-robot), a ==unified camera-frame end-effector action space== (6-DoF + dimensionality masking), and three-stage ==VLM → VLA co-pretraining → post-training==. **90.48%** / **89.28%** RoboTwin 2.0 Easy/Hard, **63.2%** RoboCasa GR1, **0.74** real AgiBot G1 (vs π0.5 **0.62**).
...
- **[[2602.15922|DreamZero]]** — Canonical **14B** pixel-space WAM reusing egocentric corpora: ==autoregressive diffusion transformer== with a ==flow-matching objective== jointly predicting future video frames and robot actions, plus a ==DreamZero-Flash== variant at **7 Hz** control. **62.2%** task progress on seen tasks in unseen environments, **39.5%** on entirely unseen tasks, **+42%** relative cross-embodiment transfer from 10–20 min of video-only demos.
