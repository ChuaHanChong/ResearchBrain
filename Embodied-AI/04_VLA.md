...
- **[[2608.24042|HSR]]** — A ==hierarchical skill retrieval== pipeline: K-means clusters prior-dataset instructions into skills, a ==Skill Confidence Score== ranks LLM subtask plans by held-out BC validation loss, and language-retrieved episodes are reranked by ==VAE behavior-feature distance==; **43.5%** LIBERO-Long (vs Behavior Retrieval **33.2%**), **48/80** real xArm.
...
- **[[2602.20231|UniLACT]]** — Pretrains on ==depth-aware latent action tokens== from **UNILARN**, a shared RGB-depth ==vector-quantization== pipeline whose second codebook re-quantizes both modalities into one unified latent, depth used only in training (RGB-only inference); **3.10** CALVIN ABC→D (**+29.2%** rel over Moto), **62.5%** real xArm7.
...
- **[[2602.00557|ConLA]]** — Splits the ==inverse-dynamics== latent into action and visual halves, supervised by an ==action-class supervised contrastive loss== and ==reverse-order frame augmentation== InfoNCE so motion disentangles from appearance before VQ-VAE quantization; **64.6%** SimplerEnv from human video alone, **+12.5pp** over LAPA.
- **[[2601.22467|CARE]]** — Predicts ==continuous latent-action tokens== straight from image-instruction pairs on Prismatic-7B, co-trained with ==next-frame feature== and ==point-track prediction== decoders fused by an ==uncertainty-weighted loss==, then LoRA-mapped to actions; **77.7%** LIBERO label-free (vs LAPA **64.3%**), **+12.5%** on Long over OpenVLA.
- **[[2511.23034|LatBot]]** — Two learnable VLM tokens split latents into ==scene tokens== (environment change) and ==motion tokens== (robot movement), fused by a SANA-initialized ==unified decoder== reconstructing frames and actions jointly, distilled via ==latent-action alignment== + ==reasoning-preservation== losses; **87.5%** SIMPLER WidowX (**+32.3pp** over π0.5).
...
- **[[2602.03983|DySta]]** — Splits each frame's visual tokens into ==multi-level static tokens== (cached once) and dynamic tokens (recomputed), with a ==Gumbel-softmax recache gate== trained under an ==InfoNCE temporal-persistence loss==; **+3.9%** SimplerEnv SR at **43.4%** FLOPs and **2.26×** speedup, **+23.3pp** on memory-dependent real tasks.
- **[[2509.21354|KV-Efficient VLA]]** — A model-agnostic ==chunked KV cache== where an MLP aggregates fixed-size blocks and an ==LSTM retention gate== keeps only high-utility chunks beside an uncompressed recent window, LoRA-tuned on OXE; **−24.6%** FLOPs, **1.34×** speedup, **1.87×** less KV memory across OpenVLA, CogACT and HybridVLA.
...
- **[[2503.10322|LiteVLP]]** (1B) — ==Conversation-style visuomotor instruction tuning== of an InternVL2-1B with prior actions and multi-image history appended, plus a ==multi-observation compression== module applying ==run-length tokenization== to static patches (**−60%** image tokens); **84.2/78.1/75.4%** VIMA-Bench L1/L2/L3 on **1.2%** of the data.
...
- **[[2508.09032|ST-VLA]]** — Projects ==Co-Tracker== keypoint trajectories directly onto ==ZoeDepth==-predicted depth maps, fusing spatial and temporal cues into one ==traced depth prompt== read by SpatialVLA's Ego3D encoder; **38.1%** SimplerEnv mean SR (vs SpatialVLA **34.1%**, TraceVLA **19.9%**) from only **52** fine-tuning trajectories.
...
- **[[2602.04315|GeneralVLA]]** — Skips on-robot data entirely: an ==Affordance Segmentation Module== (MLLM + ==SAM decoder==, iterative point refinement) marks 2D affordances, then a ==3DAgent== lifts them to 3D and plans a multi-stage end-effector path from a retrieved ==KnowledgeBank==; solves all **14** RLBench tasks zero-shot, **63.4%** reference accuracy.
...
- **[[2604.13942|Goal2Skill]]** — A VLM planner emitting sub-tasks with ==pre/post-conditions and execution horizons==, backed by ==structured memory== (episodic history + working summaries + error register) feeding a verifier and ==reflection engine==, over a diffusion VLA with ==geometry-preserving masked filtering==; **32.4%** RMBench (vs **9.8%** best baseline).
...
- **[[2509.15212|RynnVLA-001]]** — A two-stage generative curriculum: an autoregressive ==Chameleon-based image-to-video== transformer pretrained on **12M** ego-centric manipulation videos, then joint frame-and-wrist-keypoint modeling, with an ==ActionVAE== compressing action chunks into latents; **90.6%** SO100 avg (vs π0 **70.4%**), stages compounding **4.4%→90.6%**.
...
- **[[2512.05107|StARe-VLA]]** — Segments trajectories into semantic stages from end-effector geometry, turning per-stage Euclidean cost into ==potential-shaped dense rewards== for ==Stage-Aware TPO== (stage-gated preference pairs) and ==Stage-Aware PPO==, chained by an ==IPI== pipeline; **98.0%** SimplerEnv-WidowX / **96.4%** ManiSkill3 (**+5.4/+25.9pp**).
...
- **[[2608.19589|OrthoSkillVLA]]** — Replay-free continual learning via ==module-aware orthogonal subspace budgets== (looser for the VLM, stricter for the ActionHead) with ==gradient-informed orthogonal LoRA==, plus a frozen-base ==MoE velocity decoder== routed training-free; LIBERO skill-incremental **56.61%→83.50%** final SR, NBT **0.46→0.13**.
...
- **[[2603.07647|TempoFit]]** — A training-free retrofit giving frozen single-frame VLAs history via a ==layer-wise FIFO KV cache== at select intermediate layers, retrieved by parameter-free ==K-to-K matching== with an ALiBi-style ==frame-gap recency bias== and ==norm-preserving residual loading==; π0.5 LIBERO-LONG **92.6%→96.6%** at near-zero latency.
- **[[2603.01465|Keyframe-Chaining VLA]]** — Distills continuous observations into sparse ==event-triggered semantic keyframes== via a two-stage ==triplet-metric== selector with ==FiLM-modulated== transition detection, interleaved into GR00T-N1.5 with a flow-matching head; **92.0%** on a non-Markovian ManiSkill suite (vs **57.0%** best dense-history).
- **[[2512.20166|LoLA]]** — Pairs high-resolution current-frame encoding with ==selective spatial-temporal sampling== of downsampled history, then a ==State-Aware Latent Re-representation== outer-products robot-state queries against the VLM's KV cache before a flow-matching expert; **96.2%** LIBERO, **71.9%** SIMPLER WidowX (**+20.6%** rel).
...
- **[[2509.17759|MotionTrans]]** — Maps VR-collected human demos into the robot's own observation-action space via ==dex-retargeting==, relative-pose action chunks and ==unified embodiment-independent normalization==, then ==weighted human-robot cotraining==; **9/13** human-only tasks reach non-trivial zero-shot SR (~**20%**), pretraining adds **~40%** few-shot.
...
- **[[2509.22199|MimicDreamer]]** — Turns egocentric human video into VLA supervision via ==homography-based EgoStabilizer== viewpoint canonicalization, a CogVideoX ==H2R Aligner== video diffusion repainting human into robot views, and ==constrained IK== action mapping; **85.0%** SR at equal data (vs **65.8%** robot-only), **+32%** on Insert Tennis.
...
- **[[2602.21445|AutoHorizon]]** — Reads ==action self-attention weights== as a proxy for a flow VLA's predictive limit, locating attention plateaus with a ==bidirectional soft pointer== to set the execution horizon per chunk at test time, no retraining; matches brute-force Static-Oracle+ on LIBERO (**96.5/98.0/94.4/92.1%**) where fixed horizons drop to **0%**.
...
- **[[2605.11381|Kairos-Serving]]** — A physical-AI serving system whose ==dynamic execution horizon== reads ==diffusion confidence== to set actions-per-chunk at no extra cost, paired with a ==wait-ratio execution-aware scheduler== and edge-cloud ==offloading==; **2.67×** longer execution horizons at equal accuracy, **31.8–66.5%** lower end-to-end latency.
...
- **[[2606.11324|Embodied-R1.5]]** — An 8B brain unifying spatial cognition, planning-correction and pointing over a **15B-token** auto-built corpus via ==multi-task balanced RL== (difficulty filtering + degenerate-group masking + global reward normalization), served as an asynchronous ==Planner-Grounder-Corrector== loop; **70.4%** avg over 21 embodied benchmarks.
...
- **[[2604.15671|ChemBot]]** — Pairs a ==Set-of-Mark== Scene Describer + iterative ==Subtask Reasoner-Reflector== over ==dual-layer memory== with **Skill-VLA**, a GR00T policy whose ==cross-attention progress head== predicts a continuous completion scalar to self-terminate subtasks; **0.766** decomposition edit distance, beats π0.5 on 6–11-step real UR3 chemistry.
- **[[2509.08820|RoboChemist]]** — A ==dual-loop== chemistry system whose outer VLM decomposes tasks, emits ==instruction-aware visual prompts== for spatial grounding on transparent and hazardous reagents, and monitors subtask success for condition-based re-execution around an inner VLA; **+23.57pp** avg SR and **+0.298** compliance over VLA baselines.
...
- **[[2512.22983|Foundation-Model Manipulation Survey 2026]]** — Formalizes manipulation as two ==coupled mappings== (==f_plan== → structured planning artifact, ==f_act== → executable action), with a six-category planning taxonomy and a three-component low-level taxonomy (input / latent / policy learning); only **1** of **14** highlighted methods targets explicit 3D grounding.
