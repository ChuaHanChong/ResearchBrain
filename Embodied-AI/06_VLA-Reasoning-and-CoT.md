...
- **[[2503.22020|CoT-VLA]]** — A **7B** [[2409.04429|VILA-U]] ==unified multimodal foundation model== jointly trained on robot demonstrations and action-less video (EPIC-KITCHENS); at inference emits a future-frame token *first*, then conditions actions on it via a ==hybrid attention mechanism== and ==action chunking==. **+17%** real-world and **+6%** simulation over baseline VLAs, with strong LIBERO and Franka-Tabletop OOD adaptation — the foundational stage-based recipe.
- **[[2507.16815|ThinkAct]]** — ==RL-driven visual latent planning==: a ==dual-system framework== where a slow-thinking MLLM, fine-tuned with ==action-aligned rewards==, compresses reasoning into a ==compact visual plan latent== conditioning a fast action model — bridging visual CoT with latent reasoning. **84.4%** LIBERO and **+15.5pp** SimplerEnv Google-VM over base; **48.2%** EgoPlan-Bench2 and **59.8** RoboVQA BLEU, beating GPT-4V; emergent few-shot adaptation and self-correction.
- **[[2508.07917|MolmoAct]]** — Emits ==depth-aware perception tokens== + visual reasoning traces alongside actions, giving interpretable *what-and-why* explanations. **86.6%** LIBERO average; **72.1%** SimplerEnv variant-aggregation OOD (**+7.8pp** over RT-2-X) and **+22.7pp** task progression over π0-FAST on bimanual real-world; ==visual-trace steering== reaches **75%** SR, a **+33pp** gain over natural-language steering.
...
- **[[2503.11089|EmbodiedVSR]]** — ==Dynamic scene graphs== tracking explicit spatial relations feed a ==physics-constrained chain-of-thought== for per-step geometric consistency; introduces the ==eSpatial-Benchmark== and beats GPT-4o by **+18.4%** Arm Feasibility / **+6.7%** Success Judgment on eSpatial-RoboMIND, **100%** block assembly description, **80%** real-world reassembly — earliest output-head CoT with physical-feasibility filtering.
...
- **[[2601.01618|Action-Sketcher]]** — renders spatial intent as a *sparse* ==Visual Sketch== of geometric primitives in a ==See-Think-Sketch-Act== pipeline, with an ==adaptive token-gated mechanism== switching between high-level reasoning and low-latency action. **96.0%** LIBERO-Long and SOTA on long-horizon custom tasks; ablation traces **61%** of failures to sketch-generation errors, but human-in-the-loop corrections recover **+23.0pp** ("Tidy Table").
...
- **[[2605.13632|GTA-VLA]]** — Structured ==Guide-Think-Act== reasoning with optional human spatial guidance (points, boxes, traces); an ==asynchronous "slow reasoning, fast action" design== keeps interactive control real-time. Interact-306K auto-synthesized from robot data. **98.6%** LIBERO in-domain, **+22pp** SimplerEnv-Plus unseen-object generalization, human guidance recovers **+20%** of failures (SimplerEnv-Bridge **81.2% → 86.1%**).
...
> Visual CoT shines for **multi-stage manipulation** where each stage has a visually distinct goal state ("first the cup is grasped, then it's at the lip of the kettle, then it's pouring"). For continuous skills (polishing a surface), visual subgoals are too abrupt — use latent reasoning instead (§3). Cross-reference [[05_VLA#4. Reasoning & Planning-Augmented VLAs]] for the broader reasoning-and-planning landscape that feeds into visual CoT, and [[07_WAM#5.1 Visual Chain-of-Thought]] for how WAM-integrated visual subgoal generation composes with world-model-augmented VLAs.
...
- **[[2604.22709|Abstract-CoT]]** — Replaces verbalized rationales with a short sequence of ==discrete abstract tokens== from a ==reserved vocabulary==; two-stage post-training of ==policy-iteration warm-up== + ==warm-started GRPO== with an ==attention-mask information bottleneck== forcing the answer to depend on abstract tokens, not explicit CoT. Reduces reasoning-token usage **up to 12×** with comparable or improved accuracy — token-free reasoning that preserves throughput.
...
- **[[2606.03127|TTT-VLA]]** — ==Test-time latent prompt optimization==: an implicit ==latent prompt== learned alongside the policy is updated at deployment by a ==self-supervised state-grounding proxy== (predict end-effector + gripper state) over an offline buffer while the backbone stays frozen; SimplerEnv WidowX rises **51.1% → 67.4%** single-embodiment and **22.8% → 31.6%** multi-embodiment, offline TTT more stable than online updates.
- **[[2604.18486|OneVL]]** — VLM with language + visual latent tokens supervised by ==dual auxiliary decoders== (language reconstructs CoT text; visual predicts future frames as world-model auxiliary), both ==training-time only==. At inference a ==prefill mechanism== processes all latent tokens in one parallel pass, achieving ==answer-only latency== while *exceeding* explicit autoregressive CoT. **88.84 PDM-score** on NAVSIM, **+2.64 pts** over prior 8B models, **0.24s** real-time.
...
- **[[2604.27998|Latent-GRPO]]** — Patches three GRPO-on-latent failure modes causing ==model collapse==: ==Invalid Sample Advantage Masking== (zeros advantages for non-terminating trajectories), ==One-Sided Noise Sampling== (aligns gradient with trajectory advantage), ==Optimal Correct Path First-Token Selection== (eliminates mode averaging). **+7.86 pp** Pass@1 over Latent-SFT (low-difficulty), **+14.77 pp** (high-difficulty), with **3-4× shorter chains** than explicit GRPO.
- **[[2604.20328|HyLaR]]** — Fixes hybrid discrete-text + continuous-latent ==variance mismatch== via ==Decoupled Policy Optimization (DePO)==: latent actions as ==von Mises–Fisher (vMF) distribution==, ==separate tighter clipping== for the continuous branch, ==closed-form KL== for hyperspherical geometry, plus an internal ==canvas mode== interleaving visual latents with text. On Qwen2.5-VL-7B: **+7.33%** [[2312.14135|V*]], **+14.50%** HRBench-8K, **-7.11%** HallusionBench.
...
- **[[2605.02735|Silenced Visual Latents]]** — ==Frozen-backbone, two-stage inference-time optimization==. Stage I (==Visual Latent Warm-Up==) uses ==chunk-wise contrastive alignment==; Stage II (==Latent-to-Answer Reinforcement==) applies a ==confidence-progression reward== via ==Native Evolutionary Strategy (NES)==, forcing the answer pathway to *use* the latents. On Qwen2.5-VL-7B: **+8.66%** IQTest, **+5.00%** MM-Vista; utilization scales (MMVP **72.33% → 73.67%** at K=2 → K=10).
...
> The conventional wisdom was that explicit text CoT works because it forces sequential decompose-then-act reasoning. [[2604.18486|OneVL]] falsified this: with dual-modal latent supervision, latent reasoning *outperforms* explicit CoT at answer-only latency — the most important VLA-reasoning result of 2026. But [[2605.02735|Silenced Visual Latents]] adds the essential caveat: a latent system can score well on intrinsic latent-*quality* probes while the answer head learns a shortcut that *ignores* those latents entirely. Improving latent quality does not fix utilization. Any latent-reasoning ablation must measure both — does perturbing the latents change the answer (utilization), *and* do the latents encode the right information (quality)? The two diverge. Cross-reference [[08_Latent-World-Models#4. Latent Reasoning for Embodied AI]] for the latent-substrate mechanics and [[05_VLA#1. Design-Space Principles]] for where the latent slot sits in the VLA backbone.
...
- **[[2510.16281|SEAL]]** — Three-stage pipeline targeting the ==CoT faithfulness gap==: **Hypothesize** (sample K action sequences), **Predict** (roll each through a learned dynamics model), **Verify** (an off-the-shelf VLM checks which outcome best matches the VLA's text plan). Training-free, any VLA backbone. **94-97%** in-distribution, **+15pp** (to **53%**) on novel compositions, **+17pp** under viewpoint shifts, **347ms/step** at K=10.
...
- **[[2605.13119|VLAs-as-Tools]]** — VLM emits discrete tool-invocation messages (each VLA tool a bounded sub-skill), receives continuous progress feedback, triggers event-driven replanning only when needed. ==Tool-Aligned Post-Training (TAPT)== trains the base VLA on bounded invocations with ==tool-family residual parameterization==. Latency-efficient — VLM calls drop **109.5 → 1.988** per task — while delivering **+35.5pp** RoboTwin and **+34.6pp** invocation fidelity.
...
- **[[2606.03784|ERVLA]]** — Revisits embodied CoT at scale with a **226M**-sample ==hierarchical CoT corpus==, a ==Mixture-of-Transformers== fusing a vision-language reasoning module + ==Diffusion Transformer== via "knowledge truncation," and a ==CoT-dropout== supervising reasoning in training but predicting actions directly at inference; **86.9%** avg LIBERO-Plus + **53.2%** VLABench — grounded CoT works *only* when integrated to reshape representations, not bolted on as text.
...
- **[[2407.08693|ECoT]]** — The foundational grounded-CoT-training method: trains VLAs to emit ==visually-grounded reasoning steps== (plan → sub-tasks → bounding boxes → gripper positions) *before* actions, with a ==synthetic annotation pipeline== auto-generating traces on [[2403.12945|Bridge v2]]. Built on [[2406.09246|OpenVLA]]: **+28pp** absolute SR on generalization; a single reasoning correction adds **+48pp**; async inference recovers up to **40%** of reasoning-token latency.

#### 5.3 Teacher-Guided Reasoning

Use a strong reasoning model as a teacher; distill its reasoning traces into the VLA.

- **[[2606.04436|3DThinkVLA]]** — Co-trains on action + 3D reasoning data, disentangling ==3D geometry perception== and ==spatial reasoning== into distinct ==latent== representations; an ==online 3D reasoning distillation== module transfers a teacher's explicit-3D-prompt reasoning into student action prompts via a ==shared reasoning anchor token==. SOTA **98.7%** LIBERO (**100%** Spatial/Object), **81.0%** zero-shot LIBERO-Plus, **93.3%** real Realman transparent-container placement.
- **[[2604.17800|ReFineVLA]]** — Augments robotic datasets with ==natural-language reasoning annotations== from a ==Gemini 2.0 teacher==; ==selective transfer fine-tuning== freezes SpatialVLA's lower layers while jointly optimizing ==behavioral cloning + language modeling==. **+5.0pp** over SpatialVLA on WidowX (**+21.4pp** Put Spoon on Towel), **+2.3pp / +3.5pp** visual-matching / variant-aggregation on Google Robot, **+9.6pp** Move Near.
...
> CIR/SR's finding is sobering: a VLA trained to maximize task success can develop reasoning traces that *look* correct but are causally disconnected from the final action. Step-level rewards on the *reasoning process* are required for trustworthy reasoning. Cross-reference [[07_WAM#7.3 RL-Driven & Co-Evolving]] for RL-driven WAM co-evolution patterns ([[2603.19370|VAMPO]]) and [[13_Self-Evolving-VLA-WAM#3. Core Mechanisms of Self-Evolution]] for how reasoning-traced training composes with self-evolution loops.
...
| Reasoning over force / tactile / audio | Cross-modal reasoning — research gap; see [[09_Contact-Rich-and-Whole-Body-Control#3. Force-Conditioned VLA Architectures]] for the contact-modality bridge |
...
| Need physics reasoning? | [[2503.15558\|Cosmos-Reason1]] — see [[11_Physics-Aware-Embodied-AI#5. Physics-Aware Reasoning]] |
...
- [[05_VLA]] — VLA deep-dive; §4 covers the broader Reasoning & Planning landscape and feeds into this note
- [[07_WAM]] — WAM deep-dive; §5 VLM-Integrated WAMs cover the world-model side of reasoning-augmented planning
- [[08_Latent-World-Models]] — §4 latent reasoning for embodied AI; complements this note's latent slot
- [[13_Self-Evolving-VLA-WAM]] — Self-evolution; reasoning enables self-critique and proactive correction
- [[11_Physics-Aware-Embodied-AI]] — Physics priors as the substrate for [[2503.15558|Cosmos-Reason1]] and physical latent reasoning ([[2604.28192|LaST-R1]])
- [[12_Egocentric-Pretraining-and-Human-Video]] — Egocentric pretraining deep-dive
- [[09_Contact-Rich-and-Whole-Body-Control]] — Force/tactile policies deep-dive; reasoning over multi-sensor context
- [[02_Dataset-Benchmark-Environment]] — Reasoning benchmarks ([[2507.10548|EmbRACE-3K]], [[2505.05456|SITE]])

---

*See [[05_VLA]] for the full VLA design space, [[08_Latent-World-Models]] for the broader latent-prediction landscape, or [[11_Physics-Aware-Embodied-AI]] for physics-aware reasoning.*
