...
- **[[2509.18830|DexSkin]]** — ==Conformable capacitive skin==, parallel-plate grid; **294° coverage**, 60 taxels; **1.7 kPa** sensitivity, **6.52%** hysteresis, **2.09%** drift/500 cycles. ==Pneumatic calibration== is the breakthrough: cross-instance transfer **5/20 → 14/20** SR. Downstream **19/20** perturbed pen reorient (baselines **0/20**), **90%** berry pressure cut via residual RL.
- **[[2604.28156|FlexiTac]]** — Open-source ==$30/unit FPC piezoresistive sensor==, Arduino Nano + multiplexer at **100 Hz**, layouts 8×16 to 32×32. ==Direct electrode etching== drops fabrication to ~5 min/pad; inter-electrode slots act as ==compliance hinges==. ==Kelvin-Voigt calibration== between real/sim signals enables sim-to-real RL fine-tune — rare for an open-source sensor.
- **[[2604.20689|FingerEye]]** — ==Continuous vision-tactile fingertip== (28×25.4×26 mm): ==binocular RGB cameras==, compliant soft ring, AprilTag cover. PnP-tracked AprilTag pose proxies 6D wrench: force **[4.30, 4.22, 9.93] mN**, torque **[0.32, 0.13, 8.55] mN-m**. *Continuity* — vision sees alignment *before* contact, tactile deformation *after*; **+30%** SR over wrist-camera-only.
...
- **[[2410.24090|Sparsh]]** — Foundational SSL touch encoder on **~460k** unlabeled tactile images across vision-based sensors; ViT under ==[[2111.06377|MAE]]==, ==[[2104.14294|DINO]]/[[2304.07193|DINOv2]]==, ==JEPA==; introduces ==TacBench== (6 tasks). Beats end-to-end by **~95.1%** avg; latent-SSL beats pixel reconstruction; **+20–53%** bead-maze traversal.
- **[[2506.14754|Sparsh-X]]** — Jointly encodes **4 tactile modalities** (image + audio + IMU + pressure) from Digit 360 via ==attention bottlenecks==; **~1M** unlabeled contacts; teacher-student SSL. **+17%** physical-property estimation, **+500%** plug-insertion (to **90%**) vs vision-only, **+63%** vs tactile-image-only, **90%** in-hand rotation drift cut.
...
- **[[2507.09160|Tactile-VLA]]** — ==Multi-modal transformer== fusing vision + language + ==tactile== on a ==pre-trained VLM backbone==; a ==force-aware action expert== outputs position *and* force under a ==hybrid position-force controller==. CoT recovery adjusts force **3.5N → 6.7N**: **90%** Charger, **90%** OOD fragile paper-box, **80%** zero-shot wiping.
- **[[2602.23648|FAVLA]]** — ==Fast-slow VLA==: slow VLM + high-frequency ==Force-Injected Action Expert== with force adapters across transformer layers; a VLM-predicted ==force variance head== raises execution frequency during contact. **80.8%** avg SR (**+38pp** over vision-only, **+13.8pp** over the strongest force-aware baseline); peak contact force to **7.7N** on Gear Assembly.
...
- **[[2603.15169|ForceVLA2]]** — Current SOTA. Pushes force up to the VLM via ==force-based prompts== and down to a ==Cross-Scale MoE== fusing VLM guidance with real-time F/T for ==hybrid force-position regulation==; probabilistic subtask-transition + flow-matching. **66%** avg SR over 5 tasks (**+48pp** over [[2410.24164|π0]], **+31pp** over [[2505.22159|ForceVLA]]); MoE alone **+26%**.
- **[[2505.22159|ForceVLA]]** — Canonical late fusion: ==Force-aware MoE (FVLMoE)== routes 6-axis F/T into separate experts; gating learns force-vs-visual reliance. **60.5%** avg SR (vs **37.3%** [[2410.24164|π0]]+force), **90%** under occlusion, **20%** socket insertion; full FVLMoE **80%** vs **60%** concat. Ships **ForceVLA-Data** (**244** trajectories).
...
- **[[2603.04531|PTLD]]** — ==Privileged tactile latent distillation== for sim-to-real: an ==Asymmetric Actor-Critic== trains a ==privileged-sensor policy== (external-camera pose) in *one* sim stage; a ==tactile encoder== then matches real tactile + proprioception via ==DAgger==. **+182%** in-hand rotation, **+57%** reorient goals, ~**50%** lower 6D pose error (**0.43 → 0.21** rad).
- **[[2601.20321|TaF-VLA]]** — TaF-Device collects a **>10M**-pair synchronized tactile-force corpus. ==TaF-Adapter== aligns tactile to force in a shared latent via ==VQ-VAE== + ==temporal encoding==; frozen, plugs into any VLA. **64.8%** avg SR (vs **37.1%** vision-only, **42.8%** tactile-vision-aligned); **60.3%** zero-shot on unseen sensors; **+6.7–33.3%** on ACT/Diffusion Policy.
- **[[2605.14571|MTNet]]** (+ ==AMTNet==) — Cross-domain ==visuo-tactile alignment== via dual-stream projection into a unified latent; CKA **~0.74** between modalities. **AMTNet** extends to *human* hands, aligning human visual reps to the robot manifold *without human tactile ground truth*. Structural supervision, not pressure regression (unlike [[2605.13083|TouchAnything]]).
...
- **[[2605.15157|HandITL]]** — Human takeover of a bimanual 56-DoF hand-arm VLA via VR controllers + data gloves. ==Relative hand retargeting== tracks configuration *changes* from the intervention timestamp — **99.8%** gesture-jump cut on Bread Clip; ==velocity-based shared arm control== smooths wrist residuals. Fine-tuning on on-policy intervention data beats equal-volume teleop.
- **[[2511.18960|AVA-VLA]]** — Reformulates VLA as a ==POMDP== with recurrent state encoding belief over past observations + actions. The recurrent state directly accommodates *force history* — a fit where current force is meaningless without context ("still pressing, or free space?"). The ==Active Visual Attention== module generalizes to "active force attention" (unbuilt).
- **[[2508.10333|ReconVLA]]** — ==Reconstructive learning on visual gaze regions==: a ==diffusion-transformer denoiser== on the VLA's visual tokens predicts noise on latent ==scene tokens== of the gaze region (==Grounding DINO==); **3.95** avg subtask length on CALVIN ABC→D, stack-block **59.3% → 79.5%**. The denoiser generalizes to contact regions — close to [[2603.05687|CGP]]'s denoiser.
- **[[2502.14420|ChatVLA]]** — ==Phased Alignment Training== (control first, then understanding) over ==Qwen2-VL-2B== with an MLP ==MoE== splitting a ==Control-Expert== from an ==Understanding-Expert==; recovers VQA (**71.2%** TextVQA, **9.2×** over ECoT) without sacrificing control across **25** tasks at **3.5×** fewer params. The MoE split is the parent of [[2505.22159|ForceVLA]]'s FVLMoE.
...
- **[[2505.19386|Force Prompting]]** — Adapts ==CogVideoX== via [[2302.05543|ControlNet]] to accept ==physics-based force prompts== (wind + localized pokes); trained on **15–23k** synthetic Blender + [[2404.13026|PhysDreamer]] videos. Emergent ==intuitive mass understanding==; beats text-only and trajectory baselines in human eval. *Open*: force-pretrain → force-aware action head unexecuted.
...
- **[[2605.13083|TouchAnything]] (EgoTouch)** — ==Vision-to-tactile prediction== from egocentric video alone; **20 hr** multi-view ego + bimanual 3D hand pose + dense pressure maps inferred from RGB. ==View dropout== cuts the ego-only penalty **−27.20% → −5.78%** Volumetric IoU (**+6.1%** over ego-only). First dataset bridging egocentric VLA pretraining and *dense* tactile supervision.
...
- **[[2603.19201|OmniVTA]]** — Hierarchical slow-fast visuo-tactile: ==TactileVAE== + ==Visuo-Tactile World Model== + ==Adaptive Fusion Policy== + **60 Hz** ==Reflexive Latent Tactile Controller (RLTC)==; trained on **OmniViTac** (**21,000+** trajectories, **86** tasks); SOTA on 6 real tasks; RLTC lifts perturbation SR to **60%** Wipe / **63%** Peel, caps tangential deformation at **0.35**.
...
- **[[2508.19236|MemoryVLA]]** — ==Perceptual-Cognitive Memory Bank (PCMB)== dual-memory: low-level perceptual details (recent F/T, contact events) + high-level cognitive semantics (task progress). **+26pp** over [[2503.22020|CogACT]]-Large on long-horizon temporal tasks at only **+3.6%** latency, **+0.8 GB** GPU. Not yet force-specialized, but maps cleanly onto force history.
...
- **[[2507.06905|ULC]]** — ==Single unified policy== trained with parallel ==PPO==, integrating locomotion, full 3-DoF torso, and dual-arm control via a sequential ==adaptive curriculum==; quintic interpolation + stochastic delay + CoM tracking. Widest workspace (root height **0.30–0.75 m**), low arm error (**0.06±0.01 rad** under command mutation) even under **2 kg** wrist loads.
- **[[2602.06341|HiWET]]** — ==Hierarchical world-frame end-effector tracking==: a high-level Commander reasons in the world frame, a low-level Tracker executes whole-body motion, decoupling drift-prone body-centric control. A ==Kinematic Manifold Prior== halves hand error. **12.4 mm** EE error in sim, **12–15 mm** RMSE real on Unitree G1, compensating base oscillation.
...
- **[[2511.07820|SONIC]]** — Supersizes ==motion tracking== as a foundational task: dense supervision from **100M** frames, **700 hours**, **32k GPU-hours**, **42M** params, via an ==encoder-quantizer-decoder== + generative kinematic planner. **99.6%** OOD tracking; zero-shot to physical G1 on all **50** trajectories; **95%** mobile manipulation fronted by a GR00T N1.5 VLA.
- **[[2504.11054|Meta Motivo]]** — ==Behavioral foundation model== via ==FB-CPR== (Forward-Backward reps + Conditional Policy Regularization), an online ==unsupervised RL== method learning from *unlabeled* MoCap. Zero-shot whole-body control matching task-specific TD3; evaluators prefer its naturalness over higher-reward agents — the reward-free pretraining proof for whole-body behavior.
...
- **[[2606.05880|TAGA]]** — Fuses egocentric depth, height scans, and proprioception; an emergent ==active gaze== module predicts a ==Region of Interest== in the height scan, trained with ==PPO== + Adversarial Motion Priors + a ==MoE== action decoder. **120 cm** gap on G1 (**+50%** over prior perceptive humanoids), **70 cm** stepping-stone spacing, **65.2%** lower training cost.
...
- **[[2604.01158|SMASH]]** — Humanoid table tennis from *onboard egocentric vision only*: ==dual-modality stereo perception== + ==AprilTag== self-localization, a conditional autoregressive ==Motion-VAE== augmenting sparse MoCap into a strike library, and ==task-oriented motion-matching== RL. First outdoor consecutive striking; **93.7%** contact rate, **59.7%** returns over 642 launches.
- **[[2605.14417|DAJI]]** — Learns a compact **64D** ==anticipatory joint-intent latent== encoding future transitions for a closed-loop controller. ==DAJI-Act== distills from a future-aware privileged teacher; ==DAJI-Flow== is a language-conditioned ==flow-matching DiT==. **94.42%** rollout SR on HumanML3D-style benchmarks, **4.71 ms** CPU latency, lower transition jerk.
...
- **[[2606.06139|MotionDisco]]** — Couples ==LLM-guided evolutionary search== over discrete contact-mode sequences with ==contact-explicit kinodynamic planning==: the LLM mutates Python contact-plan programs, the planner returns scores + failure feedback for the next mutation. Solves all **8** extreme long-horizon scenarios within minutes; deploys zero-shot on real hardware.
- **[[2604.00202|DreamControl-v2]]** — Trains a ==guided diffusion model directly in G1 motion space== (not human space), aggregating pre-retargeted human (AMASS/HumanML3D/GRAB) + robot (OmniRetarget) data. **68%** valid-trajectory rate (vs **8%** for inference-time prompting), FID **0.265**, **0.925** SR on complex tasks; **8** loco-manip skills on real G1.
...
- **[[2603.22201|NMR]]** — Reframes retargeting as a ==dynamic mapping between motion distributions==: a ==Transformer== maps human motion to the robot manifold, trained on a ==Clustered-Expert Physics Refinement== pipeline making consistent human-robot pairs via clustering + RL. **Zero** joint jumps, **54%** fewer self-collisions (**0.87%**); WBC policies train faster, lower pose error.
- **[[2603.03243|HoMMI]]** — Whole-body mobile manipulation from *robot-free* human demos: extends bimanual ==UMI== with an egocentric iPhone (==ARKit==), a ==cross-embodiment hand-eye Diffusion Policy== with embodiment-agnostic 3D rep + relaxed "look-at point" head action, and ==constraint-aware whole-body IK==. **90%** laundry, **85%** delivery, **80%** tablescaping on a real mobile manip.
...
- **[[2606.05160|GRAIL]]** — Digital pipeline for robot-compatible ==4D human-object-interaction== data: assemble a 3D scene, generate video via a ==Video Foundation Model==, then ==interaction-aware 4D HOI reconstruction== with contact/depth/keypoint losses. **20K+** sequences, **88.9%** sim tracking; synthetic-only policies hit **90%** real stair-climb, **84%** seen / **80%** unseen pick-up.
- **[[2605.27724|HumanoidMimicGen]]** — Adapts a small set of human whole-body skills into thousands of demos via structured ==skill planning== over a ==hierarchical hybrid action space== (RL locomotion + joint-space upper-body), with motion-noise + init randomization. **0.89** PSR across **9** tasks (vs **0.33** DexMimicGen+); sim-and-real co-training lifts real **0.51 → 0.71**.
- **[[2604.27711|ExoActor]]** — ==Exocentric video generation as control==: robot-to-human transfer → prompt decomposition → ==task-consistent video generation== (Kling 3) → ==3D whole-body + hand motion estimation== fed straight to a motion-tracking controller (no explicit retargeting). Reliable on basic and coordinated-interaction tasks; fine-grained manipulation with minor scene tweaks.
- **[[2604.11251|CLAW]]** — Generates ==language-annotated whole-body motion== by composing ==parameterized motion primitives== from a kinematic planner inside ==MuJoCo physics==, with a template engine producing **8** description styles from the same motion-intent parameters. Stitches walk→squat→crawl into long-horizon sequences directly G1-compatible, no error-prone retargeting.
...
- **[[2603.20147|AGILE]]** — Four-stage config-driven workflow (==Prepare/Train/Evaluate/Deploy==) over Isaac Lab + RSL-RL: verification GUIs, ==L2C2 regularization==, value-bootstrapped terminations, virtual harness, ==TorchScript== deployment. Sim-to-real for **5** G1/Booster-T1 skills (**6–25 hr**/task on one L40); freezing locomotion + GR00T N1.5 VLA fine-tune hits **90%** pick-and-place.
