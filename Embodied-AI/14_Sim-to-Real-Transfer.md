...
- **[[2606.02058|TIDES]]** — ==Time-derivative event simulation== from a dynamic ==4D Gaussian splatting== scene: ==visibility-consistent time-derivatives of log-luminance== via forward-mode autodiff (no finite differences) + ==risk-guided adaptive time-stepping==; best fidelity (lowest **IG-NLL** + **Chamfer**), and **models trained on TIDES events transfer best to real data** — highest video-reconstruction PSNR, lowest depth RMSE, highest segmentation mIoU.
...
- **[[2601.02078|Genie Sim 3.0]]** — Hand-crafted-but-LLM-driven humanoid simulation platform (orthogonal to learned-from-video); ==Genie Sim Generator== composes scenes from natural-language instructions over a managed asset repository; LLM-VLM auto-evaluation; ==3D Gaussian Splatting== reconstruction; **10,000+ hr** synthetic data, **100,000+** eval scenarios, **R²=0.94** sim-to-real correlation; **1,500 episodes** synthetic beats real-data baselines on zero-shot manipulation.

#### 2.2 Procedural Environment Generation

Hand-crafted physics + massive procedural scale; no learned visual model.

- **[[2603.16861|MolmoBot]]** — ==MolmoBot-Engine== procedural data generation in ==[MuJoCo](https://mujoco.org)==; **232K** indoor environments, **48K** objects, **1.8M** expert trajectories; extensive ==domain randomization== over visual + physical params enables zero-shot sim-to-real *without real fine-tuning or photoreal rendering*; **79.2%** real [Franka FR3](https://franka.de) pick-and-place — beats real-data π0.5-DROID (**39.2%**); absolute joint-position beats delta-action.
...
- **[[2605.09789|DRIS]]** — a ==structured, VRAM-efficient randomization== recipe for zero-shot dexterous *reactive catching* that yields robustness to observation noise, execution error, and OOD physical parameters at far lower VRAM than brute-force massively-parallel single-instance randomization; **68%** real flat-plate catching vs hand-crafted **5%** / end-to-end sim-trained **13%**, with emergent generalization to human-thrown and irregular objects.
...
- **[[2506.13751|LeVERB]]** — Whole-body humanoid VLA with zero-shot sim-to-real on Unitree G1; learned latent "verb" vector bridges high-level VLA (LeVERB-VL, 10 Hz, ==residual CVAE==) and low-level reactive controller (LeVERB-A, 50 Hz, ==DAgger==-distilled); LeVERB-Bench produces synthetic VL whole-body data from human MoCap; **58.5%** mean success across 10 task categories (**7.8×** over naive hierarchical VLA's **7.5%**); align a *latent intermediate*, not *actions*.
- **[[2602.23253|SPARR]]** — Sister recipe to [[2603.15956|ExpertGen]] for industrial assembly; sim base via ==PPO + dense imitation rewards== under DR; the key contribution is a **real-world residual** — base policy injects Gaussian noise to autonomously generate demos, then a ==vision-conditioned residual== via ==RLPD==, *no human supervision*; **95-100%** on 10 AutoMate tasks (**+38.4%** over AutoMate, matching [[2410.21845|HIL-SERL]]); **+74.5%** on unseen NIST.

#### 3.2 Robust RL Foundations

Treat the sim-real gap as adversarial; train against the worst-case dynamics.

- **[[2204.12581|RAMBO-RL]]** — Formulates offline RL as a ==two-player zero-sum game== — agent maximizes value, adversarial environment model minimizes it by modifying transitions; ==ensemble of neural networks== for dynamics, updated adversarially via ==Model Gradient== while staying accurate on observed data; **highest total** D4RL [MuJoCo](https://mujoco.org) locomotion score; adversarial training of the *dynamics model itself* adds conservatism without uncertainty estimation.
- **[[2510.14246|DR-RPO]]** — First provably efficient *online* policy optimization for robust MDPs with ==linear function approximation==; policy-regularized d-rectangular ==DRMDPs== / ==RRMDPs== with ==KL-divergence== to a reference policy; model-free, ==softmax policy updates==, ridge regression for robust Q-functions, ==UCB== bonuses; **Õ(d²H²/√K)** average suboptimality — matches value-based robust methods while supporting *stochastic* policies and continuous actions.
- **[[2602.13040|TCRL]]** — Defends *constrained* RL against ==temporal-coupled adversarial perturbations== (cumulative attacks evolving over time); ==worst-case-perceived cost constraint== implicitly estimates safety costs without explicit adversarial-policy modeling; ==dual-constraint defense== (autocorrelation + entropy stability) mitigates reward degradation; **559-19,077%** safety-cost reduction, **8.36-34.76%** reward improvement vs baselines under worst-case attacks.
...
- **[[2605.10063|EFGCL]]** — High-risk dynamic-motion learning (backflips, lateral flips, jumps) for legged robots via ==spotting-inspired external forces== modeled on a gymnastics spotter; ==adaptive curriculum== decays assistance by success rate; with PPO + teacher-student, acquires backflip/lateral-flip that PPO baselines *cannot learn at all*; **~2×** faster jumping, zero-shot transfer to real KLEIYN quadruped; robust to force point/magnitude/timing.
- **[[2604.24916|asRoBallet]]** — Reconfigurable humanoid ballbot via ==subtractive reconfiguration== of an open-source quadruped; critical ingredient is ==friction-aware [MuJoCo](https://mujoco.org)== modeling ==tribological phenomena== (rolling, lateral, torsional friction, omni-roller mechanics) + actuator friction; with DR, achieves **zero-shot sim-to-real**, **100%** velocity-tracking in sim, **0.05 m/s** real-world MAE; recovers from pushes up to **0.3 m**.
- **[[2604.23702|QuietWalk]]** — Physics-informed RL for low-noise humanoid locomotion under diverse footwear; ==inverse-dynamics-constrained PINN== estimates per-foot vertical GRFs from proprioception alone; frozen GRF predictor integrated *into the RL reward* to penalize impact forces; **7.17 dBA** mean-noise reduction across 4 surfaces, generalizes across **4** footwear types; PINN cuts GRF errors **82-86%** vs supervised — enabling sim-to-real of the *reward signal*.
- **[[2506.12851|KungfuBot]]** — Closes the reality gap for *highly-dynamic* humanoid skills (martial arts, dancing); ==physics-based motion processing pipeline== filters untrackable mocap and corrects contact; ==adaptive motion tracking== adjusts the reward's tracking factor during RL; with ==asymmetric actor-critic== + ==reference state initialization== + DR, zero-shot sim-to-real on Unitree G1 — per-body-position error **53.25 mm** on easy motions vs **>233 mm** for OmniH2O/ExBody2.
- **[[2508.21065|Learning on the Fly]]** — Online real-time adaptation via ==differentiable simulation==: a ==hybrid dynamics model== couples a low-fidelity analytical physics model with a learned residual net for unmodeled forces, and ==BPTT== through it gives first-order policy updates on the fly. Cuts hovering error **81%** vs L1-MPC / **55%** vs DATT under large OOD disturbances; real quadrotors adapt to added mass + wind in **3 steps** (**4.5 s**).
...
- **[[2511.15200|VIRAL]]** — Visual sim-to-real at scale for humanoid loco-manipulation; ==two-phase teacher-student== — privileged RL teacher (16 GPUs) → vision-based student via DAgger + BC (up to **64 GPUs**, ==tiled rendering==); ==camera alignment==, ==system identification==, visual+sim DR; **54/59** cycles on real Unitree G1, matching expert teleop (**20.2** vs **21.4 s**); ==Reference State Initialization== is critical — removing it drops success **95% → <10%**.
- **[[2604.11138|ViserDex]]** — Closes the visual sim-to-real gap for monocular RGB-only dexterous in-hand reorientation; ==3D Gaussian Splatting== *in the simulation loop*; ==pre-rasterization augmentation== applies structured DR to 3DGS attributes (spherical harmonics) for diverse lighting + materials; **37.6** consecutive reorientations on real Allegro Hand under nominal lighting, **~25** under adversarial; single consumer GPU, **26-90 hr**, **1.6×** faster than tiled rendering.
- **[[2604.02523|Tune to Learn]]** — Systematic study of how *controller gains* shape sim-to-real; counterintuitive finding — ==stiff gains== yield *lowest* system-id errors but *worst* transfer (amplifying modeling errors into oscillations); for BC, ==compliant overdamped gains== give higher closed-loop success despite higher training loss; for RL, any gain works given task-specific tuning; sim-fidelity metrics (low sysid error) do *not* correlate with policy transfer.
- **[[2601.02778|Force-Based Sim2Real]]** — Zero-shot force-aware dexterous manipulation on a real 5-finger hand; ==asymmetric actor-critic PPO== in ==IsaacLab==; (1) ==distance-field tactile simulation== abstracts raw tactile into compact force + contact features; (2) ==current-to-torque calibration== + ==randomized actuator model== for backlash/saturation; in-hand rotation reaches **25.1** avg consecutive rotations *with* tactile vs **1.1** *without* — contact sensing is dispositive.
...
- **[[2502.12152|HUMANUP]]** — ==Two-stage RL curriculum== for humanoid fall recovery: Stage I Discovery Policy on simplified sim, Stage II Deployable Policy on full ==URDF== with **20,000** initial postures + diverse-terrain DR + control regularization; Unitree G1 recovers from supine at **78.3%** / rolls over at **98.3%** across **6 terrains** in **~6s** (vs **11s** manufacturer); *separating motion discovery from deployable refinement* is critical — single-stage failed to converge.
...
- **[[2509.18631|Sim-Real OT Co-Training]]** — Generalist co-training from abundant sim + sparse real demos: ==Unbalanced Optimal Transport== aligns the *joint* observation-action distribution (not just marginals) across domains, with ==Dynamic-Time-Warping== temporal sampling for behaviorally-matched mini-batches. **0.73** image / **0.77** point-cloud real SR, generalizing to novel textures (BoxInBin **0.7**, Stack **0.4**) unseen in the real demos.
- **[[2503.10949|SCDA]]** — Safe *continual* domain adaptation after transfer: DR pretraining + ==PCRPO== safe RL + ==Elastic Weight Consolidation== (Fisher-weighted) lets a deployed policy keep adapting to real drift without catastrophic forgetting. On real grasping, lifts SR **20% → 60%** while holding **zero** safety violations — a generalist-across-targets recipe where reward-only adaptation turns unsafe.
- **[[2407.13771|Training-Free Model Merging MTDA]]** — Builds one generalist from many single-target adapters with *no* joint data access: per-target models trained via STDA are merged via ==linear mode connectivity== (parameter averaging + ==Gaussian BatchNorm-statistic merging==). Matches combined-data training on multi-target driving segmentation and scales with target count — privacy-preserving multi-domain generalization.
...
- **[[2511.07416|PhysWorld]]** — ==Task-conditioned video generation== → ==geometry-aligned 4D reconstruction== → physical digital twin with material estimation → ==object-centric residual RL== inside the twin; **82%** avg success across **10** real-world tasks, **+15 pp** over RIGVid; cuts grasping failures **18% → 3%** and eliminates tracking failures; the explicit physical world model is what makes generated video actionable.
- **[[2504.03597|Real-is-Sim]]** — Make the digital twin the policy's *sole interface* and have the real robot mirror it; ==Embodied Gaussians== twin sync'd to real via **60 Hz** RGB feedback; policy (==Conditional Flow Matching==) reads/writes *only* the sim; 30 real + 30 sim demos lift state-based PushT **57% → 80%**; gripper-mounted virtual cameras reach **82%**; distinct from [[2503.17973|PhysTwin]]/[[2511.07416|PhysWorld]] which keep sim and real separate.
- **[[2503.17973|PhysTwin]]** — Reconstructs ==physics-informed interactive digital twins== of *deformable* objects from videos; multi-stage optimization jointly recovers geometry, infers physical properties, models appearance via ==spring-mass models== + ==generative shape priors== + ==Gaussian splats==; real-time interactive sim drives motion planning; the physical-prior structure (springs + masses) lifts videos beyond visual reconstruction into simulatable physics.
- **[[2404.09833|Video2Game]]** — Single real-world video → real-time browser-compatible interactive 3D environment with physics; enhanced ==Instant-NGP== for large-scale unbounded scenes, distilled to ==game-engine-compatible mesh + neural texture map==, decomposed into entities with ==rigid-body physics==; **>100 FPS** browser rendering; monocular 2D priors (depth, normals, semantics) regularize NeRF for robust geometry from sparse video.

- **[[2403.03949|RialTo]]** — ==Real-to-sim-to-real pipeline== building task-specific digital twins of articulated real scenes; ==inverse distillation== transfers real imitation policies into sim for privileged state trajectories, then ==PPO+IL== + ==teacher-student distillation== with small real data; **91%** object-pose, **77%** visual distractors, **75%** physical disturbances (**2.5×** vs imitation); target-specific twins give **90%** vs generic-asset **10%** on real drawers.

#### 4.2 Differentiable Real2Sim Calibration

Replace manual system-id with gradient descent on simulator parameters.

- **[[2604.27367|DOT-Sim]]** — Differentiable optical tactile sim calibrated to real soft sensors; ==Material Point Method== models soft-gel deformation; ==differentiable physics== calibrates material params from few real demos + FEA pseudo-GT; neural ==residual image== (contact − idle) for optical fidelity; **PSNR 30.48** (**+17.34%** over baselines); zero-shot — **90.5%** indenter, **96.6%** tumor detection, **0.896 mm** trajectory error; calibration becomes gradient descent.
- **[[2508.12252|Robot Trains Robot]]** — Replaces explicit param calibration with *latent* dynamics calibration: a compliant 6-DoF arm teacher supplies ==F/T reward signals== + autonomous resets, then a three-stage pipeline (==dynamics-conditioned training== → universal latent optimization in sim → real PPO refinement of a ==FiLM-conditioned dynamics latent==) fits real dynamics. A humanoid doubles walking speed in **20 min** and learns swing-up from scratch in **15 min**.
...
- **[[2603.04029|Self-Adapting RL]]** — Online continual RL with world-model feedback; built on ==[[2301.04104|DreamerV3]]==, ==Observation== + ==Reward Prediction Residuals== exceeding threshold trigger online world-model + policy fine-tuning; selective replay excludes pre-change data; Walker adapts to actuator damage in **10,000 steps** (**2 min** sim); F1Tenth adapts to *both* sim-to-real gap *and* friction reduction within **10,000 real steps** (**8 min**).
- **[[2603.13825|Explicit-WM Manipulation]]** — Zero-shot open-world manipulation via on-demand physically-grounded digital twins; ==open-set segmentation== (Grounded-SAM) + ==grasp pose prediction== (AnyGrasp) → ==digital twin== (Hunyuan 3D 2.0) + ==two-stage pose alignment== ([[2304.07193|DINOv2]] + RANSAC/ICP) → physics sampling in [Isaac Sim](https://developer.nvidia.com/isaac-sim); **6/9** tasks at **≥75%** on a real robot; two-stage alignment lifts mug-handling **27% → 91%**.

#### 4.4 Compositional Sim-Real Environments

Use sim as a safety filter, not a training source.

- **[[2604.05484|CoEnv]]** — Multi-agent collaboration via ==compositional environment== unifying real-world scene reconstruction with a physics simulator; ==VLM-based hierarchical planner== decomposes tasks; ==collision-aware sim-to-real transfer== verifies swept collision volumes before real execution; **49%** overall success across **5** real multi-agent benchmarks with up to **3** heterogeneous robots; sim is the *safety filter*, not the training source.
...
- **[[2605.06311|VISER]]** — Pushes visual realism further; ==ray tracing== + ==physically-based rendering (PBR)== materials; ==MLLM-driven asset pipeline== generates **>1,000** 3D assets with high-fidelity PBR textures *without* baked-lighting artifacts; **r = 0.92** avg Pearson sim-real correlation; ==specular highlights== critical for cavity localization, ==contact shadows== essential for spatial grounding — pinpoints the *load-bearing visual cues* current VLAs depend on.
- **[[2511.04665|Real-to-Sim GS]]** — *Deformable-object* sim-real correlation (where [[2405.05941|SimplerEnv]] / [[2605.06311|VISER]] lose fidelity); ==3D Gaussian Splatting== + physics-informed soft-body twins ([[2503.17973|PhysTwin]]); optimizes physical params from interaction videos with ==color alignment== + **NVIDIA Warp** soft-body physics; **r > 0.9** Pearson across plush-toy packing, rope routing, T-block pushing — vs **IsaacLab r=0.649** on T-block.
- **[[2405.05941|SimplerEnv]]** — First benchmark for *reliable* sim-real correlation; sim environments replicate Google Robot + BridgeData V2; control gap closed via ==system identification== of sim controller params; visual gap via ==green screening== + ==texture baking== + aggregation across variants; Pearson **r > 0.85** Google Robot, **r = 0.890** BridgeData V2; introduces ==Mean Maximum Rank Violation (MMRV)== — whether sim correctly *ranks* policies, not just predicts success.
...
- **[[2604.10856|BridgeSim]]** — Cross-simulator closed-loop evaluation; decomposes the OL-CL gap into ==Observational Domain Shift== + ==Objective Mismatch== (biased Q-value estimation + compounding errors); training-free ==Test-Time Adaptation== combines ==flow-matching calibrator==, ==truncated Q-value estimator==, ==adaptive replan==; **+19.1** Driving Score with [[2305.14992|RAP]] in IDM mode; *scaling OL training does not improve CL* — a *paradigm* gap, not a *data* gap.
- **[[2506.18088|RoboTwin 2.0]]** — ==Automated expert data generation== via ==MLLMs== + ==closed-loop simulation-in-the-loop feedback== synthesizing/refining task code; comprehensive ==domain randomization== across **5** dimensions (clutter, textures, lighting, tabletop heights, language paraphrases) + ==embodiment-aware grasp adaptation==; policies trained on this data show **+24.4%** real-world few-shot, **+21.0%** zero-shot unseen-background generalization.
...
- **[[2604.24018|Sim2Real Betting]]** — ==Sequential betting framework== — simulators inform strategic wagers via a ==bet-weighted estimator==; ==Cover's universal portfolio== with ==Kelly-style bet sizes== combines a bank of diverse simulators; ==double betting== tolerates simulator bias when an informative edge is present; **70-100%** win rates (lower error than Monte Carlo) across synthetic + sim-to-sim locomotion; sim-to-real is *variance reduction*, not *fidelity*.

#### 5.4 Interactive World-Model Evaluation

Make heterogeneous interactive simulators comparable on the same axis.

- **[[2604.21686|WorldMark]]** — Unified benchmark for *interactive* I2V world models; ==unified action-mapping layer== translates WASD-style vocabulary to each model's native control; **500** cases across **3** difficulty tiers, **8** metrics in **3** dimensions; Spearman **ρ > 0.9** with human judgments; visual quality and world consistency are *uncorrelated* ([[2402.15391|Genie]] 3 leads consistency, YUME 1.5 quality); domain-specific models fail badly outside training domains.
