...
- **[[2509.18198|MMCD]]** — A collaborative-driving decision framework fusing RGB + LiDAR across ego and connected vehicles via attention, with ==cross-modal knowledge distillation== training a reduced-modality (RGB-only) student from a multi-modal teacher; **+20.7%** accident-detection rate for ground vehicles, **+7.0%** aerial-ground, robust to missing sensors.
...
- **[[2506.14317|ClutterDexGrasp]]** — A teacher-student dexterous-grasping system: a privileged teacher learns gentle clutter-clearance via a ==Geometry and Spatial representation== + clutter-density/interaction-safety curricula, distilled into a point-cloud ==3D Diffusion Policy (DP3)== student; **83.9%** zero-shot real success, cutting max contact force **46.3%** via the safety curriculum.
...
- **[[2607.24900|PARED]]** — Recovers an *inspectable* alignment reward for LLMs by training a lightweight ==discriminator== to separate expert demos from policy samples in a low-dimensional chosen feature space (helpfulness, harmlessness), reusing that ==expert-likeness score== for best-of-N or on-policy RL; **84.6%** win-rate vs instruction-tuned base (SFT **81.1%**), **88.4%** over SFT init.
...
- **[[2607.13818|Agentic Execution RL]]** — Learns a high-level ==agentic RL== policy regulating a frozen low-level manipulation policy at runtime, selecting `EXECUTE`/`RETRY`/`REPAIR`/`RESET` from two runtime ==execution-quality metrics==; **+5.1–13.7pp** LIBERO under standard conditions and **+25.7–39.2pp** under kinematic disturbances (OpenVLA LIBERO-Long **33.4%→67.6%**).
...
- [[15_Self-Evolving-VLA-WAM]] — Self-evolving; reward models and online adaptation here become self-improvement loops there
- [[13_Egocentric-Pretraining-and-Human-Video]] — Egocentric pretraining; the human-video scaling thread (§2.1) deepens here
- [[14_Sim-to-Real-Transfer]] — Sim-to-real; §3 Policy-Side Robustness is the transfer story for the RL policies trained here

---

*See [[04_VLA]] for how these RL recipes post-train VLA backbones, [[14_Sim-to-Real-Transfer]] for transferring the policies to hardware, or [[01_Embodied-AI-101]] to start from the basics.*
