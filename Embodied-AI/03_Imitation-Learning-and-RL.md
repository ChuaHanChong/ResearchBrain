...
- **[[2505.23708|AMOR]]** — A multi-objective ==RL== framework for adaptive character control training one policy conditioned on reward-weight vectors via ==Multi-Objective PPO==, with a hierarchical high-level policy retuning weights at runtime; approximates the Pareto front and enables post-training behavior tuning that closes the sim-to-real gap for a bipedal robot's double pirouette.
