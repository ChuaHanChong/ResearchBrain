...
- **[[2604.05323|VLA-InfoEntropy]]** — training-free vision-token selection ranking tokens by ==visual entropy== + ==attention entropy==, with ==timestep-conditioned dynamic selection== and ==KV-cache== reuse of low-information tokens; **1.53x** speedup (**−39.8%** latency, **−34.9%** FLOPs) at **76.4%** LIBERO (vs OpenVLA **75.0%**).
...
- **[[2604.11757|StarVLA-alpha]]** — ==lightweight MLP action head== on a strong ==pre-trained VLM backbone== with a ==minimal data pipeline== and a ==single generalist model== across embodiments; **98.8%** [[2306.03310|LIBERO]], **76.0%** SimplerEnv Google-VM, **33.6%** real RoboChallenge — continuous-action MLP matches complex action heads, proving complex action decoders are unnecessary.
- **[[2604.05672|A1]]** (7B) — ==Molmo-7B backbone== + Qwen3 flow-matching head with ==budget-aware adaptive inference== (==multi-exit training== + action-consistency ==early-termination==) and ==Inter-Layer Truncated Flow Matching== (10→2 steps); **72.3%** latency reduction (**37.8s → 10.5s**) at **96.4%** LIBERO, plus **75.3%** LIBERO-Plus OOD (vs OpenVLA-OFT **69.6%**).
...
- **[[2406.04339|RoboMamba]]** — Swaps quadratic Transformer attention for a ==Mamba state-space backbone== (CLIP encoder + Mamba LLM + MLP pose head); two-stage align-then-manipulate training updates only a **0.1%** (**3.7M**) policy head; **7×** faster inference than LLaMA-AdapterV2/ManipLLM, **+7.0%** seen / **+2.0%** unseen SR — the SSM-backbone point in the efficiency design space.
...
- **[[2604.04161|AAC]]** — training-free ==inference-time== chunk-size selection from predictive uncertainty: ==action entropy== (==Gaussian differential entropy== for continuous + ==discrete entropy== for gripper) sampled over parallel candidates, picking the size where the entropy jump peaks; **+15%** real-world success (**67.0% → 82.0%**), **+2.3%** RoboCasa, **+4%** LIBERO-Long.
- **[[2604.02965|SV-VLA]]** — ==speculative verification== decouples a heavy ==Macro-Planner== (long action chunks + a ==planning-context feature==) from a lightweight ==Verifier== that re-plans only when execution deviates past a threshold; **2.17x** speedup over closed-loop OpenVLA-OFT and **90.9%** LIBERO (**+11.4%** over open-loop chunking).
...
- **[[2503.02310|PD-VLA]]** — training-free ==parallel decoding== swaps the LLM's ==causal attention== for ==bidirectional attention== so all action tokens update simultaneously, iterating ==Jacobi decoding== to a fixed point; **2.52x** execution-frequency gain (**4.56 Hz** vs **1.81 Hz**) at **94.7%** LIBERO, with real-world push-button **80%** (vs **60%**) and pour-water **60%** (vs **10%**).
...
- **[[2606.03943|PointAction]]** — ==3D pointmaps as universal action representation==: a ==universal video-to-point model== predicts dynamic 3D pointmaps from embodiment-agnostic video + a lightweight ==point-to-action decoder==; **47.7%** in-distribution + **17.0%** unseen-task SR on RoboCasa365 (2–**2.5×** over baselines), **43.0%** cross-embodiment real xArm7.
- **[[2606.02274|Dexterity-BEV]]** — Integrates per-pixel 3D into multi-view RGB-D via ==aligned vertex maps + vertex spectrums== and a canonical ==Bird's-Eye-View reference frame==; **89.9%** avg on modified-pose LIBERO where 2D baselines fall **<10%**, plus **76.7%** "Fold Mailer Box" (vs X-VLA **56.7%**) and **93.3%** "Handover Book" real bimanual.
- **[[2508.09071|GeoVLA]]** — ==dual-path== architecture: frozen VLM for 2D vision-language parallel to a ==Point Embedding Network== (PEN) using an ==end-effector token== as spatial anchor; ==3D-enhanced Action Expert== (3DAE) Diffusion Transformer with static-routed MoE fuses both streams. **97.7%** [[2306.03310|LIBERO]], **77%** ManiSkill2; robust to viewpoint/scale shifts.
...
- **[[2603.25399|LaMP]]** — ==dual-expert== framework: a Motion Expert learns dense ==3D scene flow== as a latent motion prior via ==conditional flow matching==, fused into last-layer VLM features by ==gated cross-attention== to condition the Action Expert; **98.3%** [[2306.03310|LIBERO]] (**96.7%** Long), **79.3%** LIBERO-Plus OOD (**+9.7pp** over OpenVLA-OFT), **62.5%** real OOD.
- **[[2603.24393|3D-MIX]]** — plug-and-play ==VGGT-derived 3D feature fusion== with ==semantic-conditioned adaptive gating== (==GatedFusion==) blending 3D geometry with 2D MLLM semantics into GR00T-/π-style VLAs unchanged; **98.05%** LIBERO, **68.23%** SIMPLER (**+10.42%** over baseline), **+12.51%** OOD on RynnBrain-8B.
...
- **[[2601.11404|ACoT-VLA]]** — ==Action Chain-of-Thought==: an ==Explicit Action Reasoner== generates kinematic reference trajectories + an ==Implicit Action Reasoner== extracts ==latent action priors==, fused by an ==Action-Guided Prediction head==; reasoning grounded in physical coordinates. **98.5%** LIBERO (**+1.6%** SOTA), **84.1%** LIBERO-Plus OOD, **66.7%** real (vs π0.5 **61.0%**).
- **[[2503.22020|CoT-VLA]]** — ==visual CoT== first predicts a future image frame as a ==visual subgoal==, then generates actions conditioned on it, on a 7B ==VILA-U== unified backbone trained two-stage over robot demos + ==action-less video==. **+17%** real-world and **+6%** simulation over SOTA VLAs, strong on multi-instruction Franka-Tabletop tasks.
...
- **[[2505.03500|TLI]]** — ==Text Latent Interpolation==: task-specific ==text latents== are ==linearly interpolated in the residual stream== to recombine skills, on a new `libero-ood` recombination benchmark; extrapolation **9% → 83%** on OOD tasks, and re-injecting the text latent restores **11–28% → 81–94%** under blank prompts — named "spatial overfitting".
...
- **[[2512.05955|SIMPACT]]** — builds a ==multi-physics simulator== from a single RGB-D image (VLM-inferred geometry + ==physical parameters==), then a ==VLM-driven planning loop== proposes ==symbolic actions==, evaluates via ==simulated rollouts==, and refines before emitting 6-DoF trajectories; **80–90%** on 7 real tasks where π0.5 scores **0%**, **89%** sim-real agreement.
...
- **[[2604.11135|AIM]]** — jointly predicts ==Action-based Spatial Value Maps== with future RGB frames as a spatial interface; an ==intent-causal attention== forces the action branch to read the future only through those maps, with a ==self-distillation RL== stage on dense map-derived rewards. **94.0%/92.1%** RoboTwin Easy/Hard (**+15.3pp** over π0.5 hard).
...
- **[[2603.03195|CoWVLA]]** — a fine-tuned ==video VAE== ==Latent Motion Extractor== disentangles static structure from dynamic motion, and an ==autoregressive VLA Decoder== aligns continuous ==latent motion== with discrete actions (==chain-of-world reasoning==); **95.6%** LIBERO, **76.0%** SimplerEnv-WidowX, latent-motion modeling (**0.877**) beating LAPA/villa-X.
- **[[2603.29844|DIAL]]** — ==dual-system== (System-2 VLM intent / System-1 policy) with a ==differentiable latent intent bottleneck==: System-2 predicts a future-subgoal ==latent intent== via ==latent world modeling==, System-1 decodes it through ==flow matching==; **70.2%** RoboCasa GR1 (vs FLARE **55.0%**, GR00T-N1.6 **47.6%**), **10×** data efficiency (**58.3%** at 10% data).
- **[[2505.15659|FLARE]]** — ==Future Latent Representation Alignment== predicts compact future-state latents (not pixels) via an ==action-aware observation embedding== + ==diffusion transformer policy==, ==co-trained== on action-free human videos; up to **+26%** over baselines, **95%** real-world SR at just **100** trajectories/task, generalizes to novel objects from single demos.
...
- **[[2604.26694|X-WAM]]** — unified 4D world-action modeling: a ==depth adaptation module== injects 3D awareness into a ==Diffusion Transformer==, ==Asynchronous Noise Sampling== aligns noise over a ==unified denoising sequence==; **79.2%** RoboCasa (**+12.1pp** over Cosmos Policy), **+2.34 dB** PSNR, **4.5×** action-latency speedup (4665→**1033 ms**) at **15 Hz**.
...
- **[[2502.05450|ConRFT]]** — two-stage reinforced fine-tuning on a lightweight ==Consistency Policy== head: offline ==Cal-ConRFT== (==BC== + ==Cal-QL==) initializes from small inconsistent demos, online ==HIL-ConRFT== adapts with human-in-the-loop; bridges flow matching and RL. **96.3%** across 8 real tasks (**+144%** over SFT), beating HIL-SERL **31.9%**.
...
- **[[2604.05614|GPLA]]** — iterative ==preference learning (SimPO)== refines a hierarchical VLA (high-level VLM decomposer + low-level action generator) scored by an ==action-conditioned grounding model==; near-supervised trajectory quality (MSE **0.045** vs **0.043** SmolVLA), **0.98** BERTScore coherence.
- **[[2604.17706|OmniVLA-RL]]** — online VLA RL with spatial understanding: a ==Mixture-of-Transformers== with ==Spatial/Reasoning/Action experts== + ==Flow-GSPO== recasting deterministic ==flow matching== as an ==SDE== for stable exploration; **97.6%** LIBERO, **70%** LIBERO-Plus in the first **50 steps**, Flow-GSPO adding **+39.1%** absolute over SFT.
...
- **[[2505.23705|Knowledge Insulation VLA]]** — ==stop gradient== from the continuous ==action expert== into the VLM backbone, with a ==joint discrete + continuous action objective== and ==co-training== on general VL data to prevent ==catastrophic forgetting==; preserves visual representations during RL fine-tuning, converging up to **7.5×** faster than diffusion VLAs.
...
- **[[2604.07993|HEX]]** — ==hierarchical== humanoid VLA (high-level policy + ==RL whole-body controller==) with a ==Unified Proprioceptive Predictor== using a ==morphology-based MoE== + a ==review-and-forecast== visual-history cache; **79.8%** in-distribution real (vs GR00T-N1.5 **70.2%**), **61.8%** across unseen scenes (vs π0.5 **44.3%**), **100%** on early box-convey stages.
...
- **[[2601.02295|CycleVLA]]** — proactive self-correction: fine-tuned with explicit ==stop/progress signals== on LLM-decomposed subtasks, an off-the-shelf VLM does zero-shot ==failure prediction== (`transit` vs `backtrack`), and ==Minimum Bayes Risk (MBR) decoding== picks robust recovery actions. **95.3%** LIBERO (vs OpenVLA **76.5%**), MBR adding up to **+9.9%**.
