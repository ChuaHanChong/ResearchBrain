...
- **[[2604.28156|FlexiTac]]** — Fully open-source, ==$30/unit FPC piezoresistive sensor== with Arduino Nano + multiplexer at **100 Hz**; layouts 8×16 to 32×32. ==Direct electrode etching== on FPC substrate (vs hand-wiring) drops fabrication to ~5 min/pad; narrow inter-electrode slots act as ==compliance hinges==. ==Kelvin-Voigt calibration== between real/simulated tactile signals enables sim-to-real RL fine-tune — a rare open-source sensor with a documented sim-to-real story.
- **[[2604.20689|FingerEye]]** — ==Continuous vision-tactile fingertip== (28×25.4×26 mm) integrating ==binocular RGB cameras==, compliant soft ring, AprilTag acrylic cover. PnP-tracked 6D pose of AprilTag layout proxies 6D contact wrench: force **[4.30, 4.22, 9.93] mN**, torque **[0.32, 0.13, 8.55] mN-m**. Key property: *continuity* — vision sees alignment *before* contact, then tactile deformation *after*; **+30%** SR over wrist-camera-only baselines.
...
- **[[2410.24090|Sparsh]]** — Foundational SSL touch encoder family on **~460k** unlabeled tactile images across heterogeneous vision-based sensors; ViT trained under ==[[2111.06377|MAE]]==, ==[[2104.14294|DINO]]/[[2304.07193|DINOv2]]==, and ==JEPA==. Introduces ==TacBench== over 6 tactile tasks. Beats end-to-end baselines by **~95.1%** avg; latent-SSL ([[2104.14294|DINO]]/JEPA) beats pixel reconstruction ([[2111.06377|MAE]]); bead-maze policies show **+20–53%** traversal.
- **[[2506.14754|Sparsh-X]]** — Multisensory generalization: jointly encodes **4 tactile modalities** (image + audio + IMU + pressure) from Digit 360 via ==attention bottlenecks==; scales to **~1M** unlabeled contacts; teacher-student SSL distillation. Downstream: **+17%** physical-property estimation, **+500%** plug-insertion (reaching **90%**) vs vision-only, **+63%** vs tactile-image-only, **90%** in-hand rotation drift reduction — *multimodal* touch unlocks the contact frontier.
...
- **[[2603.15169|ForceVLA2]]** — Current SOTA. Pushes force awareness up to the VLM via ==force-based prompts== and below to a ==Cross-Scale MoE== that fuses VLM guidance with real-time F/T for ==closed-loop hybrid force-position regulation==. Probabilistic subtask-transition modeling + flow-matching policy: **66%** avg SR across 5 contact-rich tasks (**+48pp** over [[2410.24164|π0]], **+31pp** over [[2505.22159|ForceVLA]]); Cross-Scale MoE alone contributes **+26%**.
- **[[2505.22159|ForceVLA]]** — Canonical late fusion: ==Force-aware Mixture-of-Experts (FVLMoE)== with 6-axis F/T flowing into separate expert modules; gating learns force-vs-visual reliance — ==phase-aware action generation==. **60.5%** avg SR (vs **37.3%** for [[2410.24164|π0]] + force feedback), **90%** under partial occlusion, **20%** on unstable socket insertion; full FVLMoE **80%** vs **60%** for concatenation. Ships **ForceVLA-Data** (**244** F/T trajectories).
...
- **[[2603.04531|PTLD]]** — ==Privileged tactile latent distillation== for sim-to-real: ==Asymmetric Actor-Critic== with ==online latent distillation loss== trains a ==privileged-sensor policy== (external-camera pose) in sim in *one* stage; a ==tactile encoder== then matches real tactile + proprioception to that latent via ==DAgger==. **+182%** in-hand rotation over proprioception-only, **+57%** goals on continuous reorientation, ~**50%** lower 6D pose error (**0.43 → 0.21** rad).
- **[[2601.20321|TaF-VLA]]** — TaF-Device collected a **>10M**-pair synchronized tactile-force corpus across Vision-Based Tactile Sensors. ==TaF-Adapter== aligns tactile sequences with force in a shared latent via ==VQ-VAE== + ==temporal encoding== (static deformation vs incipient slip); frozen adapter plugs into a VLA backbone. **64.8%** avg SR (vs **37.1%** vision-only, **42.8%** tactile-vision-aligned); **60.3%** zero-shot on unseen sensors; **+6.7–33.3%** on ACT/Diffusion Policy.
- **[[2605.14571|MTNet]]** (+ ==AMTNet== companion) — Cross-domain ==visuo-tactile alignment== via dual-stream projection into a unified latent with multi-level constraints; CKA **~0.74** between modalities. **AMTNet** extends to *human* hands by aligning human visual reps to the robot's manifold *without human tactile ground truth* — responding to observed human touch from vision alone. Unlike [[2605.13083|TouchAnything]], supervision is structural, not pressure regression.
...
- **[[2605.15157|HandITL]]** — Seamless human takeover of a bimanual 56-DoF hand-arm VLA via VR controllers + data gloves. ==Optimization-based relative hand retargeting== tracks configuration *changes* from intervention timestamp — reduces "gesture jumps" by **99.8%** on Bread Clip; ==velocity-based shared arm control== smooths wrist-motion residuals. Fine-tuning the VLA on on-policy intervention data beats equivalent-volume pure teleop.
...
- **[[2603.19201|OmniVTA]]** — Hierarchical slow-fast visuo-tactile framework: ==self-supervised TactileVAE== + ==Visuo-Tactile World Model (VTWM)== + ==Adaptive Visuo-Tactile Fusion Policy (AFP)== + **60 Hz** ==Reflexive Latent Tactile Controller (RLTC)==; trained on **OmniViTac** (**21,000+** trajectories, **86** tasks, 100+ objects); SOTA on 6 real contact-rich tasks; RLTC lifts perturbation SR to **60%** Wipe / **63%** Peel, caps avg tangential deformation at **0.35**.
