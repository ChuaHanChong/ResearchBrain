...
- **[[2009.12293|robosuite]]** — Modular [MuJoCo](https://mujoco.org)-based framework with separate ==Modeling/Simulation APIs==, **10** robot + **9** gripper models, ==variable-impedance control==, and **9** single-arm/bimanual/mobile benchmark tasks; SAC solves **3/9** environments and OSC-POSE control learns faster than joint-velocity — the reproducibility substrate underneath [[2406.02523|RoboCasa]] / [[2506.18088|RoboTwin 2.0]].
...
- **[[2504.18904|RoboVerse]]** — Unified platform via ==METASIM infrastructure== abstracting diverse simulators (==MuJoCo==, ==PhysX==, ==Isaac Sim==, ==SAPIEN==, ==CoppeliaSim==) into a universal interface + ==multi-source synthetic data pipeline== (rollouts + motion planning + teleop + AI-assisted task gen); standardizes IL/RL benchmarks with cross-simulator integration and proven sim-to-real transfer — the closest the field has to a simulator-stack abstraction layer.
...
- **[[2506.10966|GenManip]]** — Isaac-Sim tabletop platform with **10,000** annotated 3D objects + **100** articulated objects and an ==LLM-driven Task-oriented Scene Graph== auto-generating instruction-following scenarios; on GenManip-Bench, modular systems reach only **23.0%** SR (and **9.07%** long-horizon) while end-to-end GR-1 hits **95.0%** familiar but **0.0%** novel — a generalization stress-test sim.
...
- **[[2604.25459|GS-Playground]]** — High-throughput simulator pairing a parallel physics engine with a memory-efficient ==batch 3D Gaussian Splatting== renderer (==point-pruning==) + ==Rigid-Link Gaussian Kinematics==; **10,000 FPS** at 640×480 and **32×** over MuJoCo for 50 humanoids; zero-shot sim-to-real manipulation at **90%** real SR — photorealism without the ray-tracing throughput penalty.
...
- **[[2604.15805|WorldComposer]]** — Generates a photorealistic ==Digital Twin== (==3D Gaussian Splats== + collision mesh) plus diverse ==Digital Cousins== from a single panorama via ==prompt-driven semantic/geometric editing== + ==multi-room ICP stitching==; **r=0.91** sim-real correlation, up to **85%** real SR augmenting 50 real with 1,000 sim trajectories — twin-plus-cousins beats single-twin for generalization.
...
- **[[2307.10224|RL-ViGen]]** — visual-RL ==generalization benchmark==: **5** task categories × **5** OOD axes (appearance, lighting, camera, scene structure, cross-embodiment) under one ==unified framework== over 8 algorithms; all collapse on novel scene structures, while PIE-G (ImageNet priors) leads on appearance/lighting and SGQN on camera-view — exposes which OOD axis each algorithm fails.
...
- **[[2603.04639|RoboMME]]** — ==Cognitively-motivated memory benchmark== over **16** long-horizon tasks split into ==temporal / spatial / object / procedural memory== with **1,600** demos / **770K** timesteps on ManiSkill; **14** memory-augmented π0.5 variants — perceptual memory wins overall at **44.51%** SR, oracle-symbolic reaches **84.08%** vs **90.5%** human, decomposing *which* memory type each policy lacks.
...
- **[[2602.22663|CEBench]]** — ==Cross-embodiment practicality benchmark== (single-arm / bimanual / mobile-bimanual, sim + real, ==domain randomization==) paired with ==LLaVA-VLA== (0.5B, pre-training-free, ==hybrid direction+value action space==); LLaVA-VLA hits **50.6%** on CALVIN 5/5 beating OpenVLA-7B, **30.7%** real DR bimanual, first end-to-end VLA to do real mobile manipulation — disentangles practicality from raw scale.
...
- **[[2603.22435|CaP-X]]** — Open ==Code-as-Policy benchmark== (==CaP-Gym== + ==CaP-Bench==) probing how agent performance scales with primitive abstraction; ==CaP-Agent0== (multi-turn visual differencing + skill libraries) hits human-level on **4/7** tasks and out-robustifies trained VLAs on LIBERO-PRO, while ==CaP-RL== with verifiable rewards lifts Cube-Lift **25% → 80%** — isolates agent reasoning from embedded human priors.
...
- **[[2509.18953|Eva-VLA]]** — ==Gradient-free black-box robustness search== (CMA-ES over ==3D rotations==, ==point-light illumination==, ==natural adversarial patches==) with a cosine-action-similarity objective; drives OpenVLA/UniVLA failure rates from **4.0–23.5% → >80%**, and its adversarial examples retrain π0.5 to **85.8% → 56.8%** 3D-transform failure — physical variations as a continuous optimization problem.
...
- **[[2505.18472|ManiFeel]]** — First comprehensive visuotactile sim benchmark in IsaacGym: **13** tasks (insertion, screwing, exploration) with simulated ==GelSight R1.5== (TacRGB + TacFF) sensors validated against real data; modular pipeline benchmarks pre-trained encoders (UniT, T3, AnyTouch) + policies (DP, Flow Matching); **+26pp** TacFF on peg-insertion, **+14pp** TacFF on real gear-assembly sim-to-real consistency check — the standardized "does touch help" probe.
- **[[2408.06506|TacSL]]** — GPU-accelerated visuotactile sim integrated into Isaac with ==implicit Kelvin-Voigt soft contact== + ==Asymmetric Actor-Critic Distillation (AACD)== for high-dim tactile policy learning; **200×** speedup tactile-image gen (**1631 FPS**), **428×** force-field gen (**1.5M FPS**); zero-shot sim-to-real **91.4%** peg-place / **82.7%** peg-insert on real robot; image-augmented policy lifts real SR **27.2 → 87.7%** — the de-facto tactile-sim foundation.
...
- **[[2503.05652|BRS]]** (BEHAVIOR Robot Suite) — Integrated whole-body household manipulation framework around the ==Galaxea R1== wheeled dual-arm + ==4-DoF torso==; ==JoyLo== Joy-Con teleoperation + ==WB-VIMA== autoregressive whole-body decoding with multi-modal observation attention; **88%** avg sub-task SR / **58%** entire-task across **5** challenging household tasks at near-zero safety-violation rate; **13×**–**21×** over DP3/RGB-DP baselines.
- **[[2502.12152|HUMANUP]]** — First learned humanoid fall-recovery policy via ==two-stage RL curriculum== (Discovery Policy in simplified sim → Deployable Policy with full URDF + **20,000** initial postures); Unitree G1 recovers from supine at **78.3%** SR / rolls over at **98.3%** across **6 terrains** (concrete to snow) in **~6s** (vs **11s** manufacturer); the canonical reference for whole-body fall recovery.
...
- **[[2401.02117|Mobile ALOHA]]** — Whole-body teleop + co-training over static-ALOHA data; **16 DoF** mobile bimanual platform with ==whole-body teleoperation== (operator's waist tethered to base) + ==behavior cloning + co-training==; **>90%** SR on **7** real tasks (cook shrimp / wipe wine / call elevator); static-data co-training lifts wipe-wine **20→40%** at 25 demos, **50→95%** at 50 — established the mobile-bimanual reference platform.
...
- **[[2410.24185|DexMimicGen]]** — Automated bimanual data-gen by replaying few human demos in sim via ==SE(3)-equivariant subtask decomposition== with per-arm async + sync schemes + ordering constraints; **76.0%** vs **0.7%** Drawer Cleanup, **80.7%** vs **3.3%** Piece Assembly; **90%** real GR-1 humanoid success on can-sorting from 40 sim demos vs **0%** with 4 source demos — the bimanual extension of MimicGen.
...
- **[[2602.21992|PanoEnv]]** — ==Geometry-grounded panoramic (ERP) spatial benchmark== (**14.8K** QA from TartanAir 3D ground truth) + ==PanoEnv-RL== (GRPO with geometry-aware rewards + two-stage curriculum); lifts Qwen2.5-VL-7B open-ended accuracy **6.39% → 14.83%** (**+132%** relative) and transfers zero-shot to real OSR-Bench beating the 72B baseline — exposes 360° distortion as an unsolved spatial axis.
- **[[2412.07755|SAT]]** — ==Spatial Aptitude Training== data engine using ==3D simulators== to procedurally generate static + **5-category dynamic** spatial QA (egocentric/object movement, allocentric, goal-aiming, action-consequence); SAT-tuning LLaVA-1.5-13B improves CVBench/BLINK **+23.9%** to **75.6%** and transfers to real images **+13.3%** — sim-generated dynamic spatial supervision with sim-to-real gain.
...
- **[[2603.13033|ESPIRE]]** — Diagnostic benchmark in ==Isaac Sim== decomposing robotic tasks into ==3D localization== (2D pixel coords) and ==6-DoF execution== (goal poses). Hierarchical framework probes spatial *aspects* × reference frames × reference objects. VLMs are much stronger at localization than execution — they lack robust 3D rotational geometry; reflection feedback *improves* localization but *degrades* execution.
- **[[2503.11117|EXPRESS-Bench]]** — Embodied Question Answering benchmark in ==Habitat + HM3D== with **777** trajectories / **2,044** QA pairs and the novel ==Exploration-Answer Consistency (EAC)== metric. Companion model **Fine-EQA** combines ==frontier-based + goal-oriented exploration== (**40.55%** C, **16.22%** E_path; **+42%** path-length reduction on HM-EQA). Punishes models that hallucinate answers without observing the relevant scene.
- **[[2602.21015|CHAIN]]** — Interactive 3D physics-driven benchmark with **109 levels** across ==interlocking mechanical puzzles== + ==3D stacking/packing== (Unity + lightweight Python engine); ==closed-loop action-and-feedback== evaluation. Best VLM (GPT-5.2) hits only **22.9%** Pass@1; puzzle tasks remain near-zero even at "easy". Also exposes video-WM physics violations. Cross-listed in [[11_Physics-Aware-Embodied-AI#6. Physics Commonsense Benchmarks]].
...
- **[[2603.25887|WR-Arena]]** — ==World Reasoning Arena== scoring WMs on Action-Simulation Fidelity / Long-horizon Forecast / Simulative-Reasoning-&-Planning via ==LLM action-proposers + VLM judges==; MiniMax leads fidelity (**72.33%** agent / **51.67%** env), a PAN+VLM planner adds **+26.33%** open-ended / **+23.40%** structured trajectory success, and no model exceeds **65%** long-horizon consistency.
...
- **[[2601.07823|Video Generation in Robotics Survey]]** — Systematic review positioning ==diffusion / flow-matching video models== as embodied world models across **4** application areas (IL data-gen, RL dynamics/rewards, policy evaluation, visual planning) + **10** enumerated challenges (hallucination, physics violation, uncertainty, long-video cost); the map of the WM-as-evaluator + WM-as-environment literature.
...
- **[[2505.09694|EWMBench]]** — First embodied-WM generation benchmark on ==AgiBot-World== scoring ==Visual Scene Consistency (DINOv2)== + ==Motion Correctness (YOLO-World HSD/NDTW/DYN)== + ==Semantic Alignment==; domain-adapted models beat general video generators, with motion correctness the hardest axis and strong human-judgment correlation — separates "good embodied WM" from "good video model".
