...
- **[[2508.08269|emg2tendon]]** — The first large-scale sEMG→tendon-control corpus, deriving **39** tendon signals from emg2pose hand poses via the ==MyoSuite MyoHand== musculoskeletal model, decoded by a ==Conditional Latent Diffusion Model==; **0.201** RMSE, and the two-step sEMG→tendon→==physics-simulated== pose path hits **11.9° ± 1.3°** angular error.
...
- **[[2501.09782|SMPLest-X]]** — A **40-dataset** EHPS benchmark ranking data by ==Mean Primary Error==, paired with **SynHand** synthetic close-up hand data and a decoder-only architecture with ==wrist-consistency loss==; scaling to ViT-Huge cuts whole-body error **110mm→58.9mm**, sets AGORA SOTA at **96.2mm** NMVE.
...
- **[[2503.09320|2HandedAfforder]]** — Extracts precise bimanual affordance masks from egocentric video via ==hand-inpainting + SAM2 mask-completion==, building the **278K**-image ==2HANDS== dataset + **ActAffordance** benchmark (EPIC-KITCHENS/Ego4D); beats LISA/AffordanceLLM (mAP **0.104**), drives real Tiago++ bimanual grasps.
- **[[2407.12371|HIMO]]** — A **3.3K**-sequence / **4.08M**-frame full-body MoCap dataset of humans manipulating multiple household objects (Optitrack + inertial gloves) with temporal-segment text; paired ==dual-branch diffusion (HIMO-Gen)== hits FID **1.48** (vs MDM's **6.85**) — multi-object HOI beyond single-object GRAB/BEHAVE/ARCTIC.
- **[[2406.19353|CORE4D]]** — A 4D human-object-human rearrangement dataset: **1K** real hybrid inertial-optical mocap sequences + **10K** ==DeepSDF-retargeted== synthetic sequences across **3K** object shapes; the synthetic split lifts humanoid box-lifting SR **0.0%→26.5%** — retargeting substitutes for costly multi-object mocap.
- **[[2406.06843|HO-Cap]]** — A markerless hand-object capture system (8 RGB-D cams + HoloLens) chaining ==BundleSDF reconstruction== + ==FoundationPose== + ==MANO/MediaPipe== fitting with no per-object training; **64** videos / **656K** frames at sub-**5px** reprojection error, directly usable as robot-manipulation demonstrations.
...
- **[[2204.06950|BEHAVE]]** — A multi-view RGBD human-object-interaction dataset of **~15.2K** frames (4 Kinects / **8** subjects / **20** objects / **5** natural locations) with pseudo-GT SMPL + object meshes + annotated contacts; the paired ==unsigned-distance-field + SMPL-correspondence== joint fit reaches **4.99 cm** vertex error vs PHOSA's **13.73 cm**.
...
- **[[2201.02610|MANO]]** — The parametric hand model underlying most hand-object corpora above, learned from **1,000+** high-resolution scans of **31** subjects via ==linear blend skinning== + ==pose-dependent corrective blend shapes== + a ==PCA pose embedding== (**6** components explain **81%** variance); fused with SMPL into ==SMPL+H==.
...
- **[[2506.23152|DexH2R]]** — The first real-world dynamic human-to-robot dexterous-handover dataset (**4,282** trials / **79,911** grasp poses) + a three-stage ==DynamicGrasp== solution (grasp-pose prep → ==diffusion-policy== approach → goal alignment); grasp models hit **89.38%** stability, **DP3** reaches **66.3%** SR / **50.1%** safety in Easy Mode.
...
#### 2.8 Automated Interaction & Affordance Annotation Pipelines

VLM+physics-verified pipelines that scale grasp *and* broader skill-conditioned interaction labels across large asset libraries, distinct from the grasp-pose-only corpora above.

- **[[2606.17446|AnnotateAnything]]** — A hierarchical VLM+physics pipeline (==3D part segmentation== + ==energy-based== trajectory optimization + ==CUDA-parallel validation==) annotating **18** skills across **17,005** assets into **100M** validated interaction candidates; rollout success reaches **86.0%** vs **24.0%** unannotated, **18/20** real zero-shot grasp transfer.
- **[[2603.16866|ManiTwin-100K]]** — A physics-verified asset+grasp pipeline: ==image-to-3D generation== with a VLM quality gate, ==FPS+VLM functional/grasp labeling==, and ==SAPIEN/PhysX== grasp-close/lift/slide verification; spans **512** categories with over **5M** verified grasps and **10M** trajectories, **100%** human-rated category accuracy.
...
- **[[2509.23328|SRB]]** — An open-source ==Isaac Lab== space-robotics framework pairing ==Blender-backed procedural content generation== with per-episode ==domain randomization== across **5** gravity domains and **15+** tasks; zero-shot DreamerV3 rover transfer hit **2.3–3.6 cm** vs **11.2–14.8 cm** for PPO/TD3 — masters diversity rather than chasing single-scene fidelity.
- **[[2507.21981|DISCOVERSE]]** — An open-source ==3DGS==-based sim framework fusing a hyper-realistic neural renderer with ==MuJoCo== physics + ROS2, via a hierarchical ==Real2Sim== pipeline (laser scanning + dual 3DGS-Mesh + ==PBR relighting==) and generative ==domain randomization==; up to **86.5%** zero-shot Sim2Real SR, **~100×** faster data collection than real-world.
...
- **[[2412.05789|InfiniteWorld]]** — A unified ==Isaac Sim== framework fusing ==generation-driven asset construction== + ==depth-prior-constrained Real2Sim Gaussian-splatting== into **10K** scenes (scalable to **2.36M**) across four benchmarks (loco-nav/manip, scene-graph, social manip); GPT-4o hits **90.82%** loco-nav SR, zero-shot VLMs collapse to **≤0.06%**.
- **[[2301.04195|ORBIT]]** — A modular ==Isaac Sim== robot-learning framework (later renamed [[2511.04831|Isaac-Lab]]) pairing GPU ==PhysX 5.1== rigid + FEM deformable physics with ray-traced sensing under a ==World/Agent== abstraction; **16** robots / **20+** tasks at ~**10×** rigid throughput where CPU frameworks crash at **200–300** envs.
...
- **[[2603.04356|RoboCasa365]]** — Scales ==RoboCasa== to **2,500** kitchen scenes / **365** mobile-manipulation tasks (65 atomic, 300 composite) with **2,000+ hours** of ==MimicGen==-augmented data; foundation-model pretraining gives a **3×** data-efficiency gain, sim-and-real co-training lifts real-world SR **61.8%→79.8%**.
- **[[2508.07770|AgentWorld]]** — A household sim platform pairing a ==four-stage procedural scene-generation== pipeline (Isaac Sim + Unreal Engine, PBR materials) with ==mobile-based teleoperation== spanning wheeled/legged bases + grippers/dexterous hands; paired **1,000+**-trajectory dataset gives π0 **30%** SR multistage + **29.3%** sim-to-real.
...
- **[[2511.20620|Wanderland]]** — A real-to-sim framework for open-world urban embodied AI using a multi-sensor (LiDAR-IMU-GNSS-Camera) scanner + ==LIV-SLAM== to seed metric-accurate ==3DGS== reconstruction plus an occupancy-accurate ==collision mesh==; beats video-3DGS baselines on PSNR/SSIM/LPIPS, RL fine-tuned in it improves while video-3DGS environments degrade.
...
#### 3.4 Driving & Urban Traffic Simulators

The vehicle-scale branch of the simulator matrix (the "driving / wheeled robots" row of §3.9). Each buys a different realism axis: rendered sensor fidelity, traffic-agent behavior, or accelerator-native throughput.

- **[[2310.08710|Waymax]]** — A ==JAX/XLA functionally pure== multi-agent driving simulator seeded from Waymo Open Motion Dataset logs, ==Brax==-style stateless reset/step so rollouts jit-compile, batch, and differentiate; **>5000 Hz** per-example, and a DQN trained against ==IDM== agents collides **4×** more against log playback — reactive agents flatter eval.
- **[[1911.04074|SUMMIT]]** — Extends [[1711.03938|CARLA]] to any ==OpenStreetMap== location, auto-generating dense unregulated mixed traffic via ==Context-GAMMA== velocity-space optimization with road-context half-plane constraints plus a ==Context-POMDP== expert planner; **28.4 Hz** at **150** agents, **0.00069** collisions/step at **4.53 m/s**.
- **[[1711.03938|CARLA]]** — The foundational open urban driving simulator: a ==server-client== layer over Unreal Engine 4 with configurable RGB/depth/segmentation rigs, **18** weather-illumination presets, and a **4-task** navigation ==benchmark protocol==; modular-pipeline SR falls **80%→24%** on the held-out town, imitation **86%→40%**.
...
- **[[2608.13095|SRF]]** — Lifts ==SAM 3== segmentations into a ==Nerfacto== field via **C** independent ==sigmoid semantic heads==, exposing Render/Semantic/Occupancy queries distilled into a ==MuJoCo==-closing ==occupancy cache==; an orchard apple-reaching demo uses one field as renderer, label oracle, and collision detector.
...
- **[[2603.15612|HSImul3R]]** — A ==physics-in-the-loop bi-directional optimization== pipeline reconstructing simulation-ready human-scene interactions, using ==scene-targeted RL== to refine human motion + ==Direct Simulation Reward Optimization (DSRO)== to correct object geometry; **53.68%** stability (Easy) vs **10.52%** HSfM baseline, refined motions retarget to a real Unitree G1.
- **[[2601.06997|ObjSplat]]** — An autonomous object-digitization pipeline pairing ==Gaussian surfel reconstruction== with a ==geometry-aware uncertainty map== (back-face + opacity completeness) and a ==prize-collecting-TSP next-best-path planner==; **32.35 dB** PSNR / **91.42%** coverage on GSO objects (vs **30.83 dB** / **72.66%** GauSS-MI) at **4x** shorter capture paths.
...
- **[[2503.09040|MBGS]]** — Decouples dense ==3D Gaussians== from explicit ==motion graphs== (kinematic trees + deformable graphs) via a learnable ==weight-painting== function, optimized end-to-end from real video; **0.37** LPIPS on iPhone (beats Shape-of-Motion), enables robot-demo synthesis from human video + **46 FPS** visual planning.
...
- **[[2606.06569|PhyRoGen]]** — A procedural TAMP-puzzle generator that iteratively grows an interlocking kinematic chain under a ==two-stage RRT-Connect solvability check==, exporting **URDF** via Blender/Phobos; produces **24** unique deterministic puzzles across **6** generators where BIT* solves **100%** but RRT*/LBT-RRT solve only **6-7/24**.
...
- **[[2512.01773|IGen]]** — A ==simulation-free rendering== engine turning a single in-the-wild image into VLA training data via ==monocular-depth+DINOv2-keypoint== reconstruction plus a ==VLM spatial planner==; **27.00** PSNR (vs **17.26** Simpler), **30-200x** cheaper than video, and **1,000**-sample fine-tuning hits **66.7%** real-world SR vs **58.3%** teleop.
- **[[2505.04831|Steerable Scene Generation]]** — A ==diffusion==-based SE(3) scene generator trained on **44M** procedural scenes, steered via ==RL-based post-training== + text/inpainting conditioning + inference-time ==Monte Carlo Tree Search (MCTS)==, with physics post-processing for feasibility; MCTS packs **34** objects vs a **17.1** training mean.
...
| Driving / wheeled robots | [[1711.03938\|CARLA]], [NVIDIA Drive Sim](https://developer.nvidia.com/drive/simulation) |
...
- **[[2605.30313|UniLab]]** — The counterpoint to GPU-resident physics: a ==heterogeneous CPU-simulation / GPU-learning== architecture whose ==unified runtime== coordinates buffering and parameter sync across ==PPO/APPO/FastSAC/FlashSAC==; **3–10×** faster wall-clock (G1 Flip **32.9 min**, **3.3×** over MjLab PPO), running on macOS / ROCm / Intel XPU off CUDA.
...
- **[[2506.16012|DualTHOR]]** — A dual-arm humanoid ==Unity== simulator (AI2-THOR + H1/X1 assets) with a ==probabilistic contingency mechanism== (state-conditioned success/broken/spill/stuck outcomes) over **356** dual-arm-vs-single-arm tasks; best planner drops **36.54%→11.69%** as skill-failure rate rises — decouples planning from execution reliability.
...
- **[[2608.04574|SpatialSTALE]]** — A dynamic FrozenLake testbed isolating ==spatial memory staleness== (claims still trusted after going stale) across NoMemory/NoFilter/SelfVerify/==OMCD== auditing; unfiltered stale memory causes a **74.4%** death rate vs **28.0%** with none, ==OMCD== cuts it to **21.6%** — the field's first diagnostic separating staleness detection from raw memory capacity.
- **[[2607.21571|Sequential-EQA]]** — A ==sequential-evaluation protocol== where memory persists across consecutive questions (not reset per-episode), adapting four EQA paradigms unretrained, validated on a real quadruped; only structured 3D memory ([[2411.17735|3D-Mem]]) gains (**+33.3%** accuracy), other paradigms stay near-flat (**<3%**) — most memory evaporates once questions stop resetting.
- **[[2605.29341|WorldMemArena]]** — A multimodal agent-memory benchmark casting memory as an ==Action–World Interaction Loop== and scoring a ==four-stage lifecycle== (write / consolidate / retrieve / use) over **461** multi-session tasks; retrieval is the bottleneck and text memory beats multimodal — isolates *which* stage fails, not just whether recall happened.
...
- **[[2608.05715|Robot-Prompt-Hijack]]** — A ==physical prompt-injection== study placing printed paper notes in the workspace of VLM-controlled manipulators under a **20**-attack taxonomy (Indirect Signage / Task Redefinition / ==Authority Impersonation== / Conflict Injection); **27.0–29.4%** ASR on GPT-4o and Gemini 2.5 Flash, ==OCR text masking== **100%** effective.
...
- **[[1612.03801|DMLab]]** — The founding first-person 3D game-world RL platform the suites above descend from: a lock-stepped ==RL API== over the ioquake3 engine exposing RGBD + velocity, with a ==Lua level API== and ASCII text levels for procedural mazes; **996.6 fps** GPU vs **199.7 fps** CPU at 84×84.
...
- **[[2502.09762|AT-Drone]]** — An ==adaptive-teaming== benchmark probing generalization to *unseen teammates* rather than disturbance: a customizable multi-drone-pursuit sim configurator + real Crazyflie deployment pipeline + an algorithm zoo (HOLA-Drone V2, NAHT-D) scored on standardized "unseen drone zoos"; ablating NAHT-D's teammate-modeling decoder sometimes *helps*.
- **[[2406.08406|RRLS]]** — A Gymnasium/==MuJoCo== robust-RL suite of **6** control tasks with 1D/2D/3D ==uncertainty sets== over physics parameters + ==RARL==-style force perturbations, domain-randomization / adversarial wrappers, and a mesh-based eval protocol; **M2TD3** leads worst-case (**1.0** vs DR **0.52**) while DR leads average-case (**1.68**).
...
| Spatial memory staleness (stored claim still true?) | [[2608.04574\|SpatialSTALE]] |
| Memory persistence across consecutive questions, not per-episode reset | [[2607.21571\|Sequential-EQA]] |
...
- **[[2406.05756|EmbSpatial-Bench]]** — An ==egocentric== spatial-relation benchmark (above/below/left/right/close/far) auto-built from Matterport3D + AI2-THOR + ScanNet 3D scenes, paired with the ==EmbSpatial-SFT== tuning set; Qwen-VL-Max hits **49.11%** vs **90.33%** human, and SFT lifts MiniGPT-v2 **+34.25%** to **78.10%** — depth relations weakest.
...
- **[[2608.05747|GST-Bench]]** — A ==global== spatial-awareness video-VQA benchmark of **12** subtasks (self-localization / object localization / scene structure) enforcing ==cross-frame integration== with fine-grained numeric answers, plus an OmniGibson-generated ==GST-Train== set; Gemini-3-Pro **42.68** vs **79.08** human, GST-Train lifts Qwen3-VL-8B **25.89→53.52**.
...
- **[[2505.24257|DISJOINT-3DQA]]** — An egocentric benchmark (**5,399** QA on Aria Synthetic Environments) with a ==strict disjointness constraint== so object pairs are never co-visible, plus a ==2D-only → sparse-3D → oracle-3D== context ladder; GPT-4o **65.60%** vs **93.96%** human, oracle 3D lifting it to **83.2%** — the bottleneck is 3D construction, not reasoning.
...
- **[[2604.16788|LongBench]]** — A real-world long-horizon benchmark of **1,000+** episodes / **10** tasks (ARX-R5 arm), splitting difficulty into ==Context-Independent== (execution) vs ==Context-Dependent== (ambiguity) regimes with ==stage-wise scoring==; π0 leads execution (**86.3%**) but collapses under ambiguity (**37.3%**) — no policy masters both axes.
...
- **[[2608.13552|PlayWorld-Bench]]** — An ==agent-driven closed-loop== WAM benchmark (Claude Haiku 4.5 adaptively extends/corrects a human action sequence) plus a ==VQA rubric verifier== tracking **600** human pairwise judgments at Spearman **0.933**; Genie 3 leads overall at **2.12/5**, Preset+Agent control beats either alone while touching only **12-15%** of actions.
- **[[2608.09298|WorldSimProbe]]** — An ==Observable Simulator Contract== formalizing ==action-realization== + ==interaction-response consistency== across **5** suites (calibration, trajectory coverage, interaction grounding, **8**-primitive dynamics); six WAMs score **35.6-63.5** overall, grounding falls **75.0→38.6** under false contact, shake dynamics collapse to **0.0-1.2**.
...
- **[[2412.05337|ACT-Bench]]** — An open action-fidelity benchmark for driving world models: **2,286** nuScenes video-trajectory pairs scored by a learned ==ACT-ESTIMATOR== (**94.03%** action-classification accuracy); baseline ==TERRA== (GAIA-1-style per-frame conditioning) beats Vista **44.11%** vs **30.72%** instruction-match — both leave large gaps.
...
- **[[2502.09829|Cost-Aware Active Testing]]** — A ==cost-aware active experiment-selection== framework treating multi-task policy evaluation as ==population parameter estimation== via a language-embedding ==surrogate model== + ==Expected Information Gain (EIG)== acquisition; EIG-based selection achieves significantly lower **L1 error** than random sampling — cuts physical-trial overhead.
...
- **[[2604.26509|3D-Generation-for-Embodied-AI-Survey]]** — A survey of 3D content generation for robotic simulation under a ==simulation-readiness== criterion (geometric validity, physical parameterization, kinematic executability, URDF/MJCF compatibility), taxonomized as ==Data Generator== / ==Simulation Environments== / ==Sim2Real Bridge==.
