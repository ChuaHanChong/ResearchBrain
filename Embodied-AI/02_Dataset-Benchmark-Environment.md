...
- [[2403.12945|DROID]] — In-the-wild data across 16 institutions; **20%** SR improvement; proved environmental diversity beats curation.
...
- [[2307.00595|RH20T]] — Comprehensive multi-task robotic dataset for one-shot skill learning; complements OXE on the diverse-skill axis.
...
**Bimanual Manipulation** — Coordinated two-arm control requires specialized data with synchronized dual-arm timing, tactile feedback, and contact-rich task variety.
...
- [[2603.05687|CGP]] — Contact-Grounded Policy; dexterous bimanual visuotactile evaluation with generative contact grounding on insertion and assembly.
- [[2512.24653|RoboMIND 2.0]] — 310K bimanual + mobile manipulation trajectories with tactile sensing and digital twin.
- [[2511.17441|RoboCOIN]] — Bimanual coordination dataset emphasizing timing-sensitive dual-arm trajectories.
- [[2412.13877|RoboMIND]] — Bimanual + mobile manipulation precursor to RoboMIND 2.0.
...
**Single-Embodiment High-Quality** — Depth over breadth: consistent data from one robot in diverse environments. The case for *fixed-deployment-robot* specialists.
- [[2509.00576|G0]] — Single-embodiment in-domain data quality outperforms heterogeneous cross-embodiment scale when the deployment robot is fixed; invest in diverse *scenes*, not diverse robots.
...
**Teleoperation Hardware & Hand-Held Data Collection** — The data collection systems themselves. [[2402.10329|UMI]] introduced the "in-the-wild" hand-held paradigm that eliminates teleoperation entirely; subsequent work has extended it to dexterous hands and humanoid whole-body manipulation.
...
- [[2309.13037|GELLO]] — Low-cost (≈$100) teleoperation device for general-purpose robot arm control; pose-mirroring leader-follower design.
- [[2304.13705|ALOHA]] — Low-cost bimanual teleoperation; the original bimanual benchmark suite that enabled fine-grained dexterous data collection.
...
**Foundation Simulators** — General-purpose physics platforms for robot learning. Each picks a different point on the (visual fidelity × physics accuracy × throughput) trade-off.
- [[2003.08515|SAPIEN]] — 2,346 articulated objects with [PhysX](https://developer.nvidia.com/physx-sdk)-accurate physics; the foundational platform for articulation-heavy research.
- [[1909.12271|RLBench]] — 100 visually-guided manipulation tasks on [CoppeliaSim](https://www.coppeliarobotics.com/) with infinite waypoint-generated expert demos; the few-shot evaluation standard.
- [[2604.08258|EvoGymCM]] — Evolutionary co-design of soft-body morphology + control; benchmark for evolving robot designs in simulation.
...
**Household-Scale** — Realistic home environments with diverse objects and tasks. Bridges the visual reality gap during training.
- [[2406.02523|RoboCasa]] — Photorealistic kitchen environments at scale with [MuJoCo](https://mujoco.org) backend; pioneered data-gen-as-benchmark, proving synthetic data substitutes for real demonstrations.
- [[2602.10116|SAGE]] — Agentic 3D scene generation pipeline; LLM proposes layouts and objects, asset retrieval populates, physics-aware placement enforces feasibility — sim environments at LLM-prompting rate.

**Photorealistic 3D Environments** — Visually realistic 3D worlds for navigation, HRI, and vision-based policies.
- [[2003.08515|SAPIEN]] — 2,346 articulated objects with [PhysX](https://developer.nvidia.com/physx-sdk)-accurate physics; the foundational platform for articulation-heavy research *(also listed under Foundation Simulators above)*.
- [[2406.02523|RoboCasa]] — Photorealistic kitchen environments at scale with [MuJoCo](https://mujoco.org) backend; the modern household-photorealism reference *(also listed under Household-Scale above)*.
...
**Generative & Real-to-Sim Simulators** — Build sim worlds *from real video*, sidestepping authoring entirely. A new category that doesn't use a hand-crafted physics engine — these synthesize the dynamics or rendering from learned video priors:
...
- [[2501.03575|NVIDIA Cosmos]] — World-foundation-model platform that couples generative video with parallel physics; learned video simulator for synthetic data scaling (10 FPS real-time autoregressive generation).
...
**Teleoperation-Friendly** — Environments designed for collecting human demonstrations with low-friction interaction loops.
- [[2310.06114|UniSim]] — Learned interactive real-world simulator from heterogeneous video; the original learned-sim benchmark that established the "video → simulator" paradigm.
...
- [[2511.04831|Isaac Lab]] (NVIDIA) — Successor to [Isaac Gym](https://developer.nvidia.com/isaac-gym); [PhysX-5](https://github.com/NVIDIA-Omniverse/PhysX) + RTX + [OpenUSD](https://openusd.org); **>900K FPS** state-based, up to **1.6M FPS** in distributed training. Foundational for **[[2503.14734|GR00T N1]]/N1.5** training and Mimic synthetic-data pipeline.
...
- **[Newton](https://developer.nvidia.com/newton-physics) (NVIDIA)** — Successor to [Isaac Gym](https://developer.nvidia.com/isaac-gym) physics; [PhysX-5](https://github.com/NVIDIA-Omniverse/PhysX) with full soft-body and fluid in one runtime; aims to be the "universal" sim.
...
- [[2601.11421|GM-100]] — 100 detail-oriented tasks (precise insertion, fine alignment, tool manipulation); current VLAs achieve very low SR, exposing real precision gaps.
...
- [[2605.10921|RoboMemArena]] — First comprehensive robotic-memory benchmark; **26 sim + 5 real** tasks where **68.9%** of subtasks require historical information; PrediMem hits **38.5%** TSR.
- [[2502.09560|EmbodiedBench]] — **1,128 tasks** across 4 environments × 6 capabilities; GPT-4o scores **>60%** on high-level planning but only **28.9%** on low-level manipulation.
- [[2508.13142|EASI]] — Holistic evaluation framework for spatial intelligence in embodied agents.
- [[2507.05258|REA]] — Robust evaluation arena for embodied agents under perturbation.
...
- [[2604.07335|TAMEn]] — Tactile-Aware Manipulation Engine for closed-loop bimanual data collection — see §6.
...
- [[2603.18892|MultihopSpatial]] — Multi-step spatial reasoning requiring compositional inference (e.g., "cup on table, table in kitchen, where is cup?").
- [[2511.04670|Cambrian-S]] — Spatial supersensing in video; extends spatial benchmarks from static frames to temporal sequences (tracking, occlusion reasoning).
- [[2601.09430|Video-MSR]] — Multi-step spatial reasoning benchmark for video understanding.
- [[2507.18342|EgoExoBench]] — Ego+exo cross-view spatial benchmark; tests viewpoint-invariant spatial reasoning.
- [[2602.20901|SpatiaLQA]] — Spatial language QA benchmark.
- [[2503.23765|STI-Bench]] — Spatial-temporal intelligence benchmark.
- [[2601.15224|PROGRESSLM]] — Progress-aware spatial reasoning evaluation.
- [[2603.19231|MonoArt]] — Monocular articulated-object spatial reasoning.
...
- [[2112.03227|CALVIN]] — Standard for long-horizon, language-conditioned policy evaluation with chained instruction sequences; most-cited compositionality benchmark.
...
- [[2505.15660|AGNOSTOS]] — Cross-task agnostic benchmark for VLAs; tests transfer across diverse instructions.
- [[2305.12821|FurnitureBench]] — Multi-step assembly tasks (drawers, chairs, lamps); the original long-horizon manipulation benchmark.
- [[2506.18088|RoboTwin 2.0]] — Bimanual sim + benchmark with strong domain randomization; the modern bimanual reference.
...
### 12. World Model Benchmarks

Evaluating whether learned world models generate physically plausible, action-consistent, long-horizon predictions.

- [[2603.22212|Omni-WorldBench]] — First interaction-centric WM evaluation; tests causal consistency and action following on generated frames.
- [[2506.00613|WorldGym]] — World model AS environment; evaluates by training policies *inside* the WM and measuring downstream transfer to real.
- [[2603.23497|WildWorld]] — 108M frames from *Monster Hunter: Wilds* with explicit state annotations; Action Following + State Alignment metrics.
...
- [[2510.19430|GigaBrain-0]] — World-model-powered VLA; integrated-stack evaluation across the WM+policy pipeline.
...
- [[2604.11689|LARY]] — Latent rollout evaluation for video world models.
- [[2603.09030|PlayWorld]] — Game-based world-model benchmark; play-and-predict evaluation paradigm.
...
### 13. Sim-to-Real Transfer Evaluation
...
- [[2511.04665|Real-to-Sim GS]] — Gaussian-splat soft-body digital twins from real video; closes the visual + physical gap for deformables in one stack.
- [[2506.06440|Vid2Sim]] — Generalizable video-based reconstruction of appearance + geometry + physics; videos → mesh-free simulators.
...
### 14. Real-World Evaluation Infrastructure
...
- [[2103.04918|Embodied AI Survey]] — A Survey of Embodied AI: From Simulators to Research Tasks; the foundational survey that mapped the early simulator landscape ([Habitat](https://aihabitat.org), [AI2-THOR](https://ai2thor.allenai.org/), [iGibson](https://github.com/StanfordVL/iGibson)).
