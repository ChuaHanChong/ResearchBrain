...
- **[[2605.15178|SANA-WM]]** — **2.6B** open-source Hybrid Linear Diffusion Transformer generating ==one-minute 720p videos== with precise 6-DoF camera control on a single GPU. Frame-wise ==Gated DeltaNet== fused with ==softmax attention== plus a dual-branch camera controller (Ray-Local UCPE + Raw-Frame Plücker Mixing); algebraic key-scaling factor $1/\sqrt{D \cdot S}$ prevents minute-scale state explosion. VBench Overall **80.62/81.89** matching 480p/8-GPU baselines; **22 videos/hour** on H100, **39x** distilled speedup on RTX 5090; first viable open-source minute-scale 720p WAM.
...
- **[[2605.15153|Pelican-Unified]]** — first single-model unification of understanding + reasoning + imagination + action via shared ==latent variable z== + ==Unified Future Generator==. VLM backbone emits chain-of-thought + dense z; a diffusion transformer conditions on z and jointly models future video and actions with one loss. **64.7** multimodal-VLM avg (**+28.2pp** on Where2Place), **93.5%** on RoboTwin 50-task dual-arm, **1st** on WorldArena imagination (EWM **66.03**, 3D Accuracy **98.13**); unseen compositional + zero-shot generalization on real robots.
- **[[2605.10942|HarmoWAM]]** — resolves the generalization-precision trade-off via ==dual action experts==: a ==predictive expert== on current-step latents for precise interaction and a ==reactive expert== on *future predicted frames* for generalizable exploration. A ==Process-Adaptive Gating Mechanism== dynamically switches based on task stage (transit vs interaction). **89%** in-domain average across six real-world tasks with only **7.9%** OOD drop — smallest generalization gap reported among unified WAMs.
- **[[2603.08572|MetaWorld-X]]** — hierarchical world model that handles ==negative interference== in monolithic humanoid RL by decomposing control into ==Specialized Expert Policies (SEPs)== + a VLM-guided ==Intelligent Routing Mechanism (IRM)==. SEPs trained via ==energy-based imitation rewards== (H2O framework) for biomechanical fidelity; IRM dispatches via ==hierarchical semantic distillation==. 'Walk' converges in **0.5M steps** vs TD-MPC2's **1.8M**; **9/10** on Walk/Run/Carry; **470.0** return on 'Door' vs TD-MPC2's **285.0**. The lesson: for high-DoF multi-skill control, let the VLM compose experts rather than learn a monolithic policy.
...
- **[[2603.08403|SPIRAL]]** — closed-loop reflective planning: agent generates long-horizon action-conditioned video plan; ==CriticAgent== evaluates for temporal coherence and action completeness; failed plans regenerate with critic feedback incorporated. The canonical generate → critique → regenerate template; key insight is that *judging* plan quality is easier than *generating* perfect plans, so VLM reasoning can score "physical sense" even when the generator can't produce perfect physics first pass.
- **[[2502.05907|EvoAgent]]** — three-part loop with continual world model: (1) ==self-planning== uses WM to propose a plan, (2) ==self-control== executes while monitoring prediction error, (3) ==self-reflection== compares predicted vs actual outcomes and updates WM + policy. Continual WM is the key enabler — it provides both the prediction-error signal for control and the training signal for reflection. **+105%** on long-horizon tasks; the loop contributes **72%** of total gain (Minecraft setting).
