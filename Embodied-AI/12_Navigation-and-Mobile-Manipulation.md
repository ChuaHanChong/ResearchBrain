...
- **[[2607.23743|Traversability-Aware Global Planner]]** — A ==dual-stream network== fusing satellite imagery, aerial LiDAR and OSM into a continuous ==traversability map== plus goal-conditioned route likelihoods, trained on human GPS tracks with a ==Positive-Unlabeled loss== and ==geometric priors==; **61.3 m** Fréchet to human paths (**21–43%** better), interventions every 140→**480 m**.
...
- **[[2607.25448|RPV-V2]]** — Training-free object-goal navigation swapping VLM semantic similarity for true spatial co-occurrence: ==Room Probability Vectors== over a compact room lexicon score detected objects against the target, propagated geodesically by the ==Fast Marching Method== into a frontier-guiding value map; **0.539** SR / **0.307** SPL on HM3D (**+3%** / **+1.3%** over VLFM).
...
- **[[2607.25219|SONG]]** — A ==3D Gaussian Splatting== social-navigation simulator whose pedestrians get ==LLM-generated intent== driving generative full-body motion, wired into ==Isaac Lab== physics with the stratified **SONG-Bench**; leading vision policies score **<22%** SR even on Easy (**>75%** collision failures), and fine-tuning NavDP lifts real single-pedestrian SR **10→20%**.
...
- **[[2607.24207|FloAff-Kitchen]]** — Answers *where should the base stand* with a ==Canonical Floor Affordance Representation== cropping local geometry and aligning target orientation for viewpoint invariance, plus ==Progressive Floor Affordance Learning== distilling pick-and-place priors into harder skills; RMSE **0.147→0.073**, **0.85** SR on Styles vs NavAff's **0.37**.
- **[[2607.13154|WANDA]]** — Turns *one* RGBD demo into an open-world mobile-manipulation data engine: ==Gaussian Splatting== + ==BundleSDF== + ==VLM-guided spatial configuration== rebuild varied scenes and ==Corrective State Expansion== injects policy drift; **75.6%** sim SR at **100** demos (matching 40–60 teleop), **54.8%** real progress over **16** environments, **40%** cross-embodiment.
