...
> Use search when (1) the task is **safety-critical** (medical, autonomous driving), (2) the **policy is known to be miscalibrated** under distribution shift, or (3) **inference latency is acceptable** (planning, not real-time control). Skip it for fast pick-and-place where imitation suffices. SEAL specifically helps when **CoT and actions disagree** — the failure mode for reasoning VLAs in novel scenarios.
...
| Runtime alignment verification (SEAL) | High | ~1.5-2x (K=10, 347ms/step) | [[2510.16281\|SEAL]] |
| Test-time search (MCTS) | Highest | 3-5x | [[2509.22643\|VLA-Reasoner]] |

> [!success] The 2026 Recipe
> If latency matters: ==Latent reasoning + dual-modal auxiliary supervision== (OneVL pattern). If interpretability matters: ==Output-head visual CoT== (CoT-VLA pattern). If recovery from miscalibration matters: ==Test-time MCTS== (VLA-Reasoner pattern). RL-train the reasoning trace with verifiable step rewards ([[2509.25852|REVER]] + CIR/SR).
