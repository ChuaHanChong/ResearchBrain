...
- **[[2506.05997|SRU]]** — ==Spatially-Enhanced Recurrent Units== add a spatial-transformation term to LSTM/GRU so ego-observations align implicitly; PPO-trained with sparse rewards, hit **+23.5%** SR over vanilla RNNs and **+105.0%** over GTRL, with zero-shot transfer to a legged-wheel robot over **100+ m** — recurrent memory made spatially aware without an explicit map.
- **[[2604.02829|STRNet]]** — A ==graph-based spatial aggregation== module plus a ==hybrid temporal-shift== fusion enriches the visual encoding fed to a nav policy; reached **100%** SR with zero collisions on basic tasks and **98%** SR (**0.02** collisions) on long-range tasks, at real-time speed with fewer parameters than baselines.
- **[[2604.24391|FreqCache]]** — ==Frequency-domain token caching== with migration-aware reuse accelerates VLN VLMs; **1.59× speedup** (per-step latency **637 ms → 401 ms**), **53.5%** token reuse, while holding **76.0%** Oracle Success — the inference-efficiency axis now matters as VLN backbones grow.
- **[[2305.06341|GGCS]]** — Extends ==Graph of Convex Sets== motion planning to Riemannian manifolds via an atlas of local isometries, solving non-Euclidean configuration spaces (SE(2) bases, revolute joints) as a mixed-integer convex program; planned for a **15-DoF** PR2 mobile manipulator in **25–66 s** with optimality guarantees — the classical-planning anchor of the design space.

#### 1.3 Surveys

- **[[2311.00530|LLM Embodied Navigation Survey]]** — Categorizes LLM roles in navigation into ==grounded language understanding== and ==few-shot planning==, contrasts LLM-based vs traditional ==VLN== models, and audits datasets; flags ==spatial reasoning== and ==computational efficiency== as the persistent gaps — the reference map of the LLM-navigation landscape.
...
> - [[2603.07799|MWM]] — Demonstrated that action-conditioned consistency, not raw fidelity, is what makes a world model usable for navigation planning.

> [!tip] Safety Moved From Reward Shaping to Formal Constraint
> The strategic shift in 2025–2026 navigation policy learning is that safety is no longer a penalty term in the reward — it is a *constraint the policy is trained and certified against*. [[2605.14174|VIA]] couples CVaR-constrained RL with reachability verification to get a provable safety rate; [[2508.05634|Conformal Crowd Navigation]] turns prediction uncertainty into a constraint via conformal inference. The trade-off is explicit: constrained policies sacrifice some navigation efficiency for collision guarantees that survive OOD and sim-to-real. Compose this with world-model imagination ([[2603.07799|MWM]]) to plan *around* the constrained safe set rather than discovering it by trial. For the broader RL-for-embodiment methodology underneath these policies, see [[03_Imitation-Learning-and-RL#6. RL for Locomotion, Navigation & Whole-Body Control]]; for sim-to-real transfer of navigation policies specifically, see [[14_Sim-to-Real-Transfer#3. Policy-Side: Robustness & Domain Randomization]].
