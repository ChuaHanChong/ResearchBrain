...
LLM/VLM-based task planners that reason in natural language about a task before or during execution, in a module separate from the low-level action-executor — distinguished from §1.3 Output-Head Reasoning (single backbone jointly emits reasoning and actions) and from §5 (training-time reasoning supervision).

- **[[2608.20084|EAFG]]** — A pre-planning ==evidence-acquisition== loop where a VLM proposes reversible exploratory subgoals executed via ==TAMP (PDDLStream)==, then ==feasibility gating== emits ready-to-plan/need-more-evidence/halt; underspecified-goal recipe CR **0.05 → 0.40**, halt SR **0.45 → 0.90** — halting on a confirmed-absent object is the contribution, not a fallback.
...
- **[[2608.11739|G0.5]]** — A single autoregressive decoder emits ==native chain-of-thought== (Subtask, BBox, Trace, ActionHint) interleaved with cross-embodiment ==ActionCodec== tokens under one next-token loss; **82.5%** held-out DROID transfer, **98.9%** LIBERO, CoT toggled at inference lifts multi-stage progress **2.4→3.8**.
...
- **[[2502.03729|RAD]]** — Trains a VLA on ==partial reasoning chains==: full reasoning+action on robot demos, ==reasoning-only== on action-free human video whose ==MovePrimitive/GripperPosition== labels come from ==HaMeR== hand-tracking; up to **+27%** over zero-shot ECoT on human-only tasks and **+30%** on new objects.
...
- **[[2604.22238|CodeGraphVLP]]** — A hierarchical VLA pairing a ==persistent semantic-graph state== (Cutie-tracked, multi-view CLIP association) with a one-time ==LLM-synthesized code planner== querying it for progress, plus ==clutter-free visual prompting== masking subtask-irrelevant objects; **81.7%** avg real-world SR (best baseline **56.7%**), **10×** faster planning than VLM-in-loop.
- **[[2602.21013|Notes-to-Self]]** — Augments VLAs with an explicit ==language scratchpad== (grounding/plan/act sections) accumulated via a ==done-token update== trigger, giving stateless and recurrent VLAs spatial + temporal memory; **+48.8%** avg SR on ClevrSkills-Mem (non-recurrent), **+11%** recurrent, real-world pick-place-restore **0%→65%**.
...
#### 1.7 Safety-Aware Planning

A task-specialized form of the input-prompting slot: a dedicated safety-checking LLM/VLM audits or screens the primary planner's output *before* execution — via a risk-scoring graph, a separate safety-planning LLM, formal-logic invariant checks, or a state-diff-reading judge — rather than trusting the planner's own reasoning to self-certify safety.
...
- **[[2606.04226|PerceptTwin]]** — A real2sim pipeline reconstructing interactive AI2Thor scenes from open-vocab 3D scene graphs (==CLIP/TRELLIS mesh generation== + ==constrained ICP== placement), with an isolated ==LLM judge== reading state-diff rollouts to flag Correct/Incorrect/Unsafe plans; lifts LLM planning success **+39pp**, catching jailbreak-style unsafe plans.

#### 1.8 Multi-Agent / Multi-Robot Orchestration

A variant of the input-prompting slot where the reasoning is distributed across multiple specialized LLM/VLM agents — planner, verifier, allocator, per-role experts — coordinating over shared state rather than one model reasoning alone. Multi-agent decomposition trades a larger token/coordination budget for role-specialized reasoning and parallel verification.
...
#### 2.1a Discrete Future-Frame Subgoal Prediction
...
- **[[2607.12800|UniVR]]** — Trains a unified generative model (**Emu3.5**) to reason *natively in visual space* — no textual CoT — via ==SFT== on the **VR-X** dataset then ==VR-GRPO== RL with a ==next-token-prediction-in-visual-space== objective and a global + step-focal reward; **+18.4%** VLM score / **+20.61** JEPA physical-alignment over Emu3.5, **+23.9%** on sequences **>60s**.

#### 2.1b Visual Trace / Trajectory CoT

Reason via a continuous visual trace — a trajectory or point-sequence overlay — rather than a discrete future frame; the trace itself is the intermediate reasoning artifact conditioning the action.
...
> Visual CoT shines for **multi-stage manipulation** where each stage has a visually distinct goal state ("first the cup is grasped, then it's at the lip of the kettle, then it's pouring"). For continuous skills (polishing a surface), visual subgoals are too abrupt — use latent reasoning instead (§3). Cross-reference [[04_VLA#4. Reasoning & Planning-Augmented VLAs]] for the broader reasoning-and-planning landscape that feeds into visual CoT, and [[06_WAM#5.1 Visual Chain-of-Thought]] for how WAM-integrated visual subgoal generation composes with world-model-augmented VLAs. For the physics-grounded variant, where the intermediate step is a physical prediction rather than a subgoal image, see [[08_Physics-Aware-Embodied-AI#5. Physics-Aware Reasoning]].
...
- **[[2608.11671|StellaVLA]]** — A retrieval-augmented VLA whose ==spatial-language expert== (subtask + verbalized 2D/3D motion, cross-entropy) trains a shared backbone in parallel with the action head, then is stripped at inference behind a ==KV-cached== demonstration prefix; **1st** on VLA-Arena (**0.63**), **85.1%** zero-shot LIBERO-Plus.
- **[[2608.05369|W²-VLA]]** — A VLA whose ==compact latent interface== (task-conditioned tokens) is supervised by a ==W²-CoT== pipeline (subtask + physical-transition reasoning + wrist evidence, training-time only) and a ==V-JEPA wrist predictor== forecasting future wrist latents without inference-time decoding; **98.5%** LIBERO, **60.71%** RoboTwin 2.0, **87.43 Hz**.
...
- **[[2501.02486|LLMPC]]** — Casts ==LLMs as approximate optimizers== inside a ==Model Predictive Control== loop: sample K plans, simulate, score, execute the best, replan — the skeleton VLAPS/SVA later wrap around a pretrained VLA; trip-planning SR **14.5%→44.6%**, meeting-planning **67%** — multi-plan sampling closes the LLM-optimization gap.
...
- **[[2608.03924|ETA]]** (OpenETA) — A ==planner-centered, closed-loop architecture== (Agent/Interface/World) enforcing a strict ==runtime invariant== of fresh observation after every world-changing action, orchestrating Tools/Skills via ==execution gates== instead of end-to-end learning; **90.0%** PASS@5 on 130 LIBERO tasks (3 Tools), scaling to 44 Tools drops SR to **14.0%**.
...
- **[[2608.07905|Graph-Enhanced Embodied Planning]]** — Hierarchical LLM planner grounding prompt+reward in an LLM-built ==task graph== (==graph-based GRPO==: node/edge validity + instruction reward) plus an inference-time ==verify-feedback-correct== loop; **68.52%** ALFRED unseen SR (**+6.17pp**), **90.04%** long-horizon accuracy vs SFT's **20.57%**.
...
> CIR/SR's finding is sobering: a VLA trained to maximize task success can develop reasoning traces that *look* correct but are causally disconnected from the final action. Step-level rewards on the *reasoning process* are required for trustworthy reasoning. Cross-reference [[06_WAM#7.3 RL-Driven & Co-Evolving]] for RL-driven WAM co-evolution patterns ([[2603.19370|VAMPO]]) and [[16_Self-Evolving-VLA-WAM#3. Core Mechanisms of Self-Evolution]] for how reasoning-traced training composes with self-evolution loops.
...
- **[[2506.17486|PRISM-SLM]]** — Self-distills a cloud LLM planner into an on-device SLM via ==closed-loop plan elicitation== (==environment masking== + ==plan validation==) and ==LoRA SFT==, no manual annotation; **92%** vs GPT-4o's **98%** SPINE success at **<200ms** vs **3.64s** query time.
...
- **[[2602.08167|R&B-EnCoRe]]** — A self-supervised VLA treating reasoning as a ==latent variable== refined via ==importance-weighted variational inference (IWAE)== with ==reasoning dropout== distilling concise traces; latency **~5s→~3s/step**, SR **80.3%**, OOD SR **76.9%** (baseline **69.2%**) — the OOD-recovery gain matches this tier, not §6.1's.
...
- **==Reasoning vs reflex==** — When should the VLA reason, and when act reflexively? "Always reason" is slow; "never reason" is brittle. ==Adaptive reasoning depth== ([[2604.28192|LaST-R1]], [[2506.13757|AutoVLA]], [[2505.11917|OneTwoVLA]]) — RFT-with-CoT-penalty and decision-token gating (§5.5) cut latency — but the gate is learned implicitly from reward, not a principled trigger. That implicit gate is itself an attack surface: [[2607.01518|Overthink-Triggered Slowdown Attack]] shows a black-box genetic-algorithm search over physically-realizable scene-text triggers forces LVLM "overthinking," a **6.96×** latency blowup transferable across Kimi-VL/Qwen3-VL — no trained gate has been shown robust to an adversarial trigger, so the reason-vs-reflex problem stays open even where §5.5's methods solve it for benign inputs.
...
- **==Cross-modal reasoning==** — Most VLA reasoning is vision-centric; reasoning over force, tactile, and audio modalities (relevant for contact-rich tasks) is underexplored. This is the orthogonal frontier to the faithfulness cluster — see [[11_Contact-Rich-and-Tactile-Control#3. Force-Conditioned VLA Architectures]].
...
| Reasoning over force / tactile / audio | Cross-modal reasoning — research gap; see [[11_Contact-Rich-and-Tactile-Control#3. Force-Conditioned VLA Architectures]] for the contact-modality bridge |
...
- [[16_Self-Evolving-VLA-WAM]] — Self-evolution; reasoning enables self-critique and proactive correction
...
- [[11_Contact-Rich-and-Tactile-Control]] — Force/tactile policies deep-dive; reasoning over multi-sensor context
