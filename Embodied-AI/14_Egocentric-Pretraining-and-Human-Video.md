...
### 2. Egocentric Data & Perception Substrate

The data foundation, plus the perception layer that operates on it. **Datasets** (§2.1–2.6) — raw egocentric video corpora, each specializing in a different scale-modality-coverage trade-off. **Perception & Forecasting Primitives** (§2.7–2.9) — models that convert that raw video into structured signals (full-body pose, object tracking, hand-dynamics forecasting), the upstream layer that embodiment-alignment and retargeting pipelines (§5) consume.
...
- **[[1704.02463|FPHA]]** — The foundational first-person hand-action benchmark: **1,175** RGB-D videos (**105,459** frames, **45** actions, **26** objects) with 21-joint 3D pose from ==magnetic-sensor mo-cap== + ==inverse kinematics==; pose beats appearance (**85.39%** vs RGB two-stream's **75.30%**) and 6D object pose lifts **87.45% → 91.97%**.
...
- **[[2604.23570|EgoLive]]** — **1,680 hours** / **65,866 episodes** / **346 tasks** of real-world service-scenario egocentric video via custom ==JoyEgoCam== stereo rig + automated ==HaMeR/MANO== + ==ORB-SLAM3== 6-DoF tracking pipeline; depth error **3.0–8.8mm**, longer semantic tail than EgoDex/Xperience-10M — largest annotated real-world egocentric manipulation corpus to date.
...
- **[[2406.09598|HOT3D]]** — Egocentric 3D hand-object tracking from ==Project Aria== glasses + Quest 3: **1.5M** multi-view frames (**3.7M** images), **19** subjects, **33** scanned objects, with ==marker-based motion capture== fitted to ==UmeTrack== and ==MANO==; **1.16M** frames fully annotated — the world-space substrate hand reconstructors are scored on.
...
Robot-free egocentric corpora collected or synthesized specifically to bridge the human-robot embodiment gap.
...
- **[[2608.02580|Ego2Robot]]** — A scalable synthesis pipeline converting egocentric human manipulation video into robot-specific training data across **15** morphologies via ==action alignment==, ==visual alignment==, and ==quality curation==; **18,561 hours** synthesized from **~1,940 hours** of human video, **+2.6pp** RoboTwin (**53.5%**), **+11.2%** cross-embodiment transfer.
- **[[2608.12122|HandEdit]]** — A **200M**-instance, 26-embodiment benchmark for egocentric human-to-robot dexterous-hand image editing via ==SAM3 segmentation==, ==ProPainter inpainting==, ==MANO-to-URDF retargeting==, and a ==Harmonizer== pseudo-GT; GPT-Image-2 leads Hand-only (**0.780** structural fidelity), but Hand-Arm ID fidelity falls **0.852 → 0.563**.
...
#### 2.6 Egocentric Assistant & Collaboration Datasets
...
*The following three sub-sections shift from raw video corpora to perception primitives — models that convert first-person video into structured body-pose, object-tracking, and hand-dynamics signals that downstream retargeting and policy-training pipelines consume.*

#### 2.7 Egocentric Full-Body Pose Estimation
...
- **[[2607.15868|EgoExoMoCap]]** — Fuses continuous head/wrist HMD tracking with intermittent exocentric views from *other* co-located wearers via ==wearer-conditioned ray canonicalization== and ==DINOv3== visibility gating over a ==spatial-temporal transformer==; **5.72 cm** Nymeria MPJPE (vs **7.74 cm** ego-only), **8.80 → 7.11 cm** as observers scale.
- **[[2607.13472|EgoHTR]]** — A multi-modal 4D human-terrain reconstruction pipeline (Project ==Aria== glasses + ==Rokoko== IMU MoCap + ==Leica BLK2GO== scanner) producing centimeter-precise, terrain-grounded human motion via ==SMPL-X== + temporal/spatial alignment; **73.2mm** MPJPE, and perceptive locomotion policies trained on it deploy on a real Unitree G1.
...
The object-level counterpart to §2.7's body-level perception: models that turn raw first-person video into the persistent object identities and grounded interaction regions that memory-augmented and affordance-grounded policies consume.

- **[[2608.13014|EgoPHI]]** — Estimates dense per-vertex contact + 3D force on hands *and* manipulated objects from one egocentric RGB image via ==graph-based intra/inter-mesh attention== and ==SOFA physics-simulated== force supervision; cuts hand-force MAE **6.62 N → 4.03 N**, transfers to real instrumented objects at **0.48–1.35 N** MAE with no tactile sensing.
- **[[2608.04533|EgoAfford]]** — A multi-step ==task-oriented affordance grounding== benchmark (**15,537** synthetic + 102 real images) pairing remaining-plan inference with part-level object/instrument/destination masks; reference model **EgoLens** (Qwen2.5-VL + role-specific decoders) reaches **0.700** gIoU, **+14.7pp** over commercial VLM+SAM2, **0.666** zero-shot real gIoU.
...
- **[[2603.25539|PAWS]]** — Training-free 3D object-articulation extraction from monocular egocentric video: ==MANO== hand-trajectory smoothing + ==LO-RANSAC== line triangulation/Manhattan-world axis recovery + two-stage ==VLM== motion-type/axis reasoning; beats [[2509.01708|ArtiPoint]] on HD-EPIC (**71.43% vs 70.66%** match, **0.36 vs 0.00** MAO).
- **[[2509.01708|ArtiPoint]]** — Estimates articulated-object 3D motion from egocentric in-the-wild RGB-D video by lifting ==deep any-point tracking== (CoTracker3) to 3D and jointly optimizing a shared 1-DoF articulation model over an interaction segment via a ==factor graph==; **14.54°**/**17.14°** angular error (prismatic/revolute) on the new **Arti4D** dataset, beating all baselines.

#### 2.9 Predictive Hand Dynamics & Interaction Timing

Forecasting *future* hand motion and localizing *when* hand-object contact happens — the temporal complement to §2.7's body-pose recovery and §2.8's spatial object-grounding, both validated directly on downstream robot-policy execution.

- **[[2511.12878|Uni-Hand]]** — A ==dual-branch diffusion== model jointly forecasting head egomotion + multi-joint hand motion (2D/3D waypoints, contact states) via a ==Mamba-Transformer== + text-conditioned ==VL-fusion==; SOTA ADE/FDE across five egocentric benchmarks, **100%/90%/80%/80%/20%** SR on Hand-ALOHA-Transfer vs S-Mamba's **40%/20%/0%/0%/0%**.
- **[[2508.12349|EgoLoc]]** — Zero-shot hand-object contact/separation timestamp localization via ==HaMeR==-derived wrist velocity/acceleration sampling + a closed-loop ==VLM== localization-feedback loop; EgoPAT3D-DT SR(γ=5) **0.819** (vs T-PIVOT's **0.755**), and EgoLoc-labeled timestamps drive **10/10** real-robot pick-place SR vs **2/10** thresholding.
- **[[1703.01040|Convolutional Future Regression]]** — Regresses an ==SSD + convolutional-autoencoder== scene code **one second** forward, decodes it into future hand boxes, and feeds them with joint angles to a manipulation net on a Baxter; **30.90** F-measure (vs SSD's **13.23**), **3.29/5** in a **12-participant** study — the earliest ego-video-to-motor-command loop.
...
- **[[2608.05674|JoyAI-RA 0.5]]** — A ==Vision-Language-World-Action (VLWA)== framework with ==dual action alignment== (implicit latent + explicit physical) pretrained on **53,000+ hr** of human ego-video (surpassing EgoScale's 20,854 hr); scaling **10% → 100%** monotonically lifts seen-task score **47.8 → 85.6**, unseen **37.6 → 60.2**.
...
#### 4.1 Generation 1: Frozen-Feature Transfer (2022–2024)
...
- **[[2407.18911|HRP]]** — Mines contact/future-wrist-pose/active-object affordances from **150K** Ego4D clips via off-the-shelf detectors (==GMM== contact, ==FrankMocap==), distilling them into frozen R3M/MVP/VC-1/DINO/CLIP encoders via ==LayerNorm-only fine-tuning==; lifts every baseline **≥15%** (up to **+20%**), HRP+ImageNet DP **56% → 76%** SR.
- **[[2406.00439|MPI]]** — Jointly trains a ==Prediction Transformer== (unseen transition-frame pixels) + ==Detection Transformer== (interaction-object box) on Ego4D-HOI keyframe triplets via bidirectional cross-attention; **+26.3%** avg real-robot SR, Franka Kitchen **78.9% vs 71.7%** (MVP), grounding AP@0.75 **74.40% vs 60.37%** (MVP).
...
- **[[2604.10809|WARPED]]** — Reconstructs a ==Gaussian Splat== scene, optimizes ==MANO== hand-object pose, retargets to the end-effector via thumb-index mapping, then renders photorealistic wrist-view images for ==diffusion-policy== training with no teleoperation; matches/exceeds teleop on **4/5** tasks (**20/20 vs 16/20** Rotate Box) at **5–8x** faster collection.
...
- **[[2508.07626|AR-VRM]]** — Pretrains a keypoint-predicting VLM on ==Ego4D InterHand== keypoints, then bridges to robots via a learned ==analogical-reasoning== matrix mapping human hand keypoints to robot arm components; CALVIN unseen-scene SR **61.2% → 65.9%**, **10%**-data SR **40.0% → 45.6%**.
...
- **[[2501.02973|HaWoR]]** — Recovers world-space hand motion from egocentric video by decoupling camera-frame ==MANO== regression (==temporal self-attention== over ViT features) from ==adaptive egocentric SLAM== that masks hands out of ==DROID-SLAM=='s bundle adjustment; **33.20** W-MPJPE on [[2406.09598|HOT3D]] vs **119.41**, at **40 ms**/frame.
...
- **[[2509.10952|ImMimic]]** — An embodiment-agnostic ==co-training== ==Diffusion Policy== joining human video + limited robot data via ==hand-pose retargeting== and a ==mapping-guided MixUp== (==Dynamic Time Warping== trajectory alignment + progressive interpolation) forming a continuum of intermediate domains; reaches **100%** SR on Pick-and-Place with as few as **5** robot demos.
...
- **[[2608.05706|LAWM-3D]]** — A ==3D-aware Latent Action Model== (multi-view VAE, random view masking, non-injective depth prediction) ==geometrically aligned== to a frozen ==VGGT== backbone, pretraining a Cosmos-Predict2.5 WAM on Ego-Exo4D/Assembly101/EgoDex before robot fine-tuning; consistently beats DreamDojo across three OOD benchmarks (higher PSNR/SSIM, lower LPIPS).
- **[[2608.05215|VLAff]]** — A Qwen2.5-VL + DINOv2 model with ==visual/grasp/trajectory affordance tokens==, trained on **EgoAffordance** (**200K+** egocentric episodes) with affordances normalized to a 3D anchor point for embodiment-agnostic transfer; **68.0%** real zero-shot manipulation SR, beating VidBot (**52.0%**) and VRB (**28.0%**).
- **[[2608.00486|DreamTraj]]** — A ==Reader== extracts object-motion + scene-context signals from an early denoising step (**k=16**) of a frozen Wan2.2 video-diffusion backbone, decoded by a small **7.1M**-param ==flow-matching DiT== into a 6-DoF object trajectory; beats privileged baselines (**3.04 cm** ADE vs EgoScaler's **7.52 cm**) at **4.6×** faster inference, no full video synthesis.
...
- **[[2509.19626|EgoBridge]]** — Aligns human-robot policy latents via a ==Joint Optimal Transport== loss with ==Dynamic-Time-Warping==-weighted pseudo-pairing over a shared DETR-style trunk, absorbing the domain gap inside the representation rather than at a projection layer; **+7–44pp** SR over EgoMimic/MimicPlay/ATM, **33% vs 0%** on human-only drawer locations.
- **[[2508.13534|MimicFunc]]** — Extracts ==functional keypoints== from a single human demo video into a function-centric local coordinate ==function frame==, then aligns test-tool and demo function frames to synthesize robot actions, sidestepping geometric/visual tool similarity; **79.5%** novel-tool SR, **+41.6–50.0%** visuomotor-policy gains from generated demos.
- **[[2507.19817|Ag2x2]]** — A ==coordination-aware agent-agnostic encoder== that inpaints human appearance out of EPIC-KITCHENS frames (==HaMeR== + ==ODISE== + ==E2FGVI==) while keeping both hands' 2D coordinates as position tokens, then shapes ==PPO== reward by goal-embedding similarity; **73.5%** zero-shot bimanual SR over 13 tasks vs **63.2%** engineered rewards.
...
- **[[2504.04612|Tool-as-Interface]]** — Abstracts human tool-use video to a task-frame, ==tool-centric 6D action representation== invariant to camera/robot-base motion, learned via ==Grounded-SAM== masking + ==3D-aware view augmentation== (MASt3R + Gaussian splatting) feeding a ==diffusion== policy; **13/13** nail-hammering SR, **81%** faster data collection than teleoperation.
- **[[2405.01527|Track2Act]]** — A ==diffusion== model forecasting 2D point tracks from mixed human+robot web video (Something-Something, Epic-Kitchens, RT-1, BridgeData), converted to open-loop rigid-transform actions and corrected by a closed-loop residual policy trained on **~400** demos; **70%** SR under mild generalization vs **0%** goal-conditioned BC.
...
- **[[2607.08857|AgenticFocus]]** — Object-preserving mixed reality: a VLM picks the task object, ==SAM2== tracks it while masking inpainting to hand-minus-object pixels, then full-hand retargeting to Unitree G1 + BrainCo hands via camera-relative ==MuJoCo IK== and layered compositing; **-5.18** SPARC vs Masquerade **-5.56**.
...
- **[[2608.13049|H2R-Bench]]** — A 240-case benchmark scoring whether video-WAMs turn egocentric human demos into valid robot videos via goal/action/==functional-contact==/embodiment MLLM-judged metrics (**H2RCore**); across 11 generators scores span **30.0–84.6** while video quality barely varies (rank correlation **0.14**) — polish is not evidence of transfer.
...
- **[[2503.11423|TASTE-Rob]]** — A **100,856**-clip fixed-viewpoint task-aligned HOI dataset paired with a three-stage pose-refinement ==video-diffusion== pipeline (coarse ==DynamiCrafter== I2V → motion-diffusion pose refinement → ==ControlNet==-style regeneration); pose refinement cuts grasp-classification error **67.8% → 9.7%**, lifting downstream robot SR **84% → 96%**.
...
- **[[2512.17907|DWM]]** — Factorizes the egocentric simulator into a ==dynamics model== predicting only ==residual state changes== from rendered ==hand-mesh== motion and an ==observation model== rendering them along a camera trajectory, initialized from a ==video-inpainting== diffusion transformer as identity prior; **21.654** PSNR on unseen dynamic-camera Aria vs **18.129**.
